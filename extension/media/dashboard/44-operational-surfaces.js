...
(() => {
  ...
  function workflows(context) {
    ...
    const metrics = `<div class="metric-grid compact">${card('DEFINITIONS', number(counts.workflow_definitions), 'project + skill definitions')}${card('VALIDATOR BINDINGS', number(counts.workflow_validator_bindings), 'non-runtime validation')}${card('RUNTIME BINDINGS', number(counts.workflow_runtime_bindings), 'explicit executor bindings')}${card('RUNNABLE REVISIONS', number(counts.workflow_runnable_revisions), 'current admitted workflow revisions')}${card('DURABLE RUNS', number(counts.workflow_runs), 'retained workflow execution records')}</div>`;
    const setup = '<button class="primary" data-action="setupStudio">Set up or verify runnable Agent + Workflow</button>';
  ...
  function studioLifecycle(context) {
    ...
    return `<div class="metric-grid compact">${card('AGENT RUNS', number((state.snapshot?.counts || {}).agent_runs || 0), 'durable agent execution history')}${card('WORKFLOW RUNS', number((state.snapshot?.counts || {}).workflow_runs || 0), 'durable workflow execution history')}${card('SKILL CANDIDATES', number((state.snapshot?.counts || {}).skills || 0), 'draft and promoted revisions')}${card('THIS VIEW', number((state.studioHistory || []).length), 'recent actions retained by this webview')}</div>${section('Studio lifecycle', 'DURABLE RUNS, RECEIPTS, AND EXACT TARGET ACTIONS', `<div class="action-grid"><button data-action="openStudioRuns" data-kind="agent">Browse durable agent runs</button><button data-action="openStudioRuns" data-kind="workflow">Browse durable workflow runs</button><button data-action="openStudioRuns" data-kind="skill">Browse durable skill runs</button></div><p>Durable run history is loaded from the host by the controls above. Lifecycle actions appear on exact catalog revisions after their authenticated context is loaded.</p>`, '<span class="count-chip">durable lifecycle surface</span>')}${section('Actions in this webview', 'RECENT LOCAL CONTEXT · DURABLE RUNS REMAIN AVAILABLE ABOVE', history || empty('No lifecycle action has been taken in this webview yet. This does not mean durable Studio history is empty; use the run browsers above.'), `<span class="count-chip">${number((state.studioHistory || []).length)} local</span>`)}${section('Lifecycle notes', 'SURFACE IS A CONTROL HUB', 'Open catalog records to start exact revision lifecycle actions such as admit, validate, start, approve, promote, rollback, and compare. No host action is executed until a matching catalog context is selected.', '<span class="count-chip">control-aware</span>')}`;
  }
  ...
  function plugins(context) {
    ...
    const inventoryIntegrity = `<div class="metric-grid compact plugin-inventory-integrity"><article class="metric-card" data-plugin-integrity="generation"><span>INVENTORY GENERATION</span><strong>${inventory?.freshness?.generation === undefined ? 'Unavailable' : number(inventory.freshness.generation)}</strong><small>${esc(freshness)} · ${esc(inventory?.discovery?.captured_utc || inventory?.generated_utc || 'capture unavailable')}</small></article><article class="metric-card" data-plugin-integrity="shard-hash"><span>SHARD IDENTITY</span><strong>${shardIdentityMatches ? 'MATCH' : 'NOT CURRENT'}</strong><small>${esc(state.environmentData.extensions?.snapshot_hash || 'shard hash unavailable')} · compact ${esc(inventory?.snapshot_hash || 'inventory hash unavailable')}</small></article></div>`;
    return `<div class="metric-grid compact">${card('DETECTED / ACTIVE', inventoryCurrent ? `${number(extensions.length)} / ${number(active)}` : 'Unavailable', inventoryCurrent ? `current generation ${number(inventory?.freshness?.generation)}` : `${freshness}; records suppressed`)}${card('CAPABILITIES', inventoryCurrent ? number(capabilities) : 'Unavailable', 'declared extension contributions')}${card('CONFLICT SIGNALS', inventoryCurrent ? number(conflicts) : 'Unavailable', inventoryCurrent ? (conflicts ? 'inspect affected extensions' : 'none reported') : 'refresh required')}${card('MCP SERVER', mcpLabel, esc(mcp.detail || mcp.status))}</div>${inventoryIntegrity}${conflictLifecycle}${rollbackLifecycle}
