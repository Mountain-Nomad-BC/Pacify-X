...
(() => {
  ...
  function projection(data, context) {
    ...
    const edgeBundles = [...drawableEdges.reduce((bundles, edge) => {
      const relation = String(edge.relation || 'related_to');
      if (!bundles.has(relation)) bundles.set(relation, []);
      bundles.get(relation).push(edge);
      return bundles;
    }, new Map()).entries()].sort((left, right) => right[1].length - left[1].length || left[0].localeCompare(right[0]));
    const requestedRelation = String(state.graphRelation ?? state.graphRequest?.relation ?? data.requested_relation ?? '');
    ...
    const edgeBundleRows = edgeBundles.map(([relation, bundledEdges]) => {
      const readableRelation = relation.replaceAll('_', ' ');
      const examples = bundledEdges.slice(0, 12).map(edge => `<li><b>${esc(edge.source)}</b><span>${esc(readableRelation)}</span><b>${esc(edge.target)}</b></li>`).join('');
      return `<details class="graph-edge-bundle" data-edge-bundle="${esc(relation)}"><summary><span>${esc(readableRelation)}</span><b>${number(bundledEdges.length)} relationship${bundledEdges.length === 1 ? '' : 's'}</b></summary><div><ol>${examples}</ol>${bundledEdges.length > 12 ? `<small>${number(bundledEdges.length - 12)} additional loaded relationships remain represented in this bundle.</small>` : ''}<button data-action="graphFilterEdgeBundle" data-relation="${esc(relation)}" aria-pressed="${requestedRelation === relation}">${requestedRelation === relation ? 'Filtering this relation' : `Filter to ${esc(readableRelation)}`}</button></div></details>`;
    }).join('');
    const edgeBundlePanel = `<section class="graph-edge-bundles" aria-label="Semantic relationship bundles"><header><div><span>RELATIONSHIP BUNDLES</span><strong>${number(edgeBundles.length)} semantic types · ${number(drawableEdges.length)} loaded relationships</strong></div>${requestedRelation ? `<button data-action="graphClearEdgeBundle">Clear relation filter</button>` : ''}</header><div>${edgeBundleRows || empty('No loaded relationships are available to bundle.')}</div></section>`;
    ...
    return `${state.graphError ? `<div class="graph-inline-error" role="alert">${esc(state.graphError)}</div>` : ''}${state.graphPending ? `<div class="graph-progress" role="status"><span></span>${state.graphLoadAll ? 'Loading all remaining graph pages… Select Cancel load all to stop after the current bounded page.' : 'Loading the next bounded graph page…'}</div>` : ''}<div class="graph-commandbar"><div class="graph-segmented" role="group" aria-label="Graph coverage"><button data-action="graphOverview" aria-pressed="${mode === 'full'}">Full map</button><output>${mode === 'full' ? `${number(loadedNodes)}/${number(data.total_nodes)} records` : mode === 'overview' ? 'Community index' : 'Focused analysis'}</output></div>${layoutControls}${depthControls}<div class="graph-zoom-controls" role="group" aria-label="Map zoom"><button data-action="graphZoomOut" aria-label="Zoom out">−</button><output data-graph-zoom aria-live="polite">100%</output><button data-action="graphZoomIn" aria-label="Zoom in">+</button><button data-action="graphFit">Fit</button><button data-action="graphReset">100%</button></div><button data-action="graphBack" ${state.graphBackStack.length ? '' : 'disabled'}>Back</button><button class="graph-inspector-toggle" data-action="graphToggleInspector" aria-pressed="${state.graphInspectorOpen}">${state.graphInspectorOpen ? 'Hide' : 'Show'} inspector</button><button class="graph-focus-toggle" data-action="graphFocus" aria-pressed="${state.graphFocusMode}">${state.graphFocusMode ? 'Exit focus' : 'Focus map'}</button><button class="graph-load-more" data-action="graphLoadMore" ${hasMore && !state.graphPending ? '' : 'disabled'}>${hasMore ? `Load next page (${number(loadedNodes)}/${number(data.total_nodes)} nodes · ${number(loadedEdges)}/${number(data.total_edges)} edges)` : 'All eligible pages loaded'}</button><button class="primary graph-load-all" data-action="graphLoadAll" ${hasMore && (!state.graphPending || state.graphLoadAll) ? '' : 'disabled'}>${state.graphLoadAll && state.graphPending ? 'Cancel load all' : hasMore ? 'Load all remaining pages' : 'Complete graph loaded'}</button><span class="graph-gesture-help">Drag or arrows pan · Ctrl+wheel or +/− zoom · 0 fits</span></div><div class="graph-workspace${state.graphInspectorOpen ? '' : ' inspector-collapsed'}"><div class="graph-stage"><div class="graph-canvas" data-graph-canvas data-scene-key="${esc(sceneKey)}" data-scene-width="${finiteLayoutAttribute(width, 320, 20000, 1280)}" data-scene-height="${finiteLayoutAttribute(height, 240, 20000, 780)}" tabindex="0" role="region" aria-label="Interactive ${esc(data.view)} graph. ${number(loadedNodes)} of ${number(data.total_nodes)} eligible records are loaded. Use the nonvisual map below for a list equivalent."><div class="graph-scene" data-graph-scene data-scene-width="${finiteLayoutAttribute(width, 320, 20000, 1280)}" data-scene-height="${finiteLayoutAttribute(height, 240, 20000, 780)}" data-graph-translate-x="0" data-graph-translate-y="0" data-graph-scale="1"><svg viewBox="0 0 ${width} ${height}" aria-hidden="true" focusable="false"><defs><marker id="graph-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto"><path d="M 0 0 L 10 5 L 0 10 z"></path></marker></defs>${communityHulls}${edges}</svg>${nodes}</div><article class="graph-selection-card"><span>${mode === 'full' ? 'SELECTED RECORD' : 'FOCUS NODE'}</span><strong>${esc(selected.title)}</strong><small>${esc(selected.key)}</small><div><b>${esc(selected.kind || 'node')}</b><i>${number(incomingCount)} in</i><i>${number(outgoingCount)} out</i></div></article><button class="graph-minimap" data-action="graphFit" aria-label="Fit loaded graph records"><span></span></button><span class="graph-interaction-status" data-graph-status role="status" aria-live="polite">Map fitted</span><span class="graph-truncated${hasMore ? '' : ' complete'}">${hasMore ? 'Progressive coverage' : 'Eligible set loaded'}</span></div></div><aside class="relationship-inspector" aria-label="Selected record and readable relationships"><header><div><span>SELECTED RECORD</span><strong>${esc(selected.title)}</strong><small>${esc(selected.key)}</small></div><div class="relationship-counts"><b>${number(incident.length)} loaded</b><span>${number(incomingCount)} incoming · ${number(outgoingCount)} outgoing</span></div></header><dl class="graph-record-provenance"><div><dt>Community</dt><dd>${esc(selected.community_id || 'unclassified')}</dd></div><div><dt>Source</dt><dd class="mono">${esc(selected.path || selected.source?.path || data.source || 'not declared')}</dd></div><div><dt>Hash</dt><dd class="mono">${esc(selected.source_sha256 || 'not declared')}</dd></div><div><dt>Provenance</dt><dd>${esc(provenanceSummary(selected.provenance || selected.source))}</dd></div></dl><div class="graph-inspector-actions"><button data-action="inspectGraphRecord" data-node-key="${esc(selected.key)}">Inspect source record</button><button data-action="graphOpenNeighborhood" data-node-key="${esc(selected.key)}">Open relationship neighborhood</button></div><div class="relationship-list">${relationships || empty('No loaded relationship page currently touches this record. Load more edges or open its neighborhood.')}</div></aside></div>${edgeBundlePanel}<details class="graph-accessible-map"><summary>Nonvisual map — ${number(loadedNodes)} loaded records and selected relationships</summary><div><label><span>Loaded graph records</span><select size="10" data-graph-record-list>${nonvisualOptions}</select></label><section aria-label="Selected record summary"><h3>${esc(selected.title)}</h3><p>${esc(selected.summary || 'No canonical summary was stored.')}</p><dl><div><dt>Key</dt><dd class="mono">${esc(selected.key)}</dd></div><div><dt>Kind/status</dt><dd>${esc(selected.kind)} / ${esc(selected.status)}</dd></div><div><dt>Community</dt><dd>${esc(selected.community_id || 'unclassified')}</dd></div></dl><h4>Loaded relationships</h4><ol>${incident.map(edge => `<li><b>${esc(edge.source)}</b> ${esc(String(edge.relation).replaceAll('_', ' '))} <b>${esc(edge.target)}</b>. ${esc(edge.why || '')}</li>`).join('') || '<li>No loaded relationships touch this record.</li>'}</ol></section></div></details><div class="graph-legend"><span><i class="legend-skill"></i>Skill</span><span><i class="legend-agent"></i>Agent</span><span><i class="legend-file"></i>File / module</span><span><i class="legend-contract"></i>Contract / policy</span><span>Visual nodes are viewport-culled in bounded animation frames; the list equivalent remains available.</span></div>`;
  }
  ...
  function render(context) {
    const { state } = requireContext(context); const data = state.graphData; const requested = { ...(state.graphRequest || data || {}), relation: state.graphRelation ?? state.graphRequest?.relation ?? data?.requested_relation ?? '' };
    ...
    const activeRelation = String(requested.relation || requested.requested_relation || '');
    const availableRelations = [...new Set([...(data?.relations || []), activeRelation].filter(Boolean))].sort();
    const relationOptions = availableRelations.map(value => `<option value="${esc(value)}" ${activeRelation === value ? 'selected' : ''}>${esc(String(value).replaceAll('_', ' '))}</option>`).join('');
    ...
    const metrics = `<div class="metric-grid compact">${card('SOURCE RECORDS', number(sourceNodes ?? 'unavailable'), 'canonical denominator')}${card('ELIGIBLE RECORDS', number(eligibleNodes ?? 'unavailable'), state.graphCommunity || state.graphKind || state.graphStatus ? 'after source filters' : 'current graph source')}${card('LOADED RECORDS', number(loadedNodes), `${number(data?.total_nodes || 0)} eligible`)}${card('LOADED RELATIONSHIPS', number(loadedEdges), `${number(data?.total_edges || 0)} eligible`)}</div><div class="action-grid"><button data-action="buildRepositoryGraph">Refresh repository source map</button></div>`;
