...
function upgradeLegacyAgentTopology(root) {
  ...
  const nodes = [
    ...
    ['tests', 'PREFLIGHT CONTRACT', `${draft.required_tests.length} required`, '[data-agent-list-field="required_tests"]', 'gate'],
...
function agentInspectorHtml(node, draft, graph) {
  ...
  const body = node.kind === 'identity' ? `<label><span>Project ID</span><input data-agent-root-field="project_id" value="${esc(draft.project_id)}"></label><p>Agent identity, version, and owner remain in the revision header.</p>`
    ...
    : node.kind === 'tests' ? listField('Required structural checks', 'required_tests')
...
function agentEditorHtml(draft) {
  ...
  const validation = { valid: specValidation.valid && graphValidation.valid, issues: [...specValidation.issues, ...graphValidation.issues] };
...
function updateAgentValidationBox() {
  ...
  const validation = { valid: specValidation.valid && graphValidation.valid, issues: [...specValidation.issues, ...graphValidation.issues] }; const box = document.querySelector('[data-studio-validation]');
...
function studioLifecycleModal(kind, operation, result) {
  ...
  showInformationModal(`${kind[0].toUpperCase()}${kind.slice(1)} Studio`, `${operation.toUpperCase()} RESULT · ${succeeded ? 'ACCEPTED' : 'NOT ACCEPTED'}`, result, `<p>Lifecycle dimensions remain separate. A next action appears only when this operation's exact success field passed.</p>${runOutput}${humanRecord(result)}${buttons ? `<div class="action-grid studio-actions">${buttons}</div>` : '<p class="modal-note">No next lifecycle action is eligible from this result.</p>'}`);
}
...
function resolvedStudioPreviewModal(kind, result) {
  ...
  showInformationModal(`${kind === 'agent' ? 'Execution' : 'Workflow'} preview · ${subject}`, `REQUEST-BOUND · ${safe ? 'NO EFFECTS' : 'UNVERIFIED EFFECT BOUNDARY'} · ${eligible ? 'ELIGIBLE' : 'BLOCKED'}`, result, body);
}
...
function updateAgentDraftFromControl(control) {
  ...
  if (control.matches?.('[data-agent-root-field],[data-agent-list-field],[data-agent-json-field],[data-agent-model-field],[data-agent-host-model],[data-agent-binding-field],[data-agent-grant-field]')) agentGraphDirty = true;
...
app.addEventListener('click', event => {
  ...
  if (action === 'studioEditorTab') { const tab = control.dataset.tab; document.querySelectorAll('[data-action="studioEditorTab"]').forEach(button => button.setAttribute('aria-selected', String(button === control))); document.querySelectorAll('[data-studio-panel]').forEach(panel => panel.toggleAttribute('hidden', panel.dataset.studioPanel !== tab)); if (tab === 'json' && studioEditor) { const input = document.getElementById('studio-draft-json'); if (input) input.value = JSON.stringify(studioEditor.draft, null, 2); } return; }
  ...
  if (action === 'submitStudioAgentRun') { const objective = document.getElementById('studio-agent-objective')?.value.trim() || ''; if (!studioPendingRun || !objective) { document.getElementById('studio-agent-objective')?.setCustomValidity('A specific bounded objective is required.'); document.getElementById('studio-agent-objective')?.reportValidity(); return; } const payload = structuredClone(studioPendingRun); payload.task = { objective }; if (studioSession?.kind === 'agent') studioSession.payload.task = structuredClone(payload.task); studioPendingRun = null; vscode.postMessage({ type: 'studioOperation', kind: 'agent', operation: 'start', payload }); closeModal(); return; }
  if (action === 'submitStudioWorkflowRun') {
    ...
    try {
      ...
      for (const item of studioPendingWorkflowRun.run_input_contract || []) {
      ...
      const payload = structuredClone(studioPendingWorkflowRun); payload.run_inputs = values;
      if (studioSession?.kind === 'workflow') studioSession.payload.run_inputs = structuredClone(values);
      studioPendingWorkflowRun = null; vscode.postMessage({ type: 'studioOperation', kind: 'workflow', operation: 'start', payload }); closeModal();
  ...
  if (action === 'catalogRetry') { const kind = control.dataset.kind; if (!kind) return; delete state.catalogs[kind]; requestCatalog(kind, { offset: 0 }); render(); return; }
...
window.addEventListener('message', event => {
  ...
  if (message.type === 'catalogResult') {
    ...
    if (!activeRequest || activeRequest.requestId === message.requestId) { state.catalogs[kind] = message.result; renderPreservingControl(`[data-catalog-search="${CSS.escape(kind)}"]`); }
  }
  ...
  if (message.type === 'studioDraftResult') { const disposition = studioSaveResponseDisposition(message); if (!['agent', 'workflow', 'skill'].includes(message.kind) || disposition === 'unmatched') return; const catalogKind = `${message.kind}s`; delete state.catalogs[catalogKind]; requestCatalog(catalogKind, { offset: 0 }); const outcome = message.outcome === 'recovered' ? 'recover' : 'create'; state.studioHistory.unshift(studioEditors.historyEntry(message.kind, outcome, message.result)); persistStudioMetadata(); if (disposition === 'detached') return; clearWorkingStudioDraft(message.kind); studioDraftDirty = false; closeModal(); studioLifecycleModal(message.kind, outcome, message.result); }
  if (message.type === 'studioSetupResult') { if (!pendingStudioSetup || message.requestId !== pendingStudioSetup.requestId || message.result?.schema_version !== 'px.studio-setup-result/1.0' || message.result.ready !== true) return; pendingStudioSetup = null; delete state.catalogs.agents; delete state.catalogs.workflows; requestCatalog('agents', { offset: 0 }); requestCatalog('workflows', { offset: 0 }); vscode.postMessage({ type: 'refresh' }); showInformationModal('Agent Studio and Workflow Studio are operational', 'ADMITTED · EDITABLE · BOUNDED RUNS SUCCEEDED', message.result, `<p><b>${esc(message.result.agent.identity)} @ ${esc(message.result.agent.version)}</b> is admitted and completed run <span class="mono">${esc(message.result.agent.run_id)}</span>.</p><p><b>${esc(message.result.workflow.identity)} @ ${esc(message.result.workflow.version)}</b> is admitted and completed run <span class="mono">${esc(message.result.workflow.run_id)}</span>.</p><p>Open either catalog record and choose Edit as new revision to change it without overwriting the admitted predecessor.</p>`); return; }
  if (message.type === 'studioDraftOutcomeUnverified') { const disposition = studioSaveResponseDisposition(message); if (!['agent', 'workflow', 'skill'].includes(message.kind) || disposition === 'unmatched') return; const catalogKind = `${message.kind}s`; delete state.catalogs[catalogKind]; requestCatalog(catalogKind, { offset: 0 }); if (disposition === 'detached') return; resetStudioDetachControls(); const save = document.querySelector('[data-action="submitStudioDraft"]'); if (save) { save.disabled = true; save.textContent = 'Outcome requires catalog refresh'; } const status = document.querySelector('[data-studio-validation]') || document.querySelector('.studio-editor-root'); status?.insertAdjacentHTML('beforeend', '<span class="studio-warning" role="alert">The host could not validate the durable creation receipt. The catalog is refreshing to recover authoritative state; this draft will not be blindly retried.</span>'); return; }
  ...
  if (message.type === 'studioOperationResult') {
    if (message.kind === 'skill' && ['validate', 'admit', 'promote', 'rollback'].includes(message.operation)) {
      ...
      state.studioHistory.unshift(studioEditors.historyEntry(message.kind, message.operation, message.result)); delete state.catalogs.skills; requestCatalog('skills', { offset: 0 }); persistStudioMetadata(); studioLifecycleModal(message.kind, message.operation, message.result); return;
    }
    ...
    if (message.operation === 'next-version') {
      ...
      state.studioHistory.unshift(studioEditors.historyEntry(message.kind, message.operation, message.result)); const catalogKind = `${message.kind}s`; delete state.catalogs[catalogKind]; requestCatalog(catalogKind, { offset: 0 }); persistStudioMetadata(); studioLifecycleModal(message.kind, message.operation, message.result);
    }
  }
