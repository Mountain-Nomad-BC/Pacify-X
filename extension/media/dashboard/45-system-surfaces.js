...
(() => {
  ...
  function diagnostics(context) {
    ...
    const campaign = snapshot.repair_campaign || {};
    if (campaign.valid !== true) traces.push({ id: 'repair-campaign-invalid', severity: 'high', cause: 'The active repair-order denominator is missing or invalid.', owner: '.engineering-bootstrap/processing-order/repair-campaign.json', evidence: String(campaign.campaign_id || 'campaign unavailable'), surface: 'diagnostics', repairLabel: 'Inspect repair campaign', verify: 'A valid active campaign must enumerate each unresolved functional track exactly once.' });
    else if (Number(campaign.unresolved_count || 0) > 0) traces.push({ id: 'active-repair-campaign', severity: 'high', cause: `${number(campaign.unresolved_count)} functional repair tracks remain in the active ${campaign.phase || 'repair'} campaign.`, owner: '.engineering-bootstrap/processing-order/repair-campaign.json', evidence: (campaign.unresolved || []).join(', '), surface: 'diagnostics', repairLabel: 'Inspect active tracks', verify: 'The active campaign closes only after each functional track has direct operational evidence.' });
