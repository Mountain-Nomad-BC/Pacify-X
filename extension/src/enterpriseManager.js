...
function initializeEnterprise(projectRoot, catalog = {}, options = {}) {
  const loaded = loadEnterpriseState(projectRoot, catalog);
  if (options.persist !== false && (!fs.existsSync(loaded.paths.state) || loaded.migrated)) atomicWrite(loaded.paths.state, loaded.state);
  return loaded;
}
