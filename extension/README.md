...
npm test
npm run package
Get-FileHash .\dist\pacify-x-vscode-0.6.29.vsix -Algorithm SHA256
.\Install-PacifyX.ps1
```

Or choose **Extensions: Install from VSIX...** and select `dist/pacify-x-vscode-0.6.29.vsix`. Compare its SHA-256 with `SHA256SUMS.txt` first. Reload the VS Code window after installation.
