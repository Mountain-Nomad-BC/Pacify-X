...
test('owned installed Studio lifecycle accepts only operation-exact typed receipts', () => {
  ...
  assert.equal(validStudioLifecycleResult('agent', 'preview', { schema_version: 'px.agent-execution-preview/1.1', effects_executed: false, eligible: true }), true);
  assert.equal(validStudioLifecycleResult('agent', 'preview', { schema_version: 'px.agent-execution-preview/1.0', effects_executed: false, eligible: true }), false);
  ...
  assert.equal(validStudioLifecycleResult('skill', 'promote', { schema_version: 'px.skill-promotion-receipt/1.4', state: 'promoted', promotion_receipt_relative: '.engineering-bootstrap/studios/skills/demo/promotion-receipt.json' }), true);
  assert.equal(validStudioLifecycleResult('skill', 'promote', { schema_version: 'px.skill-promotion-receipt/1.3', state: 'promoted', promotion_receipt_relative: '.engineering-bootstrap/studios/skills/demo/promotion-receipt.json' }), false);
  ...
  assert.equal(validStudioLifecycleResult('skill', 'promote', { schema_version: 'px.skill-promotion-receipt/1.4', state: 'promoted' }), false);
  ...
  const blockedCandidate = { identity: 'agent:blocked', version: '1.0.0', memory_binding_id: 'memory:unresolved' };
  const blockedPreview = { schema_version: 'px.agent-execution-preview/1.1', agent_id: 'agent:blocked', version: '1.0.0', effects_executed: false, eligible: false, status: 'blocked', blockers: ['memory_bindings_not_runtime_resolved'], memory_binding_ids: ['memory:unresolved'] };
  assert.equal(validStudioBlockedPreviewResult(blockedPreview, blockedCandidate), true);
  assert.equal(validStudioBlockedPreviewResult({ ...blockedPreview, schema_version: 'px.agent-execution-preview/1.0' }, blockedCandidate), false);
  const source = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'run-operational-ui-walk.js'), 'utf8');
