...
function createOwnedContentEvaluationBoundary(defaultTimeoutMs = 10_000) {
  ...
  return {
    run: async (operation, timeoutMs = ceilingMs) => {
      ...
      try {
        return await boundedOwnedUiAction(operation, budget, 'owned-webview-content-evaluate');
      } catch (error) {
        const message = String(error?.message || error);
        if (message === `owned-webview-content-evaluate-timeout:${budget}`) timeoutBlocker = message;
        throw error;
      }
    },
    ...
    evaluateContent: async (operation, argument, options = {}) => {
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        try {
          ...
          return await contentEvaluationBoundary.run(
            () => content.evaluate(operation, argument),
            Math.max(1, Math.min(Number(options.timeout) || 10_000, 10_000))
          );
        } catch (error) {
          if (attempt > 0 || !reacquirableOwnedFrameError(error)) throw error;
          ...
          if (handle) {
            await boundedOwnedUiAction(
              () => handle.dispose(),
              1_000,
              'owned-webview-content-handle-dispose'
            ).catch(() => {});
          }
        }
      }
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (decision === 'dispatch') {
      if (options.rejectBeforeDispatch === true) {
        ...
        if (rejectionObservation.rejected !== true || !retained || !restored) throw new Error(`workbench-command-owned-rejection-unobserved:${title}:${JSON.stringify({ rejected: rejectionObservation.rejected, reason: rejectionObservation.reason, retained, restored })}`);
...
async function bindCurrentWorkbenchCommandRejection(workbench) {
  return workbench.evaluate(() => {
    ...
    const prior = globalThis[stateKey];
    if (prior?.handler) globalThis.removeEventListener('keydown', prior.handler, true);
    const token = globalThis.crypto?.randomUUID?.() || `owned-command-rejection-${Date.now()}-${Math.random()}`;
    const state = { token, rejected: false, reason: 'enter-not-observed', handler: null };
    state.handler = event => {
      if (event.key !== 'Enter') return;
      event.preventDefault();
      event.stopPropagation();
      event.stopImmediatePropagation();
      const currentWidgets = [...document.querySelectorAll('.quick-input-widget')].filter(visible);
      const currentInput = currentWidgets.length === 1 ? currentWidgets[0].querySelector('input') : null;
      const exactTarget = currentInput === document.activeElement && currentWidgets[0]?.contains(event.target) === true;
      state.rejected = exactTarget;
      state.reason = exactTarget ? 'exact-current-focused-input-enter-rejected' : 'enter-rejected-after-input-identity-changed';
    };
    globalThis[stateKey] = state;
    globalThis.addEventListener('keydown', state.handler, { capture: true, once: true });
    return { bound: true, reason: 'exact-current-focused-input-window-capture', token };
  });
}

async function observeCurrentWorkbenchCommandRejection(workbench, token) {
  return workbench.evaluate(expectedToken => {
    const stateKey = '__PX_OWNED_WORKBENCH_COMMAND_REJECTION__';
    const state = globalThis[stateKey];
    if (!state || state.token !== expectedToken) return { rejected: false, reason: 'rejection-observation-token-missing' };
...
async function waitForBuilderJsonControls(frameHost, kind, timeoutMs = 5_000) {
  ...
  let last = null;
  do {
    last = await frameHost.evaluate((frame, builderKind) => {
      const document = frame.contentDocument;
      const modal = document?.querySelector('.studio-modal');
      const tab = modal?.querySelector('[data-action="studioEditorTab"][data-tab="json"]');
      if (tab && !tab.disabled && tab.getAttribute('aria-selected') !== 'true') tab.click();
      const input = modal?.querySelector('#studio-draft-json');
      const apply = modal?.querySelector('[data-action="studioApplyJson"]');
      return {
        builder_kind: builderKind,
        modal_present: Boolean(modal),
        title: modal?.querySelector('h2')?.textContent?.trim() || '',
        json_tab_present: Boolean(tab),
        json_tab_selected: tab?.getAttribute('aria-selected') === 'true',
        input_present: Boolean(input),
        apply_present: Boolean(apply),
        ready: Boolean(input && apply)
      };
    }, kind);
    if (last.ready) return last;
    await wait(50);
  } while (Date.now() < deadline);
  throw new Error(`canonical-json-controls-readiness-timeout:${kind}:${JSON.stringify(last)}`);
}

async function exerciseBuilderDurability(frameHost, kind, studioSurface) {
  await waitForBuilderJsonControls(frameHost, kind);
  const before = await builderState(frameHost, kind);
...
async function installedGraphExchangeOffset(frameHost) {
  return frameHost.evaluate(frame => ({
...
async function waitForInstalledGraphExchange(frameHost, after, timeoutMs = 30_000) {
  const deadline = Date.now() + timeoutMs;
  let diagnostic = { requests: [], responses: [] };
  do {
    const exchange = await frameHost.evaluate((frame, offsets) => {
      const requests = (frame.contentWindow?.__PX_INSTALLED_REQUESTS__ || []).slice(offsets.requests)
        .filter(value => value?.type === 'graphQuery' && typeof value.requestId === 'string' && value.requestId);
      const responses = (frame.contentWindow?.__PX_INSTALLED_RESPONSES__ || []).slice(offsets.responses);
      for (const request of requests) {
        const response = responses.find(value => value?.requestId === request.requestId
          && (value?.type === 'graphResult' || (value?.type === 'operationError' && value?.operation === 'graphQuery')));
        if (response) return { request, response, diagnostic: null };
      }
      return {
        request: null,
        response: null,
        diagnostic: {
          requests: requests.slice(-6).map(value => ({ type: value.type, requestId: value.requestId, offset: value.offset, edgeOffset: value.edgeOffset })),
          responses: responses.slice(-8).map(value => ({ type: value?.type, requestId: value?.requestId || null, operation: value?.operation || null, error: String(value?.error || '').slice(0, 300) }))
        }
      };
    }, after);
    if (exchange.response?.type === 'operationError') {
      throw new Error(`installed-graph-query-failed:${exchange.request.requestId}:${String(exchange.response.error || 'unknown').slice(0, 600)}`);
    }
    if (exchange.response?.type === 'graphResult') return exchange.response;
...
async function runInstalledLateCardRepairObservationProfile(frameHost, matrix, timeoutMs = 30_000) {
  ...
  try {
    ...
    const requestGraphPage = async () => {
      const before = await installedGraphExchangeOffset(frameHost);
      ...
      return waitForInstalledGraphExchange(frameHost, before, timeoutMs);
    };
    ...
    await waitForInstalledGraphExchange(frameHost, restoreBefore, timeoutMs);
  } catch (error) {
...
async function runInstalledStudioLifecycleProfile(frameHost, candidateProfile, matrix, options = {}) {
  const onProgress = typeof options.onProgress === 'function' ? options.onProgress : () => {};
  const runLifecycleStep = async (candidate, operation, callback) => {
    const started = Date.now();
    onProgress({ scope: 'operation', kind: candidate.kind, identity: candidate.identity, operation, state: 'started' });
    try {
      const result = await callback();
      onProgress({ scope: 'operation', kind: candidate.kind, identity: candidate.identity, operation, state: 'returned', duration_ms: Date.now() - started });
      return result;
    } catch (error) {
      onProgress({ scope: 'operation', kind: candidate.kind, identity: candidate.identity, operation, state: 'threw', duration_ms: Date.now() - started, error: String(error?.message || error).slice(0, 1200) });
      throw error;
    }
  };
  const observations = [];
  for (const candidate of candidateProfile.observations || []) {
    const candidateStarted = Date.now();
    onProgress({ scope: 'candidate', kind: candidate.kind, identity: candidate.identity, state: 'started' });
    ...
    try {
      ...
      for (const operation of operations) {
        await runLifecycleStep(candidate, operation, async () => {
          const before = await frameHost.evaluate(frame => frame.contentWindow?.__PX_INSTALLED_RESPONSES__?.length || 0);
          await frameHost.evaluate((frame, item) => {
            const document = frame.contentDocument;
      ...
      if (candidate.kind === 'skill') {
        const failureStates = await runLifecycleStep(candidate, 'failure-states', () => exerciseStudioLifecycleFailureStates(frameHost, candidate));
    ...
    observations.push(observation);
    onProgress({ scope: 'candidate', kind: candidate.kind, identity: candidate.identity, state: observation.errors.length ? 'threw' : 'returned', duration_ms: Date.now() - candidateStarted, error_count: observation.errors.length, errors: observation.errors });
  }
...
async function main() {
  ...
  try {
    ...
    let studioLifecycleProfile = studioChainAdmitted
      ? studioCandidateHealthy
        ? await timedProfile('studio-lifecycle', () => runInstalledStudioLifecycleProfile(dashboard, studioCandidateSaveProfile, proofMatrix, {
          onProgress: event => appendProfileProgress({ profile: 'studio-lifecycle-step', ...event })
        }), { timeoutMs: 180_000 })
    ...
    if (studioChainAdmitted && studioCandidateHealthy && returnedProfileErrors(studioLifecycleProfile).length === 0 && returnedProfileErrors(studioRevisionEditProfile).length === 0) {
      ...
      if (baseSkill && validStudioRevisionEditObservation(revisedSkill)) {
        ...
        const rollbackProfile = await timedProfile('studio-skill-revision-rollback', () => runInstalledStudioLifecycleProfile(dashboard, { observations: [rollbackCandidate] }, proofMatrix, {
          onProgress: event => appendProfileProgress({ profile: 'studio-skill-revision-rollback-step', ...event })
        }), { timeoutMs: 180_000 });
    ...
    for (const kind of focusedProfileOnly && !builderOnly ? [] : ['agent', 'workflow']) {
    ...
    if (focusedProfileOnly && !builderOnly) {
    ...
    workbenchCommandProfile = ownedReversibleConfigurationAuthority && (!focusedProfileOnly || workbenchCommandOnly) && !hostSourceMismatch
