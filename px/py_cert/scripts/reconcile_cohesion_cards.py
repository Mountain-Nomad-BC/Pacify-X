...
from runtime.certification_contract import RELEASE_STAGES, STEP_ORDER  # noqa: E402
...
PRIOR_AUTOMATION_STEPS = STEP_ORDER[: STEP_ORDER.index("card_reconcile")]
...
def _validate_close_control_plane(
    ...
    *,
    ...
    repair_path = root / ".engineering-bootstrap/processing-order/repair-campaign.json"
    ...
    if (
        repair.get("campaign_id") != identity.get("repair_campaign_id")
        or repair.get("phase") != "installed_operational"
        or repair.get("intake_open") is not False
        or repair.get("unresolved") != []
    ):
        raise ReconciliationError(
            "current repair campaign is not the installed-operational campaign bound to the release identity"
        )
    ...
    if (
        ...
        or identity.get("repair_campaign_id") != repair.get("campaign_id")
...
def _project_management_state(
    ...
    *,
    ...
    checkpoint["failures"] = (
        [
            "Historical cohesion repair evidence is retained as immutable baseline proof; the current candidate installed-operational proof closes the active source/proof denominator."
        ]
        if target == "closed"
        else [
