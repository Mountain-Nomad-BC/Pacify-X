...
def _validate_git_convergence(config: Config) -> None:
    """Require the frozen source HEAD to equal its configured upstream.

    Candidate control files may exist as release-identity mutable-control state,
    but no blocking source drift or staged change is allowed at campaign start.
    """

    _validate_empty_git_index(config)
    head = _git_output(config.root, "rev-parse", "HEAD")
    upstream = _git_output(config.root, "rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}")
    upstream_head = _git_output(config.root, "rev-parse", "@{u}")
    if not re.fullmatch(r"[a-f0-9]{40}", head):
        raise AutomationBlocked("Git HEAD is not a canonical 40-hex commit")
    if not upstream or not re.fullmatch(r"[a-f0-9]{40}", upstream_head):
        raise AutomationBlocked("Git upstream identity is unavailable")
    if head != upstream_head:
        raise AutomationBlocked(
            f"local HEAD differs from upstream {upstream}: {head} != {upstream_head}"
        )
    ...
    if blocking:
        raise AutomationBlocked(
            "blocking source drift exists after Git convergence: "
            + ", ".join(blocking[:16])
        )
...
class Config:
    ...
    def load(cls, path: Path) -> "Config":
        ...
        for step in STEP_ORDER:
            ...
            expected_count = CONTRACT_BY_STEP[step].owner_command_count
            if len(parsed) != expected_count:
                raise AutomationBlocked(
                    f"owner {step} must have exactly {expected_count} command"
        ...
        if identity_path_manifest not in fresh_paths:
            raise AutomationBlocked(
                "fresh_paths must include identity_manifest.path so stale manifests cannot be reused"
            )
...
def readiness(config: Config) -> dict[str, Any]:
    ...
    if initial:
        ...
        try:
            _validate_git_convergence(config)
        except (OSError, ValueError, json.JSONDecodeError, RuntimeError) as exc:
            errors.append(f"Git convergence gate failed: {exc}")
        try:
...
class ReadOnlyOwners:
    """Postcondition verifier that never constructs the resource manager."""
    ...
    def run(
        ...
        raise AutomationBlocked("read-only verifier cannot execute owners")

    def verify(self, step: str) -> None:
        # SubprocessOwners.verify uses only self.config and performs no owner launch.
        SubprocessOwners.verify(self, step)
    ...
    *,
    ...
    while True:
        ...
        try:
            ...
            if not passed:
                failure = f"owner returned a non-passing result: {step}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.check:
            ...
            if report["valid"]:
                report["next_step"] = next_pending_step(config, ReadOnlyOwners(config))
