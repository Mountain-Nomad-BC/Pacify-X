...
def inspect(
    config: dict[str, Any], workload: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    if normalized["kv_heads"] is None and normalized["query_heads"] is not None:
        ...
        warnings.append(
            "KV heads were inferred as equal to query heads; verify the runtime default."
        )
    ...
    if query_heads and kv_heads:
        ...
        normalized["attention_variant"] = (
            "MHA" if query_heads == kv_heads else ("MQA" if kv_heads == 1 else "GQA")
        )
    model_type = str(config.get("model_type", "unknown"))
    normalized["model_type"] = model_type
    normalized["position_method"] = str(
        config.get("position_embedding_type")
        or (
            "RoPE-family"
            if "rope_theta" in config
            or model_type in {"llama", "mistral", "mixtral", "qwen2"}
            else "unknown"
        )
    )
    normalized["normalization"] = str(
        config.get("norm_type")
        or ("RMSNorm-like" if "rms_norm_eps" in config else "unknown")
    )
    normalized["activation"] = str(
        config.get("hidden_act") or config.get("activation_function") or "unknown"
    )
    ...
    if all(
        value is not None
        for value in (layers, hidden, query_heads, kv_heads, intermediate, vocab)
    ):
        ...
        embeddings = (
            (1 if bool(config.get("tie_word_embeddings", False)) else 2)
            * vocab
            * hidden
        )
        ...
        estimates.update(
            {
                ...
                "ffn_parameters_per_layer": ffn,
                "embedding_and_output_parameters": embeddings,
                "estimated_total_parameters": total,
                "gated_ffn_assumed": gated,
            }
        )
    ...
    element_bytes = _integer(
        workload.get("kv_bytes_per_element", 2), "kv_bytes_per_element", 16
    )
    if all(
        value is not None
        for value in (
        ...
        estimates["kv_cache_bytes"] = kv_cache_bytes(
            layers,
            kv_heads,
            normalized["head_dim"],
            tokens,
        ...
        estimates["kv_cache_bytes_per_token_per_sequence"] = kv_cache_bytes(
            layers, kv_heads, normalized["head_dim"], 1, 1, element_bytes, 1
        )
    ...
    if experts and active_experts:
        ...
        warnings.append(
            "MoE active fraction excludes shared experts, routing imbalance, capacity factors, and communication overhead."
        )
