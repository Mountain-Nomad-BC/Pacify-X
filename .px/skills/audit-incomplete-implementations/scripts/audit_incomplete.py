...
def _excluded_directory(name: str) -> bool:
    ...
    return folded in {item.casefold() for item in EXCLUDES} or folded.startswith(".venv")
            ...
            if (
                ...
                findings.append(
                    _finding(
                        ...
                        # CPython adds fields to AST nodes between releases
                        # (for example ``type_params`` in 3.12). An AST dump
                        # therefore made identical source bytes produce a new
                        # finding ID on another supported Python version.
                        ast.get_source_segment(text, node)
                        or f"{type(node).__name__}:{node.name}:ellipsis",
                    )
                )
    return findings
...
def audit(
    ...
    for current, dirs, files in os.walk(root, topdown=True, followlinks=False):
        ...
        if _excluded_relative(relative_current):
            dirs[:] = []
            continue
        dirs[:] = sorted(d for d in dirs if not _excluded_directory(d))
