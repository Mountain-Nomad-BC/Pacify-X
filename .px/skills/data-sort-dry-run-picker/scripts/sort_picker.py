        ...
        while not finished:
            ...
            while True:
                ...
                if not started:
                    ...
                    if buffer[position] != "[":
                        raise ValueError(
                            "JSON input must be a top-level array; use JSONL for streams"
                        )
                    started = True
                    position += 1
                    continue
...
def sample_records(
    ...
    for ordinal, record in enumerate(iter_records(path, input_format)):
        ...
        if current_type is not key_type:
            raise ValueError(
                f"mixed key types are not a total order: {key_type.__name__} and {current_type.__name__}"
            )
...
def merge_sort(items: list[Item]) -> list[Item]:
    ...
    while width < len(source):
        for start in range(0, len(source), width * 2):
            left, mid, right = (
                start,
                min(start + width, len(source)),
                min(start + width * 2, len(source)),
            )
...
def quick_3way(items: list[Item]) -> list[Item]:
    ...
    while stack:
        ...
        while index <= right:
            ...
            if key < pivot:
                values[left], values[index] = values[index], values[left]
                left += 1
                index += 1
            elif key > pivot:
...
def counting_sort(items: list[Item]) -> list[Item]:
    ...
    for item in items:
        buckets[item[0] - low].append(item)
...
def radix_sort(items: list[Item]) -> list[Item]:
    ...
    while maximum // place:
        ...
        for pair in shifted:
            buckets[(pair[0] // place) % 256].append(pair)
...
def bucket_sort(items: list[Item]) -> list[Item]:
    low, high = (
        ...
        max(float(item[0]) for item in items),
    )
    if low == high:
        return list(items)
    ...
    return [
        ...
        for item in sorted(bucket, key=lambda row: (row[0], row[1]))
    ]
...
def compatible_algorithms(items: list[Item]) -> tuple[list[str], dict[str, str]]:
    ...
    if numeric:
        accepted.append("bucket-sort")
        reasons["bucket-sort"] = "finite homogeneous numeric keys"
    if integer:
        span = max(keys) - min(keys) + 1
        if span <= min(2_000_000, max(4096, len(keys) * 8)):
            accepted.append("counting-sort")
            reasons["counting-sort"] = f"bounded integer range ({span})"
        accepted.append("radix-sort")
        reasons["radix-sort"] = "homogeneous signed integer keys"
    return accepted, reasons


def validate_output(
    output: list[Item], reference: list[Item]
...
def benchmark(
    ...
    return {
        ...
        "error": error,
        "runs_ns": timings,
        "median_ns": int(statistics.median(timings)) if timings else None,
        "p95_ns": p95,
    }
...
def build_receipt(
    ...
    *,
    ...
    count, sample = sample_records(
        path, input_format, key_path, coerce, sample_limit, seed
    )
    sample_hash = hashlib.sha256(
        json.dumps(
            [(item[0], item[1]) for item in sample],
            ensure_ascii=False,
            sort_keys=True,
            default=str,
        ).encode()
    ...
    pilot_results = [
        benchmark(name, ALGORITHMS[name], pilot, pilot_reference, 1) for name in names
    ]
    qualified = sorted(
        (row for row in pilot_results if row["correct"]),
        key=lambda row: (row["median_ns"], row["algorithm"]),
    )[:3]
    final_results = [
        benchmark(
            row["algorithm"], ALGORITHMS[row["algorithm"]], sample, reference, repeats
        )
        for row in qualified
    ]
    winners = sorted(
        (row for row in final_results if row["correct"] and row["stable"]),
        key=lambda row: (row["median_ns"], row["algorithm"]),
    )
    return {
        "schema_version": "1.0",
        "operation": "data-sort-dry-run-picker",
        "dry_run": True,
        "input": {
        ...
        "sample": {
            "records": len(sample),
            "coverage_ratio": len(sample) / count,
            "strategy": "full" if count <= sample_limit else "deterministic-reservoir",
            "seed": seed,
            "sha256": sample_hash,
        },
        "candidate_compatibility": [
            {"algorithm": name, "reason": reasons[name]} for name in names
        ],
        "pilot": {
            "records": pilot_size,
            "results": pilot_results,
            "advanced": [row["algorithm"] for row in qualified],
        },
        "benchmark": {"repeats": repeats, "results": final_results},
        "selected": winners[0]["algorithm"] if winners else None,
        "decision": "selected-fastest-correct-stable-median"
        if winners
        else "no-candidate-passed",
        "full_run_requirement": "external-merge-plan"
        if count > sample_limit
        else "selected-algorithm-may-be-applied-with-separate-write-approval",
        "residual_risks": (
            ["sample may not represent full-data distribution"]
            if count > sample_limit
            else []
        )
        + ["benchmark is runtime/hardware/implementation specific"],
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--format", choices=["auto", "json", "jsonl", "csv", "lines"], default="auto"
    )
    parser.add_argument("--key")
    parser.add_argument(
        "--coerce", choices=["auto", "none", "integer", "number"], default="auto"
    )
    ...
    if args.sample_records < 100 or args.sample_records > 1_000_000:
        raise SystemExit("sample-records must be between 100 and 1000000")
    if args.repeats < 1 or args.repeats > 25:
        raise SystemExit("repeats must be between 1 and 25")
    if not args.input.is_file():
        raise SystemExit("input must be a readable regular file")
    try:
        receipt = build_receipt(
            ...
            key_path=args.key,
            coerce=args.coerce,
            sample_limit=args.sample_records,
