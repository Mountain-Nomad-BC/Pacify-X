...
function capture(command, args, options = {}) {
  return new Promise(resolve => {
    ...
    let stderr = '';
    let timedOut = false;
    const timer = setTimeout(() => { timedOut = true; child.kill(); }, options.timeoutMs || 5000);
...
class CodexRunManager {
  ...
  cancel() { if (!this.active) return false; this.active.cancelled = true; this.active.child.kill(); return true; }
  run({ envelope, cwd, sandbox = 'read-only', onEvent = () => {}, timeoutMs = 30 * 60 * 1000 }) {
    if (this.active) return Promise.reject(new Error('A bridge-owned Codex run is already active.'));
    if (!/ChatGPT \(verified by Codex CLI\)/i.test(String(envelope?.target?.authentication_identity || ''))) {
      return Promise.reject(new Error('Pacify-X requires a verified ChatGPT Codex login and will not fall back to billable API credentials.'));
    }
    return new Promise((resolve, reject) => {
      const child = cp.spawn('codex', ['exec', '--json', '--ephemeral', '--sandbox', sandbox, '-C', cwd, '-'], {
        cwd, shell: false, windowsHide: true, env: nonBillableEnvironment(), stdio: ['pipe', 'pipe', 'pipe']
      });
      const record = { child, correlationId: envelope.correlation_id, cancelled: false };
      this.active = record;
      const timer = setTimeout(() => { record.cancelled = true; record.timedOut = true; child.kill(); }, timeoutMs);
      const lines = readline.createInterface({ input: child.stdout });
      lines.on('line', line => { try { onEvent(JSON.parse(line)); } catch { onEvent({ type: 'pacify.parse_error', text: line.slice(0, 4000) }); } });
      child.stderr.setEncoding('utf8'); child.stderr.on('data', data => onEvent({ type: 'pacify.stderr', text: String(data).slice(0, 8000) }));
      child.on('error', error => { clearTimeout(timer); this.active = null; reject(error); });
      child.on('close', code => { clearTimeout(timer); const cancelled = record.cancelled; this.active = null; resolve({ code: code ?? -1, cancelled, timedOut: Boolean(record.timedOut), correlationId: record.correlationId }); });
      child.stdin.end(codexPrompt(envelope));
    });
  }
}

module.exports = { capture, gitSnapshot, detectGitOperation, gitConflictDecision, providerStatus, buildContextEnvelope, codexPrompt, CodexRunManager, GIT_MUTATION_PROHIBITIONS, BILLABLE_PROVIDER_ENVIRONMENT_KEYS, nonBillableEnvironment, isWithin, relativeReference };
