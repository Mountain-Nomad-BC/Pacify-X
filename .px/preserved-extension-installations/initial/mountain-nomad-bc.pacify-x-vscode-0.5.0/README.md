...
- Complete, searchable, sortable, paged core catalogs for 176 skills, 11 tools, 270 agents, 72 workflow authorities/bindings, and the full graph registry in the current Pacify-X source.
...
Or choose **Extensions: Install from VSIX...** and select `dist/pacify-x-vscode-0.5.0.vsix`.

Open a Pacify-X workspace, or set `pacifyX.engineRoot` and `pacifyX.workspaceRoot` to bounded absolute paths. Run **Pacify-X: Open Control Plane**. The engine root must contain `runtime/dashboard_api.py`; this repository integration adds that canonical adapter to the supplied Pacify-X tree.
