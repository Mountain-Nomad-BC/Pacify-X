...
def main():
    ...
    d = {
        ...
        "subject": [{"name": x, "digest": {"sha256": h(x)}} for x in a.subject],
        "predicate": {
            "buildDefinition": {
                "buildType": "local-reference-build",
                "externalParameters": {"commands": a.command},
                "resolvedDependencies": [
                    {"uri": x, "digest": {"sha256": h(x)}} for x in a.material
                ],
            },
            "runDetails": {
                "builder": {"id": "local-python-reference"},
                "metadata": {
                    "invocationId": hashlib.sha256(
                        ("\n".join(a.command)).encode()
                    ).hexdigest()[:24],
