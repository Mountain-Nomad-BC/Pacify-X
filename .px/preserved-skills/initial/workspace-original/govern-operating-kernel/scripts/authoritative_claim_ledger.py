...
def load(p):
    return json.loads(Path(p).read_text()) if Path(p).exists() else {"claims": []}
...
def main():
    ...
    a.add_argument("--type", default="inference")
    ...
    v.add_argument(
        "--status",
        choices=["supported", "verified", "conflicted", "rejected", "stale"],
        required=True,
    )
    sub.add_parser("list")
    args = ap.parse_args()
    ...
    if args.cmd == "add":
        c = {
            ...
            + hashlib.sha256(
                json.dumps(
                    [args.statement, args.type, args.evidence], sort_keys=True
                ).encode()
            ).hexdigest()[:12],
            "statement": args.statement,
            "claim_type": args.type,
            "status": "unverified",
            "evidence": [{"locator": x} for x in args.evidence],
            "assumptions": [],
            "conflicts": [],
            "confidence": max(0, min(1, args.confidence)),
            "verified_at": None,
            "stale_after": None,
        }
        d["claims"].append(c)
