...
def quarantine_paths(
    ...
    if (
        not _inside(destination, workspace)
        or _inside(destination, project)
        or destination.exists()
    ):
        raise ValueError(
            "destination must be new, external to the project, and inside the workspace"
        )
    if (
        not _inside(manifest, workspace)
        or _inside(manifest, project)
        or manifest.exists()
    ):
        raise ValueError(
            "manifest must be new, external to the project, and inside the workspace"
        )
    ...
    for relative in paths:
        ...
        if (
            not _inside(source, project)
            or source == project
            or not source.exists()
            or source in seen
        ):
            raise ValueError(
                f"invalid, missing, broad, or duplicate source: {relative}"
            )
        seen.add(source)
    ...
    for _, relative, before in sources:
        if len(before) == 1 and relative.suffix:
            ...
            expected.update(
                {(relative / key).as_posix(): digest for key, digest in before.items()}
            )
    transaction = quarantine_candidates(
        project,
        [source for source, _, _ in sources],
        destination,
    ...
    if transaction["decision"] != "quarantined":
        raise ValueError(
            "quarantine batch rejected: "
            + ", ".join(map(str, transaction.get("reasons", ())))
        )
    ...
    payload = {
        "schema_version": "1.0",
        "operation": "recoverable_external_quarantine",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "reason": reason,
        "project": project.relative_to(workspace).as_posix(),
        "destination": destination.relative_to(workspace).as_posix(),
        "path_count": len(records),
        "file_count": sum(int(item["file_count"]) for item in records),
