...
def build_report(
    parts_root: Path, *, excluded_ids: set[str] | None = None
) -> dict[str, object]:
    ...
    for directory in sorted(
        (path for path in parts_root.iterdir() if path.is_dir()),
        ...
        discovered = sum(
            int(root.get("files_discovered", 0)) for root in summary.get("roots", ())
        )
        records = int(summary.get("record_count", 0))
        errors = int(summary.get("error_count", 0))
        reconciled = (
            all(root.get("reconciled") is True for root in summary.get("roots", ()))
            and discovered == records + errors
        )
        part = {
            "id": directory.name,
            "records": records,
    ...
    return {
        ...
        "reconciled": total_discovered == total_records + total_errors
        and all(part["reconciled"] for part in parts),
        "campaign_sha256": digest.hexdigest(),
        "excluded_partition_ids": sorted(excluded_ids),
        "partitions": parts,
    }
