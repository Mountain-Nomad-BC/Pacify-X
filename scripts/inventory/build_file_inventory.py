...
def _structure(text: str) -> dict[str, object]:
    ...
    headings = [
        line.strip()[:500] for line in lines if re.match(r"^#{1,6}\s+\S", line.strip())
    ]
    fences = sorted(
        set(
            match.group(1).casefold() or "plain"
            for match in re.finditer(r"^```\s*([^\s`]*)", clean, re.MULTILINE)
        )
    )
    links = sorted(
        set(
    ...
    tool_mentions = sorted(
        name
        for name in TOOL_NAMES
        if re.search(rf"(?i)(?<![A-Za-z0-9]){re.escape(name)}(?![A-Za-z0-9])", clean)
    )
    commands = []
    for line in lines:
        stripped = line.strip()
        if stripped.startswith(
            ("python ", "pytest ", "git ", "rg ", "docker ", "npm ", "pnpm ", "codex ")
        ):
            commands.append(stripped[:500])
    declarations: dict[str, list[str]] = {key: [] for key in DECLARATION_KEYS}
    for line in lines:
        match = re.match(
            r"^\s*[-*]?\s*(inputs?|outputs?|effects?|dependencies|tests?|evidence)\s*:\s*(.+)$",
            line,
            re.IGNORECASE,
        )
        if match:
    ...
    return {
        ...
        "links": links,
        "declarations": {
            key: sorted(set(values)) for key, values in declarations.items()
        },
...
def main() -> int:
    ...
    parser.add_argument(
        "--root", action="append", required=True, help="label=path; repeatable"
    )
    ...
    parser.add_argument(
        "--root-files-only",
        action="store_true",
        help="inventory files directly below each root and skip subdirectories",
    )
    ...
    for label, root in roots:
        for directory, directories, filenames in os.walk(root):
            if args.root_files_only:
                directories[:] = []
            directories[:] = sorted(
                name for name in directories if name.casefold() not in excludes
            )
            base = Path(directory)
            for filename in sorted(filenames):
                ...
                try:
                    ...
                    domain, confidence = _domain(
                        relative + " " + " ".join(structure.get("headings", []))
                    )
                    records.append(
                        {
                            "id": hashlib.sha256(
                                f"{label}:{relative}".encode()
                            ).hexdigest()[:20],
                            "source_tree": label,
                    ...
                    errors.append(
                        {
                            "source_tree": label,
                            "path": sanitize(str(path.relative_to(root))),
                            "error": sanitize(f"{type(error).__name__}: {error}"),
                        }
                    )
    ...
    with error_path.open("w", encoding="utf-8", newline="\n") as handle:
        for error in sorted(
            errors, key=lambda item: (item["source_tree"], item["path"])
        ):
    ...
    summary = {
        ...
        "roots": [
            {
                "label": label,
                "files_discovered": discovered[label],
                "records": sum(item["source_tree"] == label for item in records),
                "reconciled": discovered[label]
                == sum(item["source_tree"] == label for item in records)
                + sum(item["source_tree"] == label for item in errors),
            }
            for label, _ in roots
        ],
        "content_kinds": dict(
            sorted(Counter(item["content_kind"] for item in records).items())
        ),
        "domains": dict(
            sorted(Counter(item["probable_domain"] for item in records).items())
        ),
        "extensions": dict(
            Counter(item["extension"] or "[none]" for item in records).most_common()
        ),
    }
    ...
    markdown.write_text(
        ...
        + "".join(
            f"- {item['label']}: {item['records']} records; reconciled={str(item['reconciled']).lower()}\n"
    ...
    return (
        0 if not errors and all(item["reconciled"] for item in summary["roots"]) else 1
    )
