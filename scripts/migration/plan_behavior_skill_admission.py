...
def _catalog_match(
    ...
    for item in catalog:
        ...
        if normalized == item_id:
            return item_id, 1.0
        score = _similarity(
            candidate_tokens, _tokens(item_id, " ".join(map(str, item.get("tags", ()))))
        )
...
def build_plan(index_dir: Path, catalog_path: Path) -> dict[str, object]:
    ...
    for raw in skill_payload.get("skills", ()):  # type: ignore[union-attr]
        ...
        seen_hashes[str(item.get("sha256", ""))].append(
            f"{source_alias}:{item.get('relative_path')}"
        )

    source_behavior: dict[str, dict[str, object]] = {}
    for record in records:
        alias = str(record.get("source_alias", ""))
        aggregate = source_behavior.setdefault(
            alias,
            {
                "files": 0,
                "text_read": 0,
        ...
        aggregate["text_read"] = int(aggregate["text_read"]) + int(
            bool(record.get("text_read"))
        )
        aggregate["tests"] = int(aggregate["tests"]) + len(record.get("tests", ()))
        aggregate["symbols"] = int(aggregate["symbols"]) + len(
            record.get("symbols", ())
        )
        aggregate["secret_indicators"] = int(aggregate["secret_indicators"]) + int(
            record.get("secret_indicator_count", 0)
        )
    ...
    priority = sorted(
        ...
        key=lambda row: (
            {
                "already_covered_exact": 0,
                "review_as_existing_skill_extension": 1,
                "clean_room_contract_candidate": 2,
            }.get(str(row["disposition"]), 9),
            -float(row["catalog_similarity"]),
            str(row["name"]),
        ),
    )
    ...
    duplicates = [
        ...
        for digest, members in sorted(seen_hashes.items())
    ...
    plan = {
        ...
        "all_skill_candidates": sorted(
            candidate_rows,
            key=lambda row: (str(row["source_alias"]), str(row["relative_path"])),
        ),
...
def main() -> int:
    ...
    with (output / "admission-plan.json").open(
