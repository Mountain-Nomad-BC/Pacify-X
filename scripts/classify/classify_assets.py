...
CLASSES = {
    "skill", "orchestration", "builder", "contract", "policy", "repair_pattern",
    "template", "test_pattern", "evidence_pattern", "research_extraction",
    "knowledge_reference_only", "duplicate", "unsafe_untrusted",
    "clean_room_required", "not_applicable",
}
...
def classify(item: dict, duplicate_ids: set[str]) -> tuple[str, float, str]:
    ...
    rules = (
        ...
        ("research_extraction", ("paper", "research", "arxiv", "benchmark"), "reference_only"),
    )
    ...
    if not item.get("structure", {}).get("headings") and suffix not in {".py", ".js", ".ts", ".tsx", ".go", ".java", ".cs"}:
...
def main() -> int:
    ...
    for item in read_jsonl(args.inventory):
        ...
        records.append({
            "id": item["id"], "source_tree": item["source_tree"], "path": item["path"],
            "class": asset_class, "confidence": confidence, "review_state": review_state,
            "probable_domain": item["probable_domain"], "sha256": item["sha256"],
        })
    records.sort(key=lambda item: (item["class"], item["probable_domain"], item["source_tree"], item["path"]))
    count, digest = write_jsonl(args.output, records)
    summary = {
        "schema_version": "1.0", "record_count": count, "unknown_count": 0,
        "classes_present": sorted({item["class"] for item in records}), "output_sha256": digest,
    }
    args.output.with_name("asset_classification_summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
