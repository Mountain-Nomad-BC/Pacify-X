...
def main() -> int:
    ...
    for item in read_jsonl(args.input):
        grouped[(item["class"], item["probable_domain"], item["review_state"])].append(
            item
        )
    clusters = []
    for key, items in sorted(grouped.items()):
        ...
        representative = min(
            items, key=lambda item: (len(item["path"]), item["path"], item["id"])
        )
        clusters.append(
            {
                "cluster_id": f"review-{cluster_id}",
                "class": asset_class,
                "domain": domain,
    ...
    output = {
        ...
        "asset_count": sum(item["count"] for item in clusters),
        "clusters": clusters,
    }
