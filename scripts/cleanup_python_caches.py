...
def cleanup(root: Path, *, apply: bool = False, quarantine_root: Path | None = None) -> dict[str, object]:
    ...
    quarantine_base = (quarantine_root or (resolved / ".engineering-bootstrap" / "quarantine" / "disposable-cache")).resolve()
    if quarantine_base == resolved or quarantine_base == Path(quarantine_base.anchor):
        raise ValueError("quarantine root must be a bounded directory distinct from the workspace root")
    directories = sorted(
        (
            path for path in resolved.rglob("*")
            if path.is_dir() and path.name in CACHE_DIRECTORIES and not _inside(path.resolve(), quarantine_base)
        ),
        key=lambda path: len(path.parts), reverse=True,
    )
    files = sorted(
        path for path in resolved.rglob("*")
        if path.is_file() and path.suffix.casefold() in BYTECODE_SUFFIXES
    ...
    if apply:
        ...
        for item in inventory:
            ...
            if active.exists() or not quarantined.is_file() or _digest(quarantined) != item["sha256"]:
                raise RuntimeError(f"cache quarantine reconciliation failed: {item['path']}")
        receipt = {
            "schema_version": "1.0",
            "operation": "recoverable_cache_quarantine",
            "created_utc": datetime.now(timezone.utc).isoformat(),
            "source_root": ".",
            "quarantine_root": destination.relative_to(resolved).as_posix() if _inside(destination, resolved) else destination.as_posix(),
