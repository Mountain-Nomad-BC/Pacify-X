...
def classify(path: Path, relative: Path, size: int) -> tuple[str, str]:
    ...
    if any(
        token in parts
        for token in {"security", "governance", "policy", "certification", "evidence"}
    ):
        return "governance_or_assurance", "review_for_pattern"
    if (
        any(token in parts for token in {"skill_staging", "skills", ".codex"})
        or "skill" in name
    ):
        return "skill_or_registry", "admission_required"
    if any(token in parts for token in {"tests", "test-results"}) or name.startswith(
        "test_"
    ):
    ...
    if any(
        token in parts
        for token in {"docs", "references", "temp_references", "knowledge", "research"}
    ):
...
def text_summary(path: Path) -> str:
    if (
        path.suffix.casefold() not in TEXT_EXTENSIONS
...
def main() -> int:
    ...
    parser.add_argument(
        "--summaries",
        action="store_true",
        help="Read bounded excerpts from small text files",
    )
    ...
    for directory, directories, filenames in os.walk(args.root):
        directories[:] = [
            name for name in directories if name.casefold() not in EXCLUDED_SEGMENTS
        ]
        base = Path(directory)
        for filename in filenames:
            ...
            row = {
                "path": redact(str(relative)),
                "extension": redact(path.suffix.casefold()),
                "bytes": size,
                "category": category,
    ...
    with (args.out / "file_catalog.csv").open(
    ...
    report = {
        "root": redact(str(args.root)),
        "file_count": len(rows),
        "categories": categories,
        "recommendations": recommendations,
        "top_extensions": extensions.most_common(30),
    }
    (args.out / "catalog_summary.json").write_text(
