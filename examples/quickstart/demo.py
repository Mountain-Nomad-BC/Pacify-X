...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the bounded PACIFY-X quickstart demonstration.")
    parser.add_argument("--output", type=Path, required=True, help="New directory that will retain the demo project and evidence")
    ...
    steps["initialization_preview"] = _run("commission", "--mode", "new", "--project", str(project))
    steps["initialization"] = _run("commission", "--mode", "new", "--project", str(project), "--apply")
    ...
    valid = all(bool(value.get("valid", True)) for value in steps.values() if isinstance(value, dict))
    receipt = {
        "schema_version": "1.0",
        "valid": valid,
        "project": "project",
        "changed_paths": ["project/", "evidence/demo-receipt.json"],
        "effects": ["create explicitly requested demo output", "no network", "no deletion"],
        "steps": steps,
    }
    ...
    print(json.dumps({"valid": valid, "evidence": str(path), "receipt_sha256": receipt["receipt_sha256"]}, indent=2))
