...
def classify(path: str) -> tuple[str, str]:
    checks = (
        (
            "/skills/",
            "skill-component",
            "Recover an authoritative behavior contract or specification; clean-room implement or merge into a canonical skill; add discriminating tests; validate and map.",
        ),
        (
            "/orchestrations/",
            "orchestration",
            "Recover the workflow contract; rebuild a bounded DAG with dependencies, effects, failure policy, rollback, and acceptance tests.",
        ),
        (
            "/scripts/",
            "script",
            "Recover behavior and I/O contracts; clean-room implement without executing intake code; add positive, negative, and hostile-input tests.",
        ),
        (
            "/schemas/",
            "schema",
            "Recover field semantics and consumers; implement the canonical schema; validate examples, negative cases, and compatibility.",
        ),
        (
            "/tests/",
            "test-or-evaluation",
            "Recover acceptance intent independently; rebuild discriminating cases and execute them against the canonical owner.",
        ),
        (
            "/evaluations/",
            "test-or-evaluation",
        ...
        (
            "/evidence/",
            "evidence",
            "Do not recreate historical proof from filenames; regenerate evidence only from successful current validations.",
        ),
        (
            "/registry/",
            "registry",
            "Regenerate from admitted canonical owners after all dependent bodies and contracts exist.",
        ),
        (
            "/references/",
            "reference-or-knowledge",
            "Recover authoritative source material or reconstruct a sanitized, cited reference; bind it to a specific skill trigger.",
        ),
        (
            "/knowledge/",
            "reference-or-knowledge",
        ...
        (
            "/templates/",
            "template",
            "Recover required fields and consuming contract; rebuild the minimal schema-bound template and validate round trips.",
        ),
    )
    ...
    return (
        "pack-metadata",
        "Recover or reconstruct only if required by an admitted owner; otherwise record a justified non-admission.",
    )
...
def write_outputs(
    rows: list[dict], reconciliation: dict, markdown_path: Path, csv_path: Path
) -> None:
    ...
    lines.extend(
        f"| {name} | {count} |"
        for name, count in reconciliation["missing_by_pack"].items()
    )
    lines.extend(
        [
            "",
            "## Missing by artifact type",
            "",
    ...
    lines.extend(
        f"| {name} | {count} |"
        for name, count in sorted(Counter(row["artifact_type"] for row in rows).items())
    )
    lines.extend(["", "## Full recovery backlog", ""])
    lines.extend(
        f"- [ ] `{row['path']}` — {row['artifact_type']}; expected {row['expected_bytes']} bytes; SHA-256 `{row['expected_sha256']}`"
        for row in rows
    )
