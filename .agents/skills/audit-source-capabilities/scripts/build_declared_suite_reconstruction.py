...
def targets(kind: str, source_id: str, owner: str | None) -> list[str]:
    if kind == "skill":
        base = [f".agents/skills/{owner or source_id}/SKILL.md", f"registry/skill_packages/{owner or source_id}.json"]
    elif kind == "script":
        base = [f".agents/skills/{owner or 'audit-source-capabilities'}/scripts/{source_id.replace('-', '_')}.py"]
    elif kind == "orchestration":
        base = [f"orchestration/workflows/reconstructed/{source_id}.yaml", f"registry/orchestration_outcomes/{source_id}.json"]
    elif kind == "schema":
...
def acceptance(kind: str, source_id: str) -> list[str]:
    ...
    if kind == "orchestration":
        common.append("Dependency order, budget, failure policy, rollback or compensation, and evidence outputs are executable and tested.")
    elif kind == "script":
        common.append("The helper has deterministic structured output, a nonzero invalid-input exit, and direct unit tests.")
    elif kind == "skill":
        common.append("The skill has a precise trigger, bounded workflow, safety boundary, verification method, package entry, and official validation receipt.")
    elif kind == "schema":
        common.append("The schema is meta-valid and has valid, invalid, compatibility, owner, and consumer tests.")
    else:
        common.append("The artifact is consumed by an executable surface and is not retained as disconnected prose or metadata.")
    return common
...
def main() -> int:
    ...
    missing = [r for r in all_missing if r["pack"].startswith(ABSENT_PACK_PREFIXES) or r["pack"] == "registry"]
    ...
    candidates = [
        c for c in load_json(args.candidates)["candidates"]
        if c["presence"] == "manifest-only" and not (c["kind"] == "script" and c["id"] == "init")
    ]
    recoveries = {(r["kind"], r["source_id"]): r for r in load_json(args.recovery_map)["records"]}
    exact_recoveries = {r["declared_path"]: r for r in load_json(args.exact_recovery)["records"]}
    if len(candidates) != 260 or len(recoveries) != 260:
        raise SystemExit(f"unexpected operational denominator: candidates={len(candidates)} recoveries={len(recoveries)}")
    ...
    for candidate in sorted(candidates, key=lambda item: (item["kind"], item["id"])):
        ...
        paths = sorted(
            p for p in (normalize_manifest_path(s["path"]) for s in candidate["sources"])
            if p in missing_by_path
        )
        if not paths:
            declared_paths = sorted(normalize_manifest_path(s["path"]) for s in candidate["sources"])
            ...
            if not all(records) or not all(record["exact_match"] for record in records):
                raise SystemExit(f"operational candidate has neither absent paths nor exact recoveries: {key}")
        ...
        cards.append({
            ...
            "behavior_contract": inferred_behavior(candidate["kind"], candidate["id"], pack),
            "implementation_targets": targets(candidate["kind"], candidate["id"], recovery["canonical_owner"]),
            "wiring_targets": ["registry/semantic_capability_index.json", "registry/capability_aliases.json", "registry/graphs", "orchestration/workflows"],
            "required_tests": [f"tests/reconstruction/test_{candidate['kind']}_{candidate['id'].replace('-', '_')}.py"],
    ...
    for (kind, source_id), rows in sorted(support_groups.items()):
        ...
        cards.append({
            ...
            "implementation_targets": targets(kind, source_id, "audit-source-capabilities"),
            "wiring_targets": ["registry/contract_ownership.json", "registry/artifact-provenance.json", "tests/reconstruction"],
            "required_tests": [f"tests/reconstruction/test_support_{slug(kind).replace('-', '_')}_{source_id.replace('-', '_')}.py"],
    ...
    if unassigned or extras or len(assigned) != 1133:
        raise SystemExit(f"coverage failure: assigned={len(assigned)} unassigned={len(unassigned)} extras={len(extras)}")
    ...
    document = {
        ...
        "waves": [
            ...
            {"wave": 8, "packs": ["all"], "outcome": "cross-pack integration, installed-package validation, sanitation, and revocable certification"},
        ],
    ...
    for wave in document["waves"]:
        lines.append(f"{wave['wave']}. **{', '.join(wave['packs'])}** — {wave['outcome']}.")
    lines += [
        "",
        "## Completion gate",
        "",
    ...
    for card in cards:
        lines.append(f"| `{card['card_id']}` | {card['pack']} | {card['kind']} | `{card['source_id']}` | `{card['canonical_owner']}` | {len(card['source_paths'])} | {card['current_state']} |")
