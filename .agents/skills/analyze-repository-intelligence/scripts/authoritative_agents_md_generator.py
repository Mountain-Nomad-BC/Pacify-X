...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('repo_map'); ap.add_argument('--out',default='AGENTS.md'); a=ap.parse_args(); m=json.loads(Path(a.repo_map).read_text()); langs=', '.join(f'{k}:{v}' for k,v in sorted(m.get('languages',{}).items()));
 ...
 Path(a.out).write_text(body); print(json.dumps({'out':a.out,'bytes':len(body)},indent=2))
