...
Default to read-only discovery. Never store secret values. Reject symlink traversal, bound every walk, archive previous maps, and promote a new map only after validation.

## Procedure

1. Resolve the project root and confirm scope.
2. Run `scripts/build_project_map.py <project-root>`.
