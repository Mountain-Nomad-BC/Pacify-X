...
from runtime.graphs import build_graphs, find_io_paths, rank_io_paths, validate_orchestration
...
class GraphAndOrchestrationTests(unittest.TestCase):
    def test_canonical_graphs_match_all_current_registry_sources(self) -> None:
        ...
        self.assertEqual(set(build_graph_artifacts(ROOT)), {path.name for path in (ROOT / "registry/graphs").glob("*.json")})
    ...
    def test_registry_graphs_are_deterministic_and_metadata_only(self) -> None:
        ...
        self.assertTrue(any(edge.relation == "depends_on" for edge in first.capability_edges))
        self.assertTrue(any(edge.relation == "declares_effect" for edge in first.dependency_effect_edges))

    def test_io_path_reports_compatible_transformations(self) -> None:
        records = [
            {"id": "intake", "provides": ["normalized_request"], "consumes": ["request"], "dependencies": [], "conflicts": [], "effects": ["read_local"]},
            {"id": "plan", "provides": ["plan"], "consumes": ["normalized_request"], "dependencies": [], "conflicts": [], "effects": ["read_local"]},
        ]
        self.assertEqual(find_io_paths(records, "request", "plan"), (("intake", "plan"),))
    ...
    def test_io_path_ranking_prefers_safer_lower_cost_current_path(self) -> None:
        records = [
            {"id": "safe", "provides": ["result"], "consumes": ["request"], "dependencies": [], "conflicts": [], "effects": ["read_local"], "risk": "R0", "cost": {"max_tool_calls": 0}, "latency": {"max_seconds": 1}, "evidence": {"status": "current"}},
            {"id": "costly", "provides": ["result"], "consumes": ["request"], "dependencies": [], "conflicts": [], "effects": ["network"], "risk": "R2", "cost": {"max_tool_calls": 5}, "latency": {"max_seconds": 10}, "evidence": {"status": "stale"}},
        ]
    ...
    def test_orchestration_rejects_unknown_step_cycle_and_effect_conflict(self) -> None:
        spec = {
            "id": "unsafe", "version": "1", "status": "candidate", "inputs": [], "outputs": [],
            "steps": [
                {"id": "a", "capability": "skill-navigator", "depends_on": ["b"], "effects": ["read_local"]},
                {"id": "b", "capability": "missing", "depends_on": ["a"], "effects": ["network"]},
            ],
            "parallelism": {"max_agents": 1, "max_test_runners": 1},
            "stop_conditions": [], "resource_budget": {"max_tool_calls": 1, "max_seconds": 1},
        }
    ...
    def test_versioned_schema_set_is_present(self) -> None:
        names = {
            "capability-contract.schema.json", "evidence-record.schema.json", "orchestration-contract.schema.json",
            "tool-contract.schema.json", "model-contract.schema.json", "integration-contract.schema.json",
            "builder-contract.schema.json", "knowledge-source.schema.json", "policy-contract.schema.json",
