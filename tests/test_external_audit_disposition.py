...
class ExternalAuditDispositionTests(unittest.TestCase):
    def test_all_three_audits_have_zero_open_dispositions_and_live_evidence(self) -> None:
        receipt = json.loads((ROOT / "evidence/external-audit-disposition-20260803.json").read_text(encoding="utf-8"))
        ...
        self.assertEqual({item["id"] for item in receipt["dispositions"]}, {f"A{number:02d}" for number in range(1, 31)})
