...
class MetacognitiveEvolutionTests(unittest.TestCase):
    ...
    def test_epistemic_and_contradiction_operations(self) -> None:
                ...
                "hypotheses": [
                    {
                        ...
                        "prior": 0.5,
                        "evidence": [
                            {"likelihood_if_true": 0.9, "likelihood_if_false": 0.1}
                        ],
                    }
                ],
            },
        ...
        contradiction = run_operation(
            "detect-contradictions",
            {
                "claims": [
                    {"id": "a", "subject": "x", "value": 1},
                    {"id": "b", "subject": "x", "value": 2},
                ]
            },
        )
    ...
    def test_trace_rejects_private_reasoning_capture(self) -> None:
                ...
                "events": [
                    {
                        ...
                        "summary": "selected bounded option",
        ...
        self.assertTrue(
            any(
                "private chain-of-thought" in error
    ...
    def test_engineering_profile_blocks_protected_inference(self) -> None:
        result = run_operation(
            "profile-engineering-practices",
            {
                "events": [
                    {
                        "id": "e",
                        "pattern_type": "testing_practice",
        ...
        self.assertFalse(result["valid"])
        self.assertEqual(
            result["result"]["policy_violations"][0]["prohibited_fields"], ["race"]
        )

    def test_semantic_effect_lint_and_unknown_operation_fail_closed(self) -> None:
        result = run_operation(
            "lint-semantic-contract",
            {
                "capability_id": "x",
                "reads": [],
                "writes": ["memory.x"],
                "epistemic_effects": ["confidence"],
                "evidence_required": ["test"],
                "rollback": {"required": True, "method": "restore"},
                "observed_effects": ["memory.x"],
            },
        )
        self.assertTrue(result["valid"], result)
    ...
    def test_optimization_is_bounded_and_reversible(self) -> None:
        payload = {
            "id": "exp",
            "baseline": {"id": "base", "metrics": {"quality": 0.8, "cost": 0.5}},
            "candidates": [{"id": "better", "metrics": {"quality": 0.9, "cost": 0.4}}],
            "directions": {"quality": "maximize", "cost": "minimize"},
            "hard_thresholds": {"quality": 0.8},
            "utility_weights": {"quality": 1.0, "cost": 0.1},
        }
    ...
    def test_facade_is_payload_only_read_only_and_cannot_activate_or_cross_memory_roots(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            payload = {
                "id": "bounded",
                "hypotheses": [],
                "foreign_memory_path": str(sentinel),
