...
class MemoryOperationTests(unittest.TestCase):
    def test_session_summaries_process_only_deltas_and_distinguish_stop_from_end(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ledger = SessionSummaryLedger(Path(directory))
            first = ledger.summarize(
                "session",
                (
            ...
            self.assertIsNone(
                ledger.summarize("session", (SessionEvent(4, "SessionEnd", "", NOW),))
            )

    def test_graph_clustering_is_bounded_keeps_provenance_and_continues_after_missing_edges(
        self,
    ) -> None:
        nodes = (
        ...
        result = build_graph_clusters(
            nodes, (("missing", "a"), ("a", "b")), max_cluster_size=2
        )
    ...
    def test_backend_timeout_and_search_failure_are_not_reported_as_empty(self) -> None:
        ...
        self.assertEqual(
            guard.execute(
                "slow", lambda: time.sleep(0.05), timeout_seconds=0.001
            ).error_code,
            "TimeoutError",
        )
        self.assertEqual(
            guard.execute("slow", lambda: (), timeout_seconds=0.1).status,
            "circuit_open",
        )

    def test_attribution_propagates_through_bulk_actions_and_rejects_spoofing(
        self,
    ) -> None:
        payload = normalize_action_attribution(
            {"records": [{"value": 1}, {"value": 2}]}, authenticated_agent_id="agent"
        )
        self.assertTrue(
            all(
                item["createdBy"] == item["agentId"] == "agent"
                for item in payload["records"]
            )
        )
        with self.assertRaisesRegex(ValueError, "does not match"):
            normalize_action_attribution(
                {"agentId": "other"}, authenticated_agent_id="agent"
            )
    ...
    def test_graph_failure_does_not_rollback_canonical_persistence(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = persist_with_graph_isolation(
                vault,
                record(),
                lambda _: (_ for _ in ()).throw(RuntimeError("graph down")),
                StateKVGuard(),
                timeout_seconds=0.1,
            )
