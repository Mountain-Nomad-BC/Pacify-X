...
class CorpusPipelineTests(unittest.TestCase):
    def run_script(
        self, relative: str, *arguments: str
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for output in (first, second):
                result = self.run_script(
                    "scripts/inventory/build_file_inventory.py",
                    "--root",
                    f"fixture={source}",
                    "--output",
                    str(output),
            ...
            summary = json.loads(
                first.with_name("file_inventory_summary.json").read_text(
                    encoding="utf-8"
                )
            )
    ...
    def test_dedup_classification_and_review_clusters_cover_every_record(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                self.run_script(
                    "scripts/deduplicate/hash_exact_duplicates.py",
                    "--inventory",
                    str(inventory),
                    "--output",
                    str(exact),
            ...
            self.assertEqual(
                json.loads(exact.read_text(encoding="utf-8"))["group_count"], 1
            )
            classified = temp / "classified.jsonl"
            result = self.run_script(
                "scripts/classify/classify_assets.py",
                "--inventory",
                str(inventory),
                "--exact-duplicates",
                str(exact),
            ...
            self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
            summary = json.loads(
                classified.with_name("asset_classification_summary.json").read_text(
                    encoding="utf-8"
                )
            )
            ...
            self.assertEqual(
                self.run_script(
                    "scripts/classify/build_review_clusters.py",
                    "--input",
                    str(classified),
                    "--output",
                    str(clusters),
            ...
            self.assertEqual(
                json.loads(clusters.read_text(encoding="utf-8"))["asset_count"], 2
            )
    ...
    def test_partition_campaign_reconciles_records_and_logged_errors(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            (part / "file_inventory_summary.json").write_text(
                json.dumps(
                    {
                        "record_count": 1,
                        "error_count": 1,
