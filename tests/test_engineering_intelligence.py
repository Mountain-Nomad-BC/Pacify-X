...
class EngineeringIntelligenceTests(unittest.TestCase):
    def test_drift_shockwave_and_refactoring_are_deterministic_and_proposal_only(
        self,
    ) -> None:
        drift = architecture_drift(
            {"api": ["db"]}, {"api": ["db", "cache"], "worker": ["db"]}
        )
        self.assertGreater(drift["drift_score"], 0)
        shockwave = dependency_shockwave(
            {"api": ["db"], "ui": ["api"], "report": ["ui"]}, ["db"], max_depth=2
        )
        self.assertEqual(
            [item["component"] for item in shockwave["affected"]], ["api", "ui"]
        )
        plan = refactoring_plan({"findings": ({"kind": "contract"},)}, drift)
        self.assertFalse(plan["auto_apply"])
        self.assertTrue(plan["actions"])

    def test_semantics_collisions_debt_health_and_fitness_keep_unknowns_visible(
        self,
    ) -> None:
    ...
    def test_regression_engine_ranks_hypotheses_without_claiming_causality(
        self,
    ) -> None:
    ...
    def test_evolution_and_resilience_candidates_are_bounded_and_non_activating(
        self,
    ) -> None:
        dna = framework_dna(
            {"api": ["db"]},
            [{"decision": "typed contracts"}],
            [{"id": "p1", "validated": True, "principle": "fail closed"}],
            exclusions=["secrets"],
        )
        self.assertFalse(dna["auto_activate"])
        opportunities = opportunity_backlog(
            [
                {
                    "activity": "review",
                    "repetitions": 10,
        ...
        patterns = pattern_candidates(
            [
                {"signature": "verify", "example": "a"},
                {"signature": "verify", "example": "b"},
            ]
        )
        ...
        twin = repository_digital_twin(
            ["api", "db"], {"api": ["db"]}, [("api", "db")], []
        )
        self.assertFalse(twin["source_of_truth"])

    def test_research_requires_citations_and_never_becomes_production_proof(
        self,
    ) -> None:
        text = "source"
        ...
        first = ResearchMechanism(
            ...
            "Gate memory through evidence and revision checks.",
            ("current evidence",),
            "benchmark-a",
            ("small sample",),
            ("local benchmark",),
        ...
        second = ResearchMechanism(
            ...
            "doi:10.1/b",
            "memory-trust",
            "Revision checks reduce stale use.",
            "Gate retrieved memory through evidence and revision status.",
            ("revision metadata",),
            "benchmark-b",
            ("domain limited",),
            ("negative stale-memory test",),
        )
        ...
        experiment = admission_experiment(
            candidate, local_tests=["reproduce"], negative_tests=["stale rejected"]
        )
        self.assertEqual(experiment["decision"], "ready_for_local_experiment")
        missing = canonicalize_research(
            [
                ResearchMechanism(
                    "paper-c",
                    digest,
