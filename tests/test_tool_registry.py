...
class ToolRegistryTests(unittest.TestCase):
    def test_startup_candidates_remain_bounded_and_metadata_driven(self) -> None:
        ...
        self.assertEqual(dict(snapshot.tools), {"docker": "/docker", "git": "/git", "rg": "/rg"})

    def test_tool_family_is_probed_only_when_selected(self) -> None:
        probes = probe_tool_family(ROOT, "security-scanner", resolver=lambda name: f"/{name}" if name == "trivy" else None)
        self.assertTrue(any(item.candidate == "trivy" and item.location == "/trivy" for item in probes))
