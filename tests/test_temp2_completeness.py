...
from runtime.project_stream_orchestrator import ProjectStreamContext, execute_project_stream
...
def _assert_schema_subset(test: unittest.TestCase, value: object, schema: dict[str, object], path: str = "root") -> None:
    ...
    if allowed:
        ...
        test.assertTrue(any(matches.get(kind, False) for kind in allowed), f"{path}: expected {allowed}, got {type(value).__name__}")
    if "const" in schema:
    ...
    if isinstance(value, dict):
        ...
        test.assertFalse(required - set(value), f"{path}: missing {sorted(required - set(value))}")
        properties = dict(schema.get("properties", {}))
        if schema.get("additionalProperties") is False:
            test.assertFalse(set(value) - set(properties), f"{path}: unexpected {sorted(set(value) - set(properties))}")
    ...
    def test_structured_templates_conform_to_their_contracts(self) -> None:
        ...
        for template_name, contract_name in pairs.items():
            with self.subTest(template=template_name):
                ...
                value = tomllib.loads(raw) if template_name.endswith(".toml") else (json.loads(raw) if template_name.endswith(".json") else yaml.safe_load(raw))
                schema = json.loads((contract_root / contract_name).read_text(encoding="utf-8"))
                _assert_schema_subset(self, value, schema)

    def test_markdown_templates_expose_their_contract_fields(self) -> None:
        checks = {
            "decision_record.md": ("Decision ID", "Project", "Status", "## Decision", "## Evidence"),
            "memory_note.md": ("memory_id:", "project_id:", "source_sha256:", "certification_status:", "retrieval_enabled:"),
            "quarantine_review.md": ("Quarantine ID", "Project ID", "Original path", "Quarantine path", "Pre-move SHA-256"),
            "transfer_package.md": ("Transfer ID", "Source project", "Destination project", "Purpose", "Provenance and license"),
        }
        for name, markers in checks.items():
            body = (ROOT / "templates/project_stream" / name).read_text(encoding="utf-8")
            with self.subTest(template=name):
                for marker in markers:
                    self.assertIn(marker, body)

    def test_every_registered_project_stream_workflow_executes_with_synthetic_inputs(self) -> None:
        handlers = _load_json("registry/project_stream_handlers.json")["workflows"]
        effects = {item["orchestration_id"]: tuple(item["effects"]) for item in handlers}
        with tempfile.TemporaryDirectory() as directory:
            temp = Path(directory)
            vault = MemoryVault(temp / "memory-vault", workspace_id="workspace", project_id="project")
            ...
            onboard.mkdir()
            (onboard / "pyproject.toml").write_text("[project]\nname='fixture'\n", encoding="utf-8")
            ...
            payloads: dict[str, dict[str, object]] = {
                "agent_create_validate": {
                    ...
                    "specification": {
                        ...
                        "sandbox_validated": True, "evidence": ["synthetic-test"],
                    },
                },
                "chaos_resilience_cycle": {"experiments": [{
                ...
                "cross_project_transfer": {
                    ...
                    "package": TransferPackage(
                        "transfer-one", "project-old", "project-new", "sanitized_capability",
                        ("synthetic-evidence",), "internal-reference", (), ("synthetic-test",),
                        True, True, True,
                    ),
                },
                "guarded_change": {
                    ...
                    "ledger": temp / "change-ledger", "evidence": {
                        ...
                        "rollback": "quarantine", "approval_id": "approval", "idempotency_key": "change-one",
                ...
                "incident_diagnose_recover": {
                    ...
                    "ledger": temp / "recovery-ledger", "evidence": {
                        ...
                        "recovery_tests": ["synthetic"], "rollback_rehearsal": "verified",
                        "approval_id": "approval", "recovery_tests_passed": True, "rollback_rehearsed": True,
                    },
                },
                "memory_ingest_distill": {"vault": vault, "sources": (_source(),)},
                "memory_maintenance": {"vault": vault},
                "nightly_project_health": {"metrics": {name: 1 for name in ("tests", "security", "evidence", "dependencies", "memory", "operations")}},
                "project_close": {"ledger": temp / "close-ledger", "evidence": ("synthetic-test",)},
                "project_onboard": {"project": onboard, "ledger": temp / "onboard-ledger"},
                "project_pause_resume": {"ledger": temp / "pause-ledger", "action": "pause", "evidence": ("checkpoint",)},
                "project_switch": {
                    "ledger": temp / "switch-ledger",
                    "old_scope": ScopeEnvelope("workspace", "project-old", "agent", "session-old", "stream", "lease-old", "intent", "correlation"),
                    "new_scope": ScopeEnvelope("workspace", "project-new", "agent", "session-new", "stream", "lease-new", "intent", "correlation"),
                    "switch_evidence": SwitchEvidence(True, True, True, True, True, True, True, True),
                },
                "safe_cleanup": {
                ...
                "workspace_bootstrap": {
                    ...
                    "project": temp / "bootstrap-workspace" / "projects" / "commissioned",
                    "source_root": ROOT,
                },
                "workstream_plan_dispatch": {
                    "workstreams": ({"work_id": "one", "worker_id": "worker", "lane": "light", "owned_paths": ("src",)},),
            ...
            for workflow_id in sorted(payloads):
                context = ProjectStreamContext(
                    workflow_id,
                    "wsp_workspace" if workflow_id == "workspace_bootstrap" else "workspace",
                    "" if workflow_id == "workspace_bootstrap" else ("prj_project" if workflow_id == "project_onboard" else "project"),
                    "agent",
                    "session",
                    "intent",
                ...
                result = execute_project_stream(ROOT, context, checkpoint_root=temp / "checkpoints")
                results[workflow_id] = result.status
                self.assertEqual(result.status, "completed", f"{workflow_id}: {result.reasons}")
