...
class SkillNavigatorTests(unittest.TestCase):
    def setUp(self) -> None:
        self.index = [
            CapabilitySummary(
                ...
                "Map repository architecture and ownership",
                ("analyze repository",),
                ("repo map",),
                ("project_root",),
            ),
            CapabilitySummary(
                "tool-admission",
                "Review tool provenance permissions and effects",
                ("install scanner",),
                ("admit tool",),
                ("tool_manifest",),
                risk="R2",
            ...
            CapabilitySummary(
                "draft-only",
                "Unreviewed repository helper",
                ("repository",),
                status="candidate",
            ),
            CapabilitySummary(
                "evidence-assembler",
                "Build typed evidence for claims",
                ("assemble evidence",),
    ...
    def test_returns_bounded_ranked_metadata_only(self) -> None:
        result = navigate(
            "analyze repository and make a repo map",
            self.index,
            {"project_root": "."},
            max_candidates=1,
        )
    ...
    def test_candidate_status_is_not_discoverable(self) -> None:
        ...
        self.assertNotIn(
            "draft-only", [item.capability_id for item in result.candidates]
        )
    ...
    def test_deterministic_tie_breaking_and_limit(self) -> None:
        index = [
            CapabilitySummary("z-skill", "validate result"),
            CapabilitySummary("a-skill", "validate result"),
        ]
