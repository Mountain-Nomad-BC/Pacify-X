...
def test_deleted_event_breaks_hash_chain() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        append_chained_event(ledger, "one", {"value": 1}); second = append_chained_event(ledger, "two", {"value": 2})
...
def test_reordered_event_breaks_hash_chain() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ledger = Path(directory) / "events"
        one = append_chained_event(ledger, "one", {"value": 1}); two = append_chained_event(ledger, "two", {"value": 2})
        one_bytes, two_bytes = one.read_bytes(), two.read_bytes()
        one.write_bytes(two_bytes); two.write_bytes(one_bytes)
        assert not validate_event_ledger(ledger)["valid"]
...
def test_concurrent_event_append_preserves_chain() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        with ThreadPoolExecutor(max_workers=4) as pool:
            tuple(pool.map(lambda value: append_chained_event(ledger, "event", {"value": value}), range(16)))
...
def test_rewritten_history_cannot_replace_protected_head() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        record = json.loads(event.read_text(encoding="utf-8")); record["payload"] = {"value": 9}
        record["payload_sha256"] = hashlib.sha256(_canonical(record["payload"])).hexdigest()
        record["event_sha256"] = hashlib.sha256(_canonical({key: value for key, value in record.items() if key != "event_sha256"})).hexdigest()
