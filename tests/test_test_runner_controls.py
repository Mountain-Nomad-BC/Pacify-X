...
def test_timeout_kills_process_tree() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        result = run_test_command([sys.executable, "-c", parent_code], cwd=ROOT, environment=os.environ, timeout_seconds=0.5)
...
def test_timeout_preserves_partial_output() -> None:
    result = run_test_command(
        [sys.executable, "-c", "import sys,time; print('partial-out',flush=True); print('partial-err',file=sys.stderr,flush=True); time.sleep(60)"],
        cwd=ROOT, environment=os.environ, timeout_seconds=0.25,
    )
...
def test_fast_profile_excludes_release_duration_tests() -> None:
    ...
    expected = {
        "tests/test_exact_tool_certification.py", "tests/test_installed_wheel_e2e.py",
        "tests/test_release_certification.py", "tests/test_release_audit.py",
    }
