...
from runtime.release_signing import bind_content_digest, public_key_fingerprint, sign_certificate, verify_certificate_signature


def _run(root: Path, *args: str) -> None:
    subprocess.run(list(args), cwd=root, check=True, text=True, capture_output=True, timeout=30)
...
def _repository() -> Path:
    ...
    _run(root, "git", "remote", "add", "origin", "git@github.com:Mountain-Nomad-BC/Pacify-X.git")
...
def test_wrong_repository_identity_fails() -> None:
    root = _repository()
    _run(root, "git", "remote", "set-url", "origin", "git@github.com:other/repository.git")
    result = capture_git_identity(root, version="1.2.3")
...
def test_git_tree_change_during_run_aborts_certification() -> None:
    root = _repository()
    (root / "policies").mkdir()
    (root / "policies/release-artifact-policy.json").write_text(json.dumps({
        "schema_version": "1.0", "policy_version": "1.0.0",
        "product_roots": ["policies", "runtime"], "product_root_files": ["README.md", "pyproject.toml"],
        "evidence_roots": [], "intermediate_names": [".git"],
        "intermediate_name_suffixes": [], "intermediate_suffixes": [],
        "control_output_paths": [], "evidence_allowed_suffixes": [".json"],
...
def _signing_fixture() -> tuple[Path, Path, Path, dict]:
    ...
    _run(root, "ssh-keygen", "-q", "-t", "ed25519", "-N", "", "-C", "release-test", "-f", str(private))
    ...
    policy.write_text(json.dumps({
        "trusted_signers": [{
            "identity": "pacify-x-release", "publisher": "Mountain-Nomad-BC",
            "fingerprint": fingerprint, "public_key": public_line,
        }],
        "revoked_fingerprints": [],
    }), encoding="utf-8")
    return root, private, policy, {"schema_version": "3.0", "release": "1.2.3", "status": "self_certified"}
...
def test_certificate_signature_verification() -> None:
    ...
    assert verify_certificate_signature(certificate, signature_path=signature, trust_policy_path=policy)["valid"]
...
def test_recomputed_untrusted_checksum_does_not_authenticate_certificate() -> None:
    ...
    assert not verify_certificate_signature(certificate, signature_path=signature, trust_policy_path=policy)["valid"]
...
def test_unsigned_release_certificate_is_rejected() -> None:
    ...
    assert not verify_certificate_signature(value, signature_path=root / "missing.sig", trust_policy_path=policy)["valid"]
...
def test_evidence_manifest_is_deterministic() -> None:
    ...
    roles = {"tests.xml": {"type": "junit", "required": True, "generation_gate": "full_tests", "producer": "pytest 9"}}
    first = build_evidence_manifest(root, roles=roles, generated_utc="2026-08-03T00:00:00Z")
    second = build_evidence_manifest(root, roles=roles, generated_utc="2026-08-03T00:00:00Z")
...
def test_evidence_file_tampering_revokes_certificate(mutation: str) -> None:
    ...
    roles = {"tests.xml": {"type": "junit", "required": True, "generation_gate": "full_tests", "producer": "pytest"}}
    manifest = build_evidence_manifest(root, roles=roles, generated_utc="2026-08-03T00:00:00Z")
    if mutation == "edit":
...
def test_missing_required_evidence_fails_verification() -> None:
    root = Path(tempfile.mkdtemp())
    manifest = build_evidence_manifest(root, roles={"missing.json": {"type": "report", "required": True}})
    assert not manifest["valid"]


def test_evidence_manifest_path_escape_is_rejected() -> None:
    root = Path(tempfile.mkdtemp())
    manifest = build_evidence_manifest(root, roles={"../outside.json": {"type": "report", "required": True}})
    assert not manifest["valid"]
