...
def test_evidence_amplification_budget_fails_before_finalizer(tmp_path: Path) -> None:
    ...
    evidence.mkdir()
    (evidence / "coverage.json").write_bytes(b"x" * 101)
    result = evidence_budget(
        tmp_path,
        {
            "max_total_release_evidence_bytes": 1000,
            "max_single_evidence_file_bytes": 100,
        },
    )
...
def test_bounded_evidence_passes(tmp_path: Path) -> None:
    ...
    assert evidence_budget(
        tmp_path,
        {
            "max_total_release_evidence_bytes": 100,
            "max_single_evidence_file_bytes": 100,
        },
    )["valid"]
