...
def load_script(skill: str, script: str):
    ...
    spec = importlib.util.spec_from_file_location(f"test_{skill.replace('-', '_')}", path)
...
class CapabilityMiningSkillTests(unittest.TestCase):
    def test_security_aggregation_minimizes_payloads_and_preserves_failure(self):
        module = load_script("orchestrate-security-validation", "aggregate_security_findings.py")
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root / "scanner.json").write_text(
                json.dumps({"tool": "scanner", "status": "fail", "findings": [{"severity": "high", "location": "a.py:2", "message": "sensitive detail"}]}),
                encoding="utf-8",
            )
    ...
    def test_dynamic_discovery_flags_unleased_address(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root / "proxy.conf").write_text("proxy_pass http://10.0.0.7:8080;\n", encoding="utf-8")
            result = module.audit(root)
            self.assertEqual(len(result["findings"]), 1)
            (root / "proxy.conf").write_text("resolver 127.0.0.11 valid=10s;\nproxy_pass http://10.0.0.7:8080;\n", encoding="utf-8")
    ...
    def test_text_repair_is_deterministic_and_separate(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            mapping.write_text(json.dumps({"version": "1", "replacements": {"bad-token": "fixed"}}), encoding="utf-8")
            output, receipt = module.repair(source, mapping)
            self.assertEqual(output.decode(), "alpha fixed omega")
            self.assertEqual(source.read_text(encoding="utf-8"), "alpha bad-token omega")
            self.assertTrue(receipt["changed"])

    def test_import_boundary_audit_checks_patterns_and_mirrors(self):
        module = load_script("enforce-source-import-boundaries", "audit_import_boundaries.py")
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root / "sample.py").write_text("from private.impl import thing\n", encoding="utf-8")
            (root / "a.json").write_text("{}", encoding="utf-8")
            (root / "b.json").write_text("{\"drift\": true}", encoding="utf-8")
            policy = root / "policy.json"
            policy.write_text(json.dumps({"forbidden_import_patterns": ["^private\\."], "mirrored_contracts": [{"canonical": "a.json", "mirror": "b.json"}]}), encoding="utf-8")
    ...
    def test_incomplete_audit_uses_payload_minimized_ids(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root / "sample.py").write_text("def unfinished():\n    pass\n", encoding="utf-8")
            result = module.audit(root)
            ...
            registry.write_text(json.dumps({"records": [{**finding, "classification": "test_fixture", "rationale": "deliberate scanner fixture", "owner": "tests", "review_condition": "review when fixture changes"}]}), encoding="utf-8")
    ...
    def test_source_auditor_accounts_for_every_file(self):
        module = load_script("audit-source-capabilities", "audit_source_capabilities.py")
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root / "SKILL.md").write_text("---\nname: example\ndescription: deterministic audit skill\n---\n# Example\n", encoding="utf-8")
            result = module.audit(root, existing_catalog=None, excluded_names=set(), max_bytes=1_000_000)
    ...
    def test_source_auditor_streams_oversize_catalogs_and_accounts_for_exclusions(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            generated.mkdir()
            (generated / "derived.py").write_text("raise NotImplementedError\n", encoding="utf-8")
            result = module.audit(root, existing_catalog=None, excluded_names=set(), max_bytes=64)
            self.assertEqual(result["coverage"]["files"], 1)
            ...
            self.assertGreater(result["mechanism_counts"]["orchestration-checkpoint"], 0)
    ...
    def test_source_inventory_reconciler_requires_an_owner_for_every_record(self):
        module = load_script("audit-source-capabilities", "reconcile_source_inventory.py")
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            ...
            inventory.write_text(
                "\n".join(
                    [
                        json.dumps({"source_tree": "old", "path": "owned.txt", "sha256": owner_hash, "bytes": 7}),
                        json.dumps({"source_tree": "old", "path": "missing.txt", "sha256": "a" * 64, "bytes": 1}),
                    ]
                )
                + "\n",
                encoding="utf-8",
    ...
    def test_skill_identity_reconciler_accounts_for_specialty_alias_and_vendor(self):
        module = load_script("audit-source-capabilities", "reconcile_skill_identities.py")
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            inventory = root / "inventory.jsonl"
            ...
            source = [
                {"id": "1", "path": "a/SKILL.md", "sha256": "1" * 64, "source_tree": "first"},
                {"id": "2", "path": "b/SKILL.md", "sha256": "2" * 64, "source_tree": "first"},
                {"id": "3", "path": "c/SKILL.md", "sha256": "3" * 64, "source_tree": "reference-x"},
            ]
            ...
            specialty.write_text(json.dumps({"records": [{"id": "special", "active_semantic_mappings": ["target"], "delivery_state": "mapped"}]}), encoding="utf-8")
            aliases.write_text(json.dumps({"vendor_source_prefixes": ["reference-"], "aliases": [{"source_id": "old-name", "targets": ["target"], "disposition": "merge"}]}), encoding="utf-8")
            report = module.reconcile(inventory, structured, catalog, specialty, aliases)
    ...
    def test_classified_asset_reconciler_preserves_error_denominators(self):
        module = load_script("audit-source-capabilities", "reconcile_classified_assets.py")
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            inventory = root / "inventory.jsonl"
            ...
            inventory.write_text(json.dumps({"id": "a", "content_kind": "text"}) + "\n", encoding="utf-8")
            classified.write_text(json.dumps({"id": "a", "class": "builder", "source_tree": "source", "path": "build.py", "sha256": "a" * 64, "probable_domain": "engineering"}) + "\n", encoding="utf-8")
            policy.write_text(json.dumps({"terminal_classes": {}, "capability_classes": {"builder": {"disposition": "merge", "targets": ["target"]}}, "inventory_error_classes": [{"id": "missing", "contains": "FileNotFoundError", "disposition": "broken_pointer"}], "default_inventory_error_disposition": "unresolved_error"}), encoding="utf-8")
            skills.write_text(json.dumps({"records": []}), encoding="utf-8")
            errors.write_text(json.dumps({"path": "gone", "error": "FileNotFoundError: gone", "source_tree": "source"}) + "\n", encoding="utf-8")
            report = module.reconcile(inventory, classified, policy, skills, [], [errors])
            self.assertTrue(report["summary"]["complete"])
    ...
    def test_historical_incomplete_reconciler_requires_a_boundary_rule(self):
        module = load_script("audit-incomplete-implementations", "reconcile_incomplete_findings.py")
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            ...
            report.write_text(json.dumps({"finding_count": 1, "findings": [{"id": "a", "path": "snapshot/module.py", "line": 1, "rule": "python-pass"}]}), encoding="utf-8")
            policy = root / "policy.json"
            policy.write_text(json.dumps({"rules": []}), encoding="utf-8")
            self.assertFalse(module.reconcile(report, root, policy)["summary"]["complete"])
            policy.write_text(json.dumps({"rules": [{"id": "snapshot", "path_glob": "snapshot/*", "disposition": "historical", "targets": []}]}), encoding="utf-8")
            self.assertTrue(module.reconcile(report, root, policy)["summary"]["complete"])
