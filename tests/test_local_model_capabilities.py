...
class LocalModelCapabilityTests(unittest.TestCase):
    def test_architecture_inspection_normalizes_gqa_and_matches_kv_fixture(self) -> None:
        ...
        }, {"tokens": 4096, "batch": 1, "kv_bytes_per_element": 2, "bits_per_weight": 16})
        self.assertEqual(result["normalized"]["attention_variant"], "MHA")
        self.assertEqual(result["estimates"]["kv_cache_bytes"], 2 * 1024 ** 3)
        self.assertGreater(result["estimates"]["estimated_total_parameters"], 6_000_000_000)
        self.assertFalse(result["security"]["remote_code_executed"])

    def test_architecture_inspection_rejects_hostile_or_inconsistent_dimensions(self) -> None:
        module = load_script("inspect-llm-architecture", "inspect_llm_architecture.py")
    ...
    def test_planner_emits_complete_matrix_and_forbids_private_remote_fallback(self) -> None:
        module = load_script("plan-local-model-deployment", "plan_local_model_deployment.py")
        result = module.plan({
            "hardware": {"available_ram_gib": 64, "available_vram_gib": 24, "free_disk_gib": 120, "sustained_disk_read_mib_s": 3000},
            "model": {"weight_gib": 28, "transformed_weight_gib": 16, "architecture_supported": True, "streaming_supported": True, "quantized_weight_gib": {"4bit": 8}},
            "workload": {"kv_cache_gib": 2, "privacy_required": True, "remote_fallback_allowed": True},
        })
    ...
    def test_planner_keeps_missing_measurement_visible(self) -> None:
        module = load_script("plan-local-model-deployment", "plan_local_model_deployment.py")
        result = module.plan({
            "hardware": {"available_ram_gib": 32, "available_vram_gib": 8, "free_disk_gib": 60},
            "model": {"weight_gib": 12, "architecture_supported": False, "streaming_supported": False},
            "workload": {"privacy_required": False, "remote_fallback_allowed": False},
        })
