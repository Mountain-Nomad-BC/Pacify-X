...
def record(memory_id: str = "mem-one", **updates: object) -> MemoryRecord:
    values = {
        "memory_id": memory_id, "workspace_id": "wsp", "project_id": "prj", "owner_id": "agent",
        "session_id": "session", "lease_id": "lease", "title": "Preserve project memory",
        "memory_type": "decision", "summary": "Store concise evidence backed decisions",
        "source_artifact": "decision.md", "source_sha256": "a" * 64, "evidence_locator": "E-1",
        "epistemic_status": "observation", "confidence": 0.9, "confidence_method": "direct",
        "classification": "internal", "acl": ("prj",), "observed_at": NOW, "effective_at": NOW,
    }
...
class MemoryVaultTests(unittest.TestCase):
    def test_append_writes_human_note_and_never_overwrites_revision(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            related = vault.append(record("related", relationships=("Preserve project memory",)))
    ...
    def test_lifecycle_controls_retrieval_and_correction_supersedes_old_memory(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            vault = MemoryVault(Path(directory), workspace_id="wsp", project_id="prj")
            vault.append(record())
            self.assertEqual(vault.search("evidence backed decisions", actor_id="agent"), ())
            vault.transition("mem-one", "validated", evidence=("test-1",))
            vault.transition("mem-one", "certified", evidence=("test-2",))
            self.assertEqual([item.memory_id for item in vault.search("evidence backed decisions", actor_id="agent")], ["mem-one"])
            corrected = record("mem-two", summary="Store only current corrected decisions", supersedes=("mem-one",))
            vault.append(corrected)
            self.assertEqual([item.memory_id for item in vault.retrieval_records(actor_id="agent")], ["mem-one"])
            vault.transition("mem-two", "validated", evidence=("test-3",))
            self.assertEqual([item.memory_id for item in vault.retrieval_records(actor_id="agent")], ["mem-one"])
            vault.transition("mem-two", "certified", evidence=("test-4",))
            self.assertEqual([item.memory_id for item in vault.retrieval_records(actor_id="agent")], ["mem-two"])

    def test_index_publication_is_append_only_and_orphans_are_not_authoritative(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            vault = MemoryVault(root, workspace_id="wsp", project_id="prj")
            vault.append(record())
    ...
    def test_concurrent_index_requests_publish_distinct_complete_generations(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            vault = MemoryVault(root, workspace_id="wsp", project_id="prj")
            vault.append(record())
            ...
            self.assertEqual(sorted(item.generation for item in generations), [1, 2, 3, 4])
            self.assertTrue(all((root / item.manifest_path).is_file() for item in generations))

    def test_project_scope_revision_and_bucket_capacity_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            vault = MemoryVault(root, workspace_id="wsp", project_id="prj", max_records_per_bucket=1)
    ...
    def test_parallel_appends_and_index_publication_are_serialized(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            vault = MemoryVault(root, workspace_id="wsp", project_id="prj")
            with ThreadPoolExecutor(max_workers=4) as pool:
                writes = list(pool.map(lambda index: vault.append(record(f"mem-{index}", title=f"Memory {index}")), range(8)))
            ...
            self.assertEqual(vault.reconcile_indexes()["complete_generations"], ("000001", "000002"))
