...
def _fixture(
    root: Path, *, effects=("write_workspace",), expires_minutes=10, destructive=False
):
    ...
    base = {
        ...
        "approved_adapter": "sandbox",
        "approved_environment": "test",
        "project_id": "prj_one",
        "session_id": "session_one",
        "writable_roots": [str(root / "project")],
        "network_policy": {"mode": "deny", "allow_hosts": []},
        "secret_access_policy": {"mode": "deny", "allow_refs": []},
        "destructive_operation_policy": {
            "approved": destructive,
            "approval_id": "approval-delete" if destructive else None,
        },
        "approval_identity": "operator@test",
        "issued_utc": now.isoformat(),
        "expires_utc": (now + timedelta(minutes=expires_minutes)).isoformat(),
        "nonce": "nonce-1",
        "idempotency_key": "effect-1",
        "publisher": "test",
    }
    signature = root / "grant.sig"
...
def test_restricted_admission_requires_enforced_runtime_grant() -> None:
    ...
    decision = enforce(
        request,
        PolicyDecision(True, ("write_workspace",), "approval"),
        {"id": "writer", "status": "admitted", "effects": ["write_workspace"]},
    )
    assert not decision.approved and any(
        "runtime effect grant" in reason for reason in decision.reasons
    )
...
def test_network_effect_fails_without_network_grant() -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        grant, _, signature, policy = _fixture(root, effects=("network",))
        result = validate_effect_grant(
            grant,
            signature_path=signature,
        ...
        assert (
            not result["valid"]
            and "network effect exceeds grant policy" in result["errors"]
        )
...
def test_destructive_effect_fails_without_approval() -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        grant, _, signature, policy = _fixture(root, effects=("destructive",))
        result = validate_effect_grant(
            grant,
            signature_path=signature,
            trust_policy_path=policy,
            capability_id="writer",
            requested_effects=("destructive",),
            adapter="sandbox",
            environment="test",
            project_id="prj_one",
            session_id="session_one",
        ...
        assert (
            not result["valid"]
            and "destructive effect lacks explicit approval" in result["errors"]
        )
...
def test_effect_grant_is_bound_to_project_and_session() -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        grant, _, signature, policy = _fixture(root)
        result = validate_effect_grant(
            grant,
            signature_path=signature,
            trust_policy_path=policy,
            capability_id="writer",
            requested_effects=("write_workspace",),
            adapter="sandbox",
            environment="test",
        ...
        assert (
            not result["valid"]
            and "effect grant project or session binding mismatch" in result["errors"]
        )
...
def test_expired_effect_grant_is_rejected() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert (
            not result["valid"]
            and "effect grant is expired or has invalid time bounds" in result["errors"]
        )
...
def test_effect_grant_replay_with_different_idempotency_key_is_rejected() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert (
            not result["valid"]
            and "effect grant idempotency key mismatch" in result["errors"]
        )
