...
class DeclaredCapabilityRecoveryTests(unittest.TestCase):
    def test_every_absent_declared_outcome_has_one_hash_bound_canonical_owner(self) -> None:
        payload = json.loads((ROOT / "registry/declared_capability_recovery_map.json").read_text(encoding="utf-8"))
        ...
        for item in records:
            ...
            self.assertEqual(item["owner_body_sha256"], hashlib.sha256(body.read_bytes()).hexdigest())
            if owner in {"govern-operating-kernel", "analyze-repository-intelligence", "engineer-verification-lab", "operate-memory-retrieval-observability", "secure-agent-supply-chain", "govern-runtime-protocol-deployment", "manage-revocable-certification"}:
                self.assertEqual(item["source_body_state"], "exact_authoritative_recovery")
                self.assertEqual(item["historical_validation_state"], "supplied_and_revalidated")
            contract = json.loads((ROOT / item["owner_package"]).read_text(encoding="utf-8"))
            self.assertEqual(contract["status"], "active")
            if "skill_packages" in item["owner_package"]:
                self.assertEqual(contract["id"], owner)
                if contract.get("clean_room") is False:
                    self.assertEqual(contract["provenance"]["type"], "authoritative_contract_and_safe_body_assimilation")
                else:
    ...
    def test_every_reconstructed_outcome_routes_to_its_owner_without_body_hydration(self) -> None:
        payload = json.loads((ROOT / "registry/declared_capability_recovery_map.json").read_text(encoding="utf-8"))
