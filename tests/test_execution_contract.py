...
class ExecutionContractTests(unittest.TestCase):
    ...
    def test_allows_declared_read_only_request(self) -> None:
        result = enforce(
            ExecutionRequest("repo-reader", ("read_local",), 30, 2),
    ...
    def test_rejects_unadmitted_or_mismatched_capability(self) -> None:
        ...
        result = enforce(
            ExecutionRequest("repo-reader", ("read_local",), 30, 1),
            PolicyDecision(True, ("read_local",)),
            manifest,
        )
    ...
    def test_rejects_undeclared_effect(self) -> None:
        result = enforce(
            ExecutionRequest("repo-reader", ("write_workspace",), 30, 1, "k"),
            PolicyDecision(True, ("write_workspace",), "A-1"),
            self.manifest,
        )
        self.assertFalse(result.approved)
    ...
    def test_non_read_requires_approval_and_idempotency(self) -> None:
        ...
        result = enforce(
            ExecutionRequest("writer", ("write_workspace",), 30, 1),
            PolicyDecision(True, ("write_workspace",)),
            manifest,
        )
        self.assertFalse(result.approved)
        self.assertEqual(len(result.reasons), 3)
        self.assertIn(
            "non-read effects require an enforced runtime effect grant", result.reasons
        )

    def test_enforces_budgets_and_policy(self) -> None:
        result = enforce(
            ExecutionRequest("repo-reader", ("read_local",), 121, 13),
