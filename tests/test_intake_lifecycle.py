    ...
    def test_intake_is_append_only_and_requires_two_stable_snapshots(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "two recorded snapshots"):
                close_intake(source, state, source_alias="staged", approved_by="user", minimum_stability_seconds=0)
            record_snapshot(source, state, source_alias="staged")
            close_intake(source, state, source_alias="staged", approved_by="user", minimum_stability_seconds=0)
            self.assertEqual(intake_status(state)["status"], "closed")
            self.assertEqual(require_closed_stable(source, state, source_alias="staged")["file_count"], 1)
            self.assertEqual([event["event"] for event in load_events(state)], ["open", "snapshot", "snapshot", "close"])
