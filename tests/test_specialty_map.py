...
class SpecialtyMapTests(unittest.TestCase):
    def test_every_source_candidate_has_one_explicit_lifecycle_state(self) -> None:
        queue = json.loads(
            (ROOT / "registry/admission_queue.json").read_text(encoding="utf-8")
        )
        mapped = json.loads(
            (ROOT / "registry/specialty_map.json").read_text(encoding="utf-8")
        )
        ...
        self.assertTrue(
            all(item["state"] in {"active", "mapped_deferred"} for item in entries)
        )
    ...
    def test_active_candidate_states_match_canonical_skill_catalog(self) -> None:
        ...
        self.assertEqual(
            set(mapped["framework_only_active"]), active_ids - candidate_ids
        )
        ...
        self.assertEqual(
            mapped["deferred_candidate_count"], len(candidate_ids - mapped_active)
        )
