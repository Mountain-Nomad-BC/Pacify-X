...
class Rel009ReleaseTests(unittest.TestCase):
    ...
    def test_rel009_certificate_is_historical_and_explicitly_revoked(self) -> None:
        ...
        self.assertEqual(
            certificate["gates"]["exact_tools"]["directly_loaded"],
            certificate["gates"]["exact_tools"]["admitted_tools"],
        )
        self.assertEqual(
            certificate["gates"]["exact_tools"]["passed_tools"],
            certificate["gates"]["exact_tools"]["admitted_tools"],
        )
        self.assertTrue(certificate["gates"]["python_surfaces"]["map_current"])
        revocation = json.loads(
            (ROOT / "evidence/release-revocation-0.6.1.json").read_text(
                encoding="utf-8"
            )
        )
