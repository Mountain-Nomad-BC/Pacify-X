...
def successful_result(task_id: str = "task-1") -> CapabilityResult:
    return CapabilityResult(
        ...
        evidence=(EvidenceRecord("evidence-1", task_id, EvidenceKind.TEST, "tests/result.json", NOW),),
...
class OrchestratorTests(unittest.TestCase):
    def test_runs_authorized_handler_and_verifies_result(self) -> None:
        ...
        orchestrator = Orchestrator(ROOT, lambda _: lambda context: successful_result(context.request.task_id), checkpoint_sink=sink, now=lambda: NOW)
    ...
    def test_handler_is_not_resolved_before_policy_authorization(self) -> None:
        ...
        orchestrator = Orchestrator(ROOT, lambda capability_id: resolutions.append(capability_id), now=lambda: NOW)
    ...
    def test_missing_inputs_blocks_before_activation(self) -> None:
        resolutions: list[str] = []
        orchestrator = Orchestrator(ROOT, lambda capability_id: resolutions.append(capability_id), now=lambda: NOW)
        result = orchestrator.run(request(inputs={}), PolicyDecision(True, ("read_local",)))
        self.assertEqual(result.status, "blocked")
        self.assertIn("missing inputs", result.errors[0])
        self.assertEqual(resolutions, [])

    def test_unregistered_handler_fails_closed(self) -> None:
        result = Orchestrator(ROOT, lambda _: None, now=lambda: NOW).run(request(), PolicyDecision(True, ("read_local",)))
        self.assertEqual(result.status, "blocked")
    ...
    def test_failed_postcondition_is_not_complete(self) -> None:
        ...
        bad = CapabilityResult(bad.claims, bad.evidence, bad.links, {"result": False}, True)
        result = Orchestrator(ROOT, lambda _: lambda __: bad, now=lambda: NOW).run(request(), PolicyDecision(True, ("read_local",)))
    ...
    def test_handler_exception_becomes_structured_failure_and_unloads(self) -> None:
        ...
        result = Orchestrator(ROOT, lambda _: fail, now=lambda: NOW).run(request(), PolicyDecision(True, ("read_local",)))
    ...
    def test_retry_requires_new_evidence_and_respects_budget(self) -> None:
        previous = FailureRecord("task-1", "evidence-assembler", "abc", 1, ("old",), "failure")
        denied = decide_retry(previous, candidate_attempt=2, evidence_ids=("old",), max_retries=1, require_new_evidence=True)
        allowed = decide_retry(previous, candidate_attempt=2, evidence_ids=("old", "new"), max_retries=1, require_new_evidence=True)
        exhausted = decide_retry(previous, candidate_attempt=3, evidence_ids=("new",), max_retries=1, require_new_evidence=True)
