...
class ProjectBriefTests(unittest.TestCase):
    def test_brief_updates_existing_commissioned_state_with_history(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            questionnaire = Path(directory) / "answers.json"; questionnaire.write_text(json.dumps(answers()), encoding="utf-8")
            preview = apply_project_brief(project, questionnaire, source_root=ROOT)
            result = apply_project_brief(project, questionnaire, source_root=ROOT, apply=True)
            state = json.loads((project / ".engineering-bootstrap/project-management/state.json").read_text(encoding="utf-8"))
            history_count = len(list((project / ".engineering-bootstrap/project-management/history").glob("state-*")))
