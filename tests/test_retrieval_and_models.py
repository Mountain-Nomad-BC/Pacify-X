...
class RetrievalAndModelTests(unittest.TestCase):
    ...
    def test_model_routing_policy_is_lazy_loadable_and_fail_closed(self) -> None:
        ...
        self.assertEqual(
            rank_models(("coding",), (model,), policy=policy)[0].model_id, "local"
        )

    def test_retrieval_uses_identity_scope_filters_sources_and_emits_lineage(
        self,
    ) -> None:
        sources = (
            RetrievalSource(
        ...
        result = retrieve(
            ...
            sources,
            identity_scope=("reader",),
            client_claimed_role="admin",
        )
    ...
    def test_local_runtime_discovery_is_bounded_and_does_not_load_models(self) -> None:
        calls = []
        found = discover_local_runtimes(
            ("z", "a", "b", "c"),
            resolver=lambda name: calls.append(name)
            or (f"/{name}" if name == "a" else None),
            max_runtimes=2,
        )
        self.assertEqual(calls, ["a", "b"])
        self.assertEqual(found, (("a", "/a"),))

    def test_model_routing_is_capability_based_sensitive_and_explicit_on_fallback(
        self,
    ) -> None:
        models = (
            ModelCapability(
