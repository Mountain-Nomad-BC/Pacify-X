...
from runtime.planner import Requirement, build_work_package, detect_scope_drift, load_work_package, save_work_package
...
class RuntimeWave4Tests(unittest.TestCase):
    def test_bounded_startup_loads_metadata_and_gracefully_marks_missing_tools(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            project = Path(directory)
            snapshot = bounded_startup(ROOT, project, tool_names=("available", "missing"), tool_resolver=lambda name: "/tool" if name == "available" else None, max_probe_workers=2)
            self.assertEqual(snapshot.hydrated_skill_bodies, ())
            self.assertEqual(dict(snapshot.tools), {"available": "/tool", "missing": None})
            self.assertLessEqual(len(snapshot.capabilities), snapshot.config.budget.max_initial_registry_records)
            self.assertEqual(snapshot.project_profile["status"], "unconfigured")

    def test_classifier_is_multi_domain_explainable_and_low_confidence_is_bounded(self) -> None:
    ...
    def test_selector_enforces_budget_dependencies_redundancy_and_is_deterministic(self) -> None:
        index = (
            CapabilitySummary("base", "support workflow", aliases=("workflow",), risk="R0"),
            CapabilitySummary("primary", "build workflow", aliases=("build workflow",), dependencies=("base",), redundancy_group="builders"),
            CapabilitySummary("duplicate", "build workflow", aliases=("build workflow",), redundancy_group="builders", cost=4),
    ...
    def test_ten_thousand_descriptors_remain_unloaded_until_selected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            descriptors = tuple(SkillDescriptor(f"skill-{index}", "body.md") for index in range(9999)) + (
    ...
    def test_deferred_skill_body_cannot_be_hydrated_before_admission(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            loader = LazySkillLoader(root, (SkillDescriptor("candidate", "body.md", status="mapped_deferred"),))
            with self.assertRaisesRegex(PermissionError, "not admitted"):
                loader.hydrate("candidate")

    def test_frozen_plan_maps_every_requirement_persists_and_detects_drift(self) -> None:
        requirements = (
            Requirement("r1", "inspect repository", "repository-reader", "existing"),
            Requirement("r2", "unknown adapter", None, "blocked"),
        )
        package = build_work_package("onboard", requirements, deferred=("non-blocking idea",))
        ...
        self.assertTrue(detect_scope_drift(package, requirements + (Requirement("r3", "new", "builder"),)))

    def test_scheduler_serializes_heavy_lane_and_detects_pressure_and_lost_worker(self) -> None:
        scheduler = ResourceScheduler(ResourcePolicy(max_agents=2, max_light_lanes=2, max_heavy_lanes=1, lost_worker_seconds=10))
        self.assertTrue(scheduler.admit("one", "heavy", {"agents": 0, "memory_percent": 20}).admitted)
        self.assertFalse(scheduler.admit("two", "heavy", {"agents": 0, "memory_percent": 20}).admitted)
        self.assertFalse(scheduler.admit("memory", "light", {"agents": 0, "memory_percent": 99}).admitted)
        now = datetime.now(timezone.utc)
        scheduler.heartbeat("worker", now - timedelta(seconds=20))
        self.assertEqual(scheduler.lost_workers(now), ("worker",))

    def test_recovery_is_idempotent_and_interrupted_steps_are_non_certifying(self) -> None:
        state = DurableState("pkg", ("inspect",), (("reader", "1.0"),), ("approval-1",), ("evidence-1",), ("write-1",), ("test",))
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "state.json"
            persist_state(state, path)
            loaded = load_state(path)
            persist_state(state, path)
            self.assertEqual(len(list((path.parent / ".history" / path.name).glob("*.json"))), 1)
        self.assertEqual(loaded, state)
        allowed, reasons = reconcile_resume(state, actual_evidence=("evidence-1",), requested_idempotency_key="write-1")
