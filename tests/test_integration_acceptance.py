...
class IntegrationAcceptanceTests(unittest.TestCase):
    def test_working_set_never_leaves_orphan_dependencies_after_bundle_rejection(
        self,
    ) -> None:
        index = (
            CapabilitySummary(
                "best", "exact goal", aliases=("exact goal",), status="active"
            ),
            CapabilitySummary(
                "parent", "goal", dependencies=("dep-a", "dep-b"), status="active"
            ),
        ...
        self.assertIn(
            ("parent", "dependency bundle does not fit: dep-b"), selected.rejected
        )
    ...
    def test_linear_branching_optional_cycle_privilege_adapter_and_resume_paths(
        self,
    ) -> None:
        contracts = (
        ...
        valid = {
            ...
            "steps": [
                ...
                },
                {
                    ...
                    "capability": "check",
                    "depends_on": ["read"],
                    "optional_depends_on": ["telemetry"],
                    "effects": ["read_local"],
                },
            ],
            "parallelism": {"max_agents": 2, "max_test_runners": 1},
        ...
        self.assertTrue(
            any("cycle" in error for error in validate_orchestration(cycle, contracts))
        )
        ...
        interrupted = DurableState(
            "pkg", ("read",), (("read", "1"),), (), ("E1",), (), ("check",)
        )
    ...
    def test_two_agent_resource_policy_enforces_ownership_heavy_lane_pressure_and_recovery(
        self,
    ) -> None:
        policy = ResourcePolicy(
            max_agents=2, max_light_lanes=2, max_heavy_lanes=1, lost_worker_seconds=5
        )
        scheduler = ResourceScheduler(policy)
        safe = {
            "agents": 0,
            "memory_percent": 20,
            "wsl_memory_percent": 20,
            "docker_memory_percent": 20,
            "gpu_percent": 20,
        }
        self.assertTrue(
            scheduler.admit("primary", "heavy", safe, owned_paths=("runtime",)).admitted
        )
        self.assertFalse(
            scheduler.admit(
                "overlap", "light", safe, owned_paths=("runtime/cli.py",)
            ).admitted
        )
        self.assertFalse(
            scheduler.admit("heavy-two", "heavy", safe, owned_paths=("tests",)).admitted
        )
        self.assertTrue(
            scheduler.admit("helper", "light", safe, owned_paths=("builders",)).admitted
        )
        ...
        self.assertFalse(
            scheduler.admit(
                "pressure", "light", {**safe, "wsl_memory_percent": 99}
            ).admitted
        )

    def test_new_and_existing_project_modes_are_accepted_without_unauthorized_installation(
        self,
    ) -> None:
