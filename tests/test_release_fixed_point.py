...
def test_fixed_point_accepts_unchanged_second_pass(tmp_path: Path, monkeypatch) -> None:
    target = tmp_path / "registry/a.json"
    target.parent.mkdir()
    target.write_text("{}\n")
    monkeypatch.setattr(
        "scripts.clean_source_export._rebuild_candidate_projections", lambda root: None
    )
...
def test_fixed_point_rejects_second_pass_change(tmp_path: Path, monkeypatch) -> None:
    ...
    def rebuild(root: Path) -> None:
        calls["count"] += 1
        (root / "registry/a.json").write_text(str(calls["count"]))
