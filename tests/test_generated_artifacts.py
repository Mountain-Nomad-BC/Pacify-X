...
from scripts.build_declared_suite_template_projections import reconcile as template_reconcile
...
def test_all_generated_projections_match_one_canonical_owner():
    ...
    assert result["checks"]["commissioned_skill_registry"]["skill_count"] == 60
...
def test_one_projection_mutation_is_detected_without_rewriting(tmp_path):
    ...
    for owner in ("analyze-repository-intelligence", "engineer-verification-lab", "govern-operating-kernel", "govern-runtime-protocol-deployment", "manage-revocable-certification", "operate-memory-retrieval-observability", "secure-agent-supply-chain"):
        target = root / ".agents/skills" / owner / "scripts/domain_tool.py"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes((ROOT / ".agents/skills" / owner / "scripts/domain_tool.py").read_bytes())
    changed = root / ".agents/skills/analyze-repository-intelligence/scripts/domain_tool.py"
...
def test_generation_is_byte_stable_when_inputs_do_not_change():
    before = [path.read_bytes() for path in sorted((ROOT / "templates/declared_suite").glob("pack-*.json"))]
    assert template_reconcile(ROOT, check=True)["valid"]
    assert profile_reconcile(ROOT, check=True)["valid"]
    after = [path.read_bytes() for path in sorted((ROOT / "templates/declared_suite").glob("pack-*.json"))]
    assert before == after
