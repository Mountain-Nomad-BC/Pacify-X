...
def task(identifier: str, **overrides):
    value = {"id": identifier, "capability": identifier, "priority": 500, "state": "pending", "resources": {"cpu_cores": 1}, "acceptance": {}}
...
class CapabilitySchedulerTests(unittest.TestCase):
    def test_layer_denominators_and_lazy_contracts(self) -> None:
        ...
        self.assertEqual(result["counts"], {"capabilities": 30, "owners": 30, "workflows": 5, "policies": 3, "schemas": 8})
    ...
    def test_dependency_order_and_deterministic_replay(self) -> None:
        payload = {"now": "2026-01-01T00:00:00Z", "resources": {"cpu_cores": 2}, "workload": {"tasks": [task("b", priority=900, dependencies=["a"]), task("a", priority=100)]}}
    ...
    def test_cycle_and_missing_dependency_fail_closed(self) -> None:
        cycle = simulate_schedule({"resources": {}, "workload": {"tasks": [task("a", dependencies=["b"]), task("b", dependencies=["a"])]}})
        self.assertFalse(cycle["valid"])
        self.assertIn("dependency cycle", cycle["errors"][0])
        missing = simulate_schedule({"resources": {}, "workload": {"tasks": [task("a", dependencies=["missing"])]}})
    ...
    def test_approval_resource_privacy_budget_and_retry_gates(self) -> None:
        ...
        result = simulate_schedule({"resources": {"cpu_cores": 2, "network": "external"}, "policy": {"external_network": "deny"}, "workload": {"tasks": tasks}})
    ...
    def test_scoring_records_normalized_factors_and_preserves_acceptance(self) -> None:
        result = simulate_schedule({"now": "2026-01-01T00:00:00Z", "resources": {"cpu_cores": 1}, "workload": {"tasks": [task("a", acceptance={"required_quality": 0.9, "expected_success": 0.8})]}})
        self.assertTrue(result["valid"], result)
        ...
        self.assertEqual(result["execution_authority"], "none; dispatch events are plans only")
