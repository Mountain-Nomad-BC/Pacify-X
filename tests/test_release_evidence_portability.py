...
def test_machine_local_release_evidence_fails_preflight(tmp_path: Path) -> None:
    evidence = tmp_path / "evidence"; evidence.mkdir(); (evidence / "installed.json").write_text('{"python":"C:\\\\Python314\\\\python.exe"}')
...
def test_portable_release_evidence_passes(tmp_path: Path) -> None:
    evidence = tmp_path / "evidence"; evidence.mkdir(); (evidence / "installed.json").write_text('{"python":"[release-python]"}')
