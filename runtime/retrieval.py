...
def retrieve(
    ...
    *,
    ...
    for source in visible:
        ...
        score = 3 * len(query_terms & _terms(source.title)) + len(query_terms & haystack)
    ...
    for score, source in ranked:
        ...
        if used + size > max_context_bytes:
            continue
        hits.append(RetrievalHit(source.source_id, source.title, excerpt, f"source:{source.source_id}", source.lineage, score))
        used += size
        if len(hits) >= max_results:
            break
    trace = (
        f"strategy={strategy}", f"identity_scope_count={len(scope)}", f"visible={len(visible)}",
        f"filtered={len(filtered)}", f"client_role_authoritative=false", f"context_budget={max_context_bytes}",
    )
    mode = "read_only" if hits else "degraded_no_match"
    return RetrievalDecision(strategy, mode, tuple(hits), tuple(sorted(filtered)), used, trace)
