...
def validate_source_coverage(root: Path) -> dict[str, Any]:
    ...
    for control in record.get("controls", ()):
        ...
        if control.get("release_role") == "required" and state not in {
            "operational",
            "operational_bounded",
        }:
        ...
        for skill_id in control.get("skills", ()):
            ...
            if state in {"operational", "operational_bounded"} and status not in {
                "active",
                "admitted",
            }:
                errors.append(
                    f"{control_id}: operational control uses non-admitted skill {skill_id}"
                )
