...
def _positive_cases(f: dict[str, str], temp: Path) -> dict[str, ToolCase]:
    ...
    return {
        ...
        "claim-ledger": ToolCase((out("claims.json"), "add", "bounded claim", "--type", "observed_fact", "--confidence", "0.9", "--evidence", "fixture"), output_files=("claims.json",)),
        "durable-state": ToolCase((out("state.json"), "new", "--task", "bounded task"), output_files=("state.json",)),
        ...
        "skill-compiler": ToolCase((f["contract.yaml"], "--out", out("compiled-skill")), output_files=("compiled-skill/registry_entry.json", "compiled-skill/permission_manifest.json", "compiled-skill/TEST_SKELETON.md")),
...
def _negative_cases(f: dict[str, str], temp: Path) -> dict[str, ToolCase]:
    cases = {
        ...
        "certification-aggregator": ToolCase((f["fail-report.json"], "--out", str(temp / "failed-aggregate.json")), (1,), output_files=("failed-aggregate.json",)),
    ...
    for identifier in _positive_cases(f, temp):
        cases.setdefault(identifier, ToolCase(
            ("--pacify-x-invalid-boundary",), (1, 2, 3, 4),
            json_output=False,
        ))
    return {
        identifier: ToolCase(
            case.args, case.expected_exit_codes, case.json_output, case.output_files,
...
def _run(script: Path, case: ToolCase, temp: Path, timeout_seconds: float, *, python_executable: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "observed_filesystem_effects": [], "unexpected_filesystem_effects": [],
            "output_digests": {}, "fixture_classes": list(case.fixture_classes),
        }
    ...
    unexpected_effects = [path for path in changed if not any(path == name or path.startswith(name + "/") for name in allowed)]
    passed = process.returncode in case.expected_exit_codes and not parse_error and not missing_outputs and not unexpected_effects
    return {
        "passed": passed,
        ...
        "output_digests": {
            ...
            for name in case.output_files if (temp / name).is_file()
        },
...
def certify_exact_tools(
    ...
    *,
        ...
        for item in admitted:
            ...
            if allow_cache and cache_path is not None and cache_path.is_file():
                try:
                    ...
                    if candidate.get("cache_key") == key and _sealed(candidate) and candidate.get("result", {}).get("passed") is True and "repeat_behavior" in candidate.get("result", {}):
            ...
            if cached is not None:
                ...
                repeat = _run(script, repeat_positive[identifier], repeat_temp, timeout_seconds, python_executable=python_executable)
            ...
            deterministic = bool(
                repeat["passed"]
                and repeat["normalized_stdout_sha256"] == behavior["normalized_stdout_sha256"]
            ...
            results.append({
                ...
                "fixture_classes": {
                    "positive": behavior["passed"], "malformed_input": denial is not None and denial["passed"],
                    ...
                    "unauthorized_effect": not behavior["unexpected_filesystem_effects"] and (denial is None or not denial["unexpected_filesystem_effects"]),
                    "deterministic_repeat": deterministic, "timeout_bound": not behavior["timed_out"],
                    "output_contract": not behavior["parse_error"] and not behavior["missing_outputs"],
                },
                "certification_strength": "negative-path-certified" if denial is not None and denial["passed"] and deterministic else "contract-certified",
                "cache_key": key,
                "cache_hit": cached is not None,
            })
            tool_passed = load_result["passed"] and behavior["passed"] and denial is not None and denial["passed"] and deterministic
            if cache_path is not None and cached is None and tool_passed:
                cache_path.parent.mkdir(parents=True, exist_ok=True)
                _dump(cache_path, _seal({"schema_version": "1.0", "cache_key": key, "result": {"passed": True, "direct_load": load_result, "positive_behavior": behavior, "negative_behavior": denial, "repeat_behavior": repeat}}))
            publish("passed" if tool_passed else "failed", identifier, duration_seconds=round(sum(float(x.get("duration_seconds", 0)) for x in (load_result, behavior, denial or {}, repeat)), 6), cache_hit=cached is not None)
    ...
    passed_tools = sum(
        ...
        and item["negative_behavior"] is not None and item["negative_behavior"]["passed"]
    ...
    final = {
        ...
        "deterministic_repeat_cases": sum(1 for item in results if item["deterministic_repeat"]),
