...
def _intermediate_records(paths: list[Path], root: Path) -> list[dict[str, object]]:
    ...
    for base in sorted(paths, key=lambda item: item.name.casefold()):
        ...
        candidates = [base] if base.is_file() else sorted(base.rglob("*"), key=lambda item: item.as_posix().casefold())
        for path in candidates:
            if path.is_symlink():
                raise ValueError(f"build intermediate contains a symbolic link: {path.relative_to(root).as_posix()}")
            if path.is_file():
                records.append({
...
def quarantine_build_intermediates(root: Path, custody_root: Path) -> dict[str, Any]:
    ...
    candidates.extend(sorted(root.glob("*.egg-info"), key=lambda item: item.name.casefold()))
    errors: list[str] = []
    try:
        before = _intermediate_records(candidates, root)
    except (OSError, ValueError) as error:
        return {"valid": False, "moved": [], "records": [], "hard_delete": False, "errors": [str(error)]}
    moved: list[str] = []
    ...
    for source in candidates:
        ...
        try:
            ...
            errors.append(f"could not quarantine build intermediate {source.name}: {error}")
    ...
    errors.extend(f"build intermediate remains in product tree: {name}" for name in sorted(set(remaining)))
    receipt = {
        "schema_version": "1.0",
        "valid": not errors,
        "moved": moved,
...
def _artifact_class(source_path: str) -> tuple[str, str]:
    ...
    if top == ".agents":
        return ("skill-tool" if "/scripts/" in f"/{source_path}" else "skill-resource", "skill-orchestration")
    return {
...
def _record(root: Path, source: Path, installed_path: str, target: str) -> dict[str, Any]:
...
def generate_artifact_manifest(root: Path) -> dict[str, Any]:
    ...
    package_dirs = {str(key): str(value) for key, value in setuptools.get("package-dir", {}).items()}
    for package in setuptools.get("packages", ()):
        candidates = [key for key in package_dirs if package == key or package.startswith(key + ".")]
        if not candidates:
        ...
        if not source_dir.is_dir():
            errors.append(f"package source directory is missing: {source_dir.relative_to(root).as_posix()}")
            continue
        for source in sorted(source_dir.glob("*.py"), key=lambda item: item.name.casefold()):
    ...
    for target, patterns in setuptools.get("data-files", {}).items():
        for pattern in patterns:
            matches = sorted(
                (path for path in root.glob(str(pattern)) if path.is_file() and not path.is_symlink()),
    ...
    for declared in project.get("license-files", ()):
        ...
        if not source.is_file() or source.is_symlink():
            ...
            continue
        records.append(_record(root, source, f"{dist_info}/licenses/{source.name}", "wheel"))
        wheel_sources.add(source.resolve(strict=True))
    ...
    for source in sorted(sdist_sources, key=lambda item: item.relative_to(root).as_posix().casefold()):
    ...
    for record in records:
        ...
        if prior is not None and prior != record["source_path"]:
            errors.append(f"artifact projection collision at {key[1]}: {prior} and {record['source_path']}")
        collisions[key] = record["source_path"]
    records.sort(key=lambda item: (item["package_target"], item["installed_path"].casefold(), item["source_path"]))
    payload = {
        "schema_version": "1.0",
    ...
    canonical = json.dumps({key: value for key, value in payload.items() if key != "errors"}, sort_keys=True, separators=(",", ":")).encode()
    return {**payload, "valid": not errors, "manifest_sha256": hashlib.sha256(canonical).hexdigest()}
...
def verify_declared_projection_duplicates(
    ...
    for source, records in grouped.items():
        ...
        for record in records:
            ...
            archived = entries_by_target.get(target, {}).get(str(record["installed_path"]))
            if archived is not None and archived.get("sha256") != record.get("source_sha256"):
                errors.append(f"declared projection hash mismatch: {target}:{record['installed_path']}")
    return {"valid": not errors, "duplicate_source_count": duplicate_count, "errors": errors}
...
def verify_built_artifact(
    ...
    *,
    ...
    inspected = inspect_wheel(artifact) if package_target == "wheel" else inspect_sdist(artifact)
    ...
    generated = tuple(str(pattern) for pattern in manifest.get("allowed_generated", {}).get(package_target, ()))
...
def inspect_wheel(path: Path) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        for info in sorted(archive.infolist(), key=lambda item: item.filename.casefold()):
            ...
            entries.append({"path": info.filename, "size_bytes": len(data), "sha256": hashlib.sha256(data).hexdigest()})
            if info.filename.endswith(".dist-info/METADATA"):
                version = _metadata_version(data.decode("utf-8", errors="strict"))
    if version is None:
        errors.append("wheel metadata version is missing")
    return {"valid": not errors, "version": version, "entries": entries, "errors": errors}
    ...
    with tarfile.open(path, mode="r:gz") as archive:
        ...
        for member in sorted(members, key=lambda item: item.name.casefold()):
            ...
            if stream is None:
                ...
                continue
            data = stream.read()
            entries.append({"path": member.name, "size_bytes": len(data), "sha256": hashlib.sha256(data).hexdigest()})
...
def build_release_artifacts_once(
    ...
    *,
    ...
    command = [python_executable, "-m", "build", "--no-isolation", "--wheel", "--sdist", "--outdir", str(output_dir)]
    process = runner(command, cwd=root, env=dict(environment), text=True, capture_output=True, timeout=300)
    errors: list[str] = []
    if process.returncode:
        errors.append(f"release build failed with exit code {process.returncode}: {(process.stderr or '')[-2000:]}")
    wheels = sorted(output_dir.glob("*.whl"))
    sdists = sorted(output_dir.glob("*.tar.gz"))
    if len(wheels) != 1 or len(sdists) != 1:
        errors.append(f"release build produced wheels={len(wheels)} sdists={len(sdists)}")
    artifacts = ([file_record(wheels[0], "wheel")] if len(wheels) == 1 else []) + ([file_record(sdists[0], "sdist")] if len(sdists) == 1 else [])
    ...
    if (root / "pyproject.toml").is_file() and (root / "MANIFEST.in").is_file():
        ...
        if len(wheels) == 1:
            verification["wheel"] = verify_built_artifact(wheels[0], manifest, package_target="wheel")
            errors.extend(verification["wheel"]["errors"])
        if len(sdists) == 1:
            verification["sdist"] = verify_built_artifact(sdists[0], manifest, package_target="sdist")
    ...
    intermediates_exist = (root / "build").exists() or (root / "dist").exists() or any(root.glob("*.egg-info"))
    if intermediates_exist:
        if intermediate_quarantine is None:
            errors.append("release build left source-tree intermediates without a quarantine target")
        else:
            intermediate_custody = quarantine_build_intermediates(root, intermediate_quarantine)
    ...
    return {
        ...
        "artifact_manifest_sha256": manifest.get("manifest_sha256") if manifest else None,
...
def verify_artifact_records(directory: Path, records: list[Mapping[str, object]]) -> dict[str, Any]:
    ...
    for record in records:
        ...
        if any(actual[key] != record.get(key) for key in ("filename", "type", "sha256", "size_bytes")):
...
def install_exact_wheel(
    ...
    *,
    ...
    if actual != expected_sha256:
        return {"valid": False, "installed_wheel_sha256": None, "errors": ["wheel changed before installation"]}
    ...
    errors = []
    if process.returncode:
        errors.append(f"exact wheel installation failed: {(process.stderr or '')[-2000:]}")
    if after != expected_sha256:
        errors.append("wheel changed during installation")
    return {
        "valid": not errors, "python_executable": str(python),
        "installed_wheel_sha256": expected_sha256 if not errors else None,
        "wheel_filename": wheel.name, "errors": errors,
    }
...
def bind_artifact_set(
    ...
    *,
    ...
    if wheel_record is None or sdist_record is None:
        ...
        return {"valid": False, "source_product_digest": source_product_digest, "errors": errors}
    ...
    manifest = generate_artifact_manifest(source_root) if source_root is not None else None
    ...
    if manifest is not None:
        ...
        artifact_checks = {"wheel": wheel_check, "sdist": sdist_check, "projections": projection_check}
        errors.extend([*wheel_check["errors"], *sdist_check["errors"], *projection_check["errors"]])
    return {
        "valid": not errors, "version": version, "source_product_digest": source_product_digest,
        "wheel_manifest_sha256": hashlib.sha256(json.dumps(wheel["entries"], sort_keys=True, separators=(",", ":")).encode()).hexdigest(),
        "sdist_manifest_sha256": hashlib.sha256(json.dumps(sdist["entries"], sort_keys=True, separators=(",", ":")).encode()).hexdigest(),
        "artifact_manifest_sha256": manifest.get("manifest_sha256") if manifest else None,
        "artifact_checks": artifact_checks,
        "wheel": wheel, "sdist": sdist, "errors": errors,
    }
