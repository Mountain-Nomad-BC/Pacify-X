...
def validate_evidence_portability(root: Path) -> dict[str, Any]:
    ...
    registry = json.loads((root / "registry/historical_external_references.json").read_text(encoding="utf-8"))
    errors = []
    if registry.get("records") != actual or registry.get("reference_count") != len(actual):
    ...
    return {"schema_version": "1.0", "valid": not errors, "reference_count": len(actual), "errors": errors, "records": actual}
