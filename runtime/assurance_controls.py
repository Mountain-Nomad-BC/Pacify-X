...
def discover_environment(payload: Mapping[str, object]) -> AssuranceResult:
    observed = {
        str(key): dict(value)
        for key, value in dict(payload.get("observed_tools", {})).items()
    }
    required = set(map(str, payload.get("required_tools", ())))
    missing = tuple(
        sorted(name for name in required if not observed.get(name, {}).get("available"))
    )
    ...
    approval_required = tuple(
        sorted(
            ...
            if str(item.get("effect")) in MUTATING_EFFECTS
        )
    )
    ...
    todo_records = tuple(
        sorted(str(item) for item in payload.get("todo_stub_findings", ()))
    )
    ...
    return AssuranceResult(
        ...
        reasons,
        {
            ...
            "credential_guidance": "use_os_or_ephemeral_secret_store"
            if credential_warning
            else "approved_or_not_requested",
...
def commission_project(payload: Mapping[str, object]) -> AssuranceResult:
    ...
    invalid_facts = tuple(
        sorted(
            str(item.get("id", "unnamed")) for item in facts if not item.get("evidence")
        )
    )
    ...
    high_risk = tuple(
        sorted(
            str(item.get("id", "unnamed"))
            for item in (*assumptions, *unknowns)
            if str(item.get("risk", "low")) in {"high", "critical"}
    ...
    return AssuranceResult(
        "commission-evidence-first-project",
        "review_ready" if not reasons else "blocked",
        reasons,
        {
            ...
            "implementation_authorized": not high_risk
            and not missing
            and bool(payload.get("human_acceptance")),
            "brief_hash": _stable(
                {
                    "answers": answers,
                    "facts": facts,
...
def supervise_action(payload: Mapping[str, object]) -> AssuranceResult:
    ...
    outside = (
        tuple(sorted(path for path in paths if not _path_within(path, owned)))
        if paths
        else ()
    )
    if outside:
    ...
    exceeded = tuple(
        sorted(
            ...
            if key in limits and float(value) > float(limits[key])
        )
    )
    if exceeded:
    ...
    decision = (
        "allow"
        if not reasons
        else ("escalate" if reasons == ["approval_missing"] else "block")
    )
    return AssuranceResult(
        "supervise-contained-execution",
        decision,
...
def quarantine_tool(payload: Mapping[str, object]) -> AssuranceResult:
    ...
    return AssuranceResult(
        ...
        tuple(reasons),
        {
            "execution": "allowed_after_admission"
            if decision == "admit"
            else "blocked",
            "integration": "allowed_after_admission"
            if decision == "admit"
            else "blocked",
            "missing_static_fields": missing,
            "vulnerability_count": len(vulnerabilities),
            "malicious_indicator_count": len(malicious),
            "dynamic_phase_separately_approved": dynamic_requested
            and "dynamic_analysis_not_contained" not in reasons,
...
def certify_skeptically(payload: Mapping[str, object]) -> AssuranceResult:
    ...
    reasons = tuple(
        ...
        for name, active in (
            ...
            (
                "coverage_denominator_incomplete",
                denominator <= 0 or covered != denominator,
            ),
    ...
    return AssuranceResult(
        "certify-skeptical-engineering",
        "certified" if not reasons else "not_certified",
        reasons,
        {
            "level": level,
            ...
            "claim": "current evidence survived approved invalidation attempts"
            if not reasons
            else "certification incomplete",
        },
    )
...
def audit_runtime(payload: Mapping[str, object]) -> AssuranceResult:
    ...
    prohibited = tuple(
        sorted(
            ...
            if key.casefold()
            in {"raw_prompt", "raw_response", "secret", "token", "credential"}
        )
    )
    required = (
        "runtime_id",
    ...
    reasons = tuple(
        ...
        for name, active in (
            ...
            (
                ...
                retention < 0 or retention > int(payload.get("max_retention_days", 30)),
            ),
            ("benchmark_not_passed", telemetry.get("benchmark") != "passed"),
            (
                "drift_detected",
                telemetry.get("drift") not in {"none", "within_threshold"},
            ),
        )
        if active
    )
    safe_telemetry = {
        key: value for key, value in telemetry.items() if key not in prohibited
    }
    passport = {
    ...
    return AssuranceResult(
        "audit-ai-runtime-assurance",
        "certified" if not reasons else "degraded",
        reasons,
        {
            "collected": True,
            "passport": passport,
...
def propose_change_intelligence(payload: Mapping[str, object]) -> AssuranceResult:
    ...
    return AssuranceResult(
        "propose-change-intelligence",
        "candidate" if not missing else "incomplete",
        (() if not missing else ("candidate_contract_incomplete",)),
        {
            "missing_fields": missing,
...
def run_assurance_control(
    control_id: str, payload: Mapping[str, object]
) -> AssuranceResult:
    handlers = {
