...
def decayed_trust(initial: float, age_days: float, half_life_days: float, validation_boost: float = 0.0) -> float:
    if half_life_days <= 0:
...
def simulate(state: dict[str, Any]) -> dict[str, Any]:
    ...
    for subject, group in sorted(by_subject.items()):
        ...
        volatility = sum(abs(float(r.get("recent_delta", 0.0))) for r in group) / max(1, len(group))
        validation_age = max((r["age_days"] for r in group), default=0.0)
        stability = clamp(1.0 - 0.55 * pressure - 0.25 * volatility - 0.20 * min(1.0, validation_age / 365.0))
        subjects.append({
            "subject": subject,
            ...
            "stability": stability,
            "revalidation_priority": clamp(0.45 * pressure + 0.25 * volatility + 0.20 * (1-stability) + 0.10 * min(1.0, validation_age/365)),
        })
    relations = state.get("relations", [])
    coexecution = defaultdict(float)
    for relation in relations:
        if relation.get("type") == "coexecuted":
            pair = tuple(sorted([str(relation.get("source")), str(relation.get("target"))]))
