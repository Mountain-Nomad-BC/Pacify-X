...
ALLOWED_EVENT_TYPES = {
    "input_received", "skill_selected", "orchestration_selected", "tool_requested",
    "tool_result", "evidence_added", "assumption_added", "hypothesis_updated",
    "memory_read", "memory_written", "decision", "validation", "rollback", "output"
}
...
def validate_events(events: list[dict[str, Any]]) -> list[str]:
    ...
    for index, event in enumerate(events):
        ...
        if "private_chain_of_thought" in event:
            errors.append(f"event {event_id or index} improperly contains private chain-of-thought")
    return errors

def reconstruct(trace: dict[str, Any]) -> dict[str, Any]:
    events = list(trace.get("events", []))
    errors = validate_events(events)
    ordered = sorted(events, key=lambda x: (str(x.get("timestamp", "")), str(x.get("event_id", ""))))
    counts = Counter(str(e.get("event_type")) for e in ordered)
    skills = [e.get("capability_id") for e in ordered if e.get("event_type") == "skill_selected"]
    orchestrations = [e.get("capability_id") for e in ordered if e.get("event_type") == "orchestration_selected"]
    assumptions = [e.get("artifact_id") for e in ordered if e.get("event_type") == "assumption_added"]
    evidence = [e.get("artifact_id") for e in ordered if e.get("event_type") == "evidence_added"]
    memories = [e.get("artifact_id") for e in ordered if e.get("event_type") == "memory_read"]
    decisions = [
        {
        ...
        for e in ordered if e.get("event_type") == "decision"
    ]
    orphan_decisions = [
    ...
    validation_results = [
        e for e in ordered if e.get("event_type") == "validation"
    ]
    return {
...
def compare_alternatives(trace: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "unexplored_decision_count": sum(1 for d in result["decisions"] if not d.get("alternatives")),
    }
