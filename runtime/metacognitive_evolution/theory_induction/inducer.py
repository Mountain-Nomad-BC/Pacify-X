...
def _signature(record: dict[str, Any]) -> set[str]:
    fields = ("mechanisms", "formulas", "inputs", "outputs", "failure_modes", "invariants")
    return {str(item).strip().lower() for field in fields for item in record.get(field, []) if str(item).strip()}
...
def induce(records: list[dict[str, Any]], threshold: float = 0.45) -> dict[str, Any]:
    ...
    for left, right in combinations(records, 2):
        ...
        similarities.append({"left": left["id"], "right": right["id"], "similarity": round(score, 6)})
        if score >= threshold:
    ...
    return {
        "threshold": threshold,
        "clusters": clusters,
        "pairwise_similarities": sorted(similarities, key=lambda x: (-x["similarity"], x["left"], x["right"])),
        "proposal_count": len(clusters),
        "warning": "Clusters are hypotheses, not automatically admitted theories.",
    }

def validate_proposal(proposal: dict[str, Any], held_out_cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "admit": total > 0 and rate >= minimum and bool(proposal.get("distinctness_evidence")),
    }
