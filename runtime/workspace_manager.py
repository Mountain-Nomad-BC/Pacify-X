...
from .project_stream_controls import ContextObject, ScopeEnvelope, SwitchEvidence, TransferPackage, authorize_context
...
def _validate_control(value: object, schema_name: str) -> None:
    validate_instance(value, framework_root() / "contracts" / "project_stream" / schema_name)
...
def _snapshot_and_replace(paths: WorkspacePaths, path: Path, value: Mapping[str, object]) -> None:
    ...
    if path.is_file():
        ...
        archive = paths.history / relative / f"{_now().replace(':', '-')}-{_sha(path)[:16]}.json"
...
def _replace_text_control(paths: WorkspacePaths, path: Path, text: str) -> None:
    if path.is_file():
        relative = path.relative_to(paths.tracking).as_posix().replace("/", "--")
        archive = paths.history / relative / f"{_now().replace(':', '-')}-{_sha(path)[:16]}.txt"
        _write_new(archive, path.read_bytes())
    payload = text.encode("utf-8")
    temporary = path.with_name(path.name + f".next-{hashlib.sha256(payload).hexdigest()[:12]}")
    _write_new(temporary, payload)
    os.replace(temporary, path)
...
def _replace_registry(paths: WorkspacePaths, value: Mapping[str, object]) -> None:
    ...
    _replace_text_control(paths, paths.tracking / REGISTRY_SEAL_NAME, _sha(paths.registry) + "\n")
...
def _load_config(paths: WorkspacePaths) -> dict[str, object]:
    ...
    expected = {
        "projects": "projects", "tracking": "projects_tracking",
        "quarantine": "repo_quarantine", "shared_capabilities": "shared_capabilities",
    }
...
def _load_registry(paths: WorkspacePaths) -> dict[str, object]:
    ...
    if not seal.is_file() or seal.read_text(encoding="utf-8").strip() != _sha(paths.registry):
        raise ValueError("workspace registry projection integrity seal is missing or mismatched; run workspace rebuild --apply")
...
def _project_by_id(registry: Mapping[str, object], project_id: str) -> dict[str, object]:
    matches = [item for item in registry.get("projects", []) if isinstance(item, dict) and item.get("project_id") == project_id]
    if len(matches) != 1:
...
def _dashboard(paths: WorkspacePaths, registry: Mapping[str, object], active_sessions: Iterable[Mapping[str, object]]) -> str:
    ...
    lines = [
        ...
        "## Project drop location", "", "`projects/`", "",
    ...
    for item in registry.get("projects", []):
        lines.append(f"| `{item['project_id']}` | `{item['state']}` | `{item['path']}` | `{item['memory_namespace']}` |")
    if not registry.get("projects"):
        lines.append("| _none_ |  |  |  |")
    lines.extend(["", "## Active session leases", "", "| Session | Agent | Project | Expires |", "|---|---|---|---|"])
    for active in active_values:
        lines.append(f"| `{active['scope']['session_id']}` | `{active['scope']['agent_id']}` | `{active['project_id']}` | `{active['expires_utc']}` |")
...
def _refresh_dashboard(paths: WorkspacePaths, registry: Mapping[str, object]) -> None:
    ...
    if paths.dashboard.is_file():
        ...
        archive = paths.history / "PROJECT_MANAGEMENT.md" / f"{_now().replace(':', '-')}-{hashlib.sha256(old).hexdigest()[:16]}.md"
        _write_new(archive, old)
        temporary = paths.dashboard.with_name(paths.dashboard.name + f".next-{hashlib.sha256(data).hexdigest()[:12]}")
...
def _bind_project_management(paths: WorkspacePaths, project: Path, binding: Mapping[str, object]) -> None:
    ...
    state_archive = paths.history / "projects" / project_id / "project-management-state" / f"{_now().replace(':', '-')}-{hashlib.sha256(old_state).hexdigest()[:16]}.json"
    ...
    if marker not in dashboard.read_text(encoding="utf-8"):
        ...
        dashboard_archive = paths.history / "projects" / project_id / "PROJECT_MANAGEMENT" / f"{_now().replace(':', '-')}-{hashlib.sha256(old_dashboard).hexdigest()[:16]}.md"
        ...
        temporary_dashboard = dashboard.with_name(dashboard.name + f".next-{hashlib.sha256(old_dashboard + addition).hexdigest()[:12]}")
        _write_new(temporary_dashboard, old_dashboard + addition)
        os.replace(temporary_dashboard, dashboard)


@_locked_when_apply
def initialize_workspace(root: Path, *, workspace_id: str | None = None, apply: bool = False) -> dict[str, object]:
    paths = workspace_paths(root)
    ...
    effects = [
        WORKSPACE_CONFIG, "projects/", "projects_tracking/", "repo_quarantine/", "shared_capabilities/",
        "projects_tracking/project-registry.json", "projects_tracking/workspace-config.sha256",
        "projects_tracking/project-registry.sha256", "projects_tracking/PROJECT_MANAGEMENT.md",
    ]
    config_path = paths.root / WORKSPACE_CONFIG
    if config_path.exists():
        ...
        return {"valid": status["valid"], "applied": False, "already_initialized": True, "workspace_id": selected_id, "effects": effects, "status": status}
    if not apply:
        return {"valid": True, "applied": False, "workspace_id": selected_id, "effects": effects, "approval_required": True}
    paths.root.mkdir(parents=True, exist_ok=True)
    for directory in (paths.projects, paths.tracking, paths.quarantine, paths.shared_capabilities, paths.events, paths.checkpoints, paths.project_state, paths.sessions, paths.history):
    ...
    _write_new(paths.tracking / CONFIG_SEAL_NAME, (_sha(config_path) + "\n").encode("utf-8"))
    registry = {"schema_version": "1.0", "workspace_id": selected_id, "revision": 1, "projects": [], "updated_utc": _now()}
    _validate_control(registry, "workspace-registry.schema.json")
    _write_json_new(paths.registry, registry)
    _write_new(paths.tracking / REGISTRY_SEAL_NAME, (_sha(paths.registry) + "\n").encode("utf-8"))
    append_event(_event_ledger(paths), "workspace-initialized", {"workspace_id": selected_id, "layout": effects, "hard_delete": False})
    _refresh_dashboard(paths, registry)
    return {"valid": True, "applied": True, "workspace_id": selected_id, "effects": effects, "drop_location": paths.projects.as_posix(), "tracking": paths.tracking.as_posix()}


@_locked_when_apply
def discover_projects(root: Path, *, source_root: Path, apply: bool = False, max_files: int = 100_000) -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
    registry = _load_registry(paths)
    registered_paths = {str(item["path"]): item for item in registry["projects"]}
    registered_ids = {str(item["project_id"]): str(item["path"]) for item in registry["projects"]}
    proposals = []
    for project in sorted((item for item in paths.projects.iterdir() if item.is_dir()), key=lambda item: item.name.casefold()):
        if not _inside(project, paths.projects) or project.resolve() == paths.projects.resolve():
        ...
        existing_record = json.loads(record_path.read_text(encoding="utf-8")) if record_path.is_file() else None
        project_id = str(existing_record.get("project_id")) if isinstance(existing_record, Mapping) else _slug(project.name, prefix="prj")
        ...
        substantive = any(item.is_file() for item in project.rglob("*") if ".engineering-bootstrap" not in item.parts)
        if substantive:
            inspect_existing_project(project, max_files=max_files)
        mode = str(existing_record.get("commissioning_mode")) if isinstance(existing_record, Mapping) and existing_record.get("commissioning_mode") in {"new", "existing"} else ("existing" if substantive else "new")
        proposals.append({"project_id": project_id, "path": relative, "mode": mode, "action": "commission_and_register"})
    if not apply:
        return {"valid": True, "applied": False, "workspace_id": config["workspace_id"], "proposals": proposals, "approval_required": bool(proposals)}
    admitted = []
    for proposal in proposals:
        project = paths.root / proposal["path"]
        result = commission(project, str(proposal["mode"]), apply=True, source_root=source_root)
        if not result.get("valid") or not result.get("applied"):
            raise RuntimeError(f"project commissioning failed: {proposal['path']}: {result.get('errors') or result.get('conflicts')}")
        check = project_check(project, source_root=source_root)
        if not check.get("valid"):
            raise RuntimeError(f"project integrity check failed: {proposal['path']}: {check.get('errors')}")
        local = json.loads((project / ".engineering-bootstrap" / "project-record.json").read_text(encoding="utf-8"))
        ...
        binding = {
            ...
            "project_id": local["project_id"], "project_path": proposal["path"],
        ...
        central = {
            "project_id": local["project_id"], "name": local["name"], "state": "registered",
            "path": proposal["path"], "commissioning_mode": local["commissioning_mode"],
            "classification": local.get("classification", "internal"),
            "memory_namespace": f"project/{local['project_id']}",
            "memory_root": memory.relative_to(paths.root).as_posix(),
            "project_record_sha256": _sha(project / ".engineering-bootstrap" / "project-record.json"),
            "workspace_binding_sha256": _sha(binding_path),
            "project_management": (project / "PROJECT_MANAGEMENT.md").relative_to(paths.root).as_posix(),
            "registered_utc": _now(), "cross_project_access": "deny",
        }
        registry["projects"].append(central)
        registry["projects"] = sorted(registry["projects"], key=lambda item: str(item["project_id"]))
        registry["revision"] = int(registry["revision"]) + 1
        registry["updated_utc"] = _now()
        _validate_control(registry, "workspace-registry.schema.json")
        _replace_registry(paths, registry)
        append_event(_event_ledger(paths), "project-admitted", {"project": central, "commissioning": {"mode": proposal["mode"], "valid": True}})
        admitted.append(central)
    _refresh_dashboard(paths, registry)
    return {"valid": True, "applied": True, "workspace_id": config["workspace_id"], "admitted": admitted, "registered_count": len(registry["projects"])}


def create_project(root: Path, name: str, *, source_root: Path, apply: bool = False) -> dict[str, object]:
    paths = workspace_paths(root)
    _load_config(paths)
    safe_name = ID_SAFE.sub("-", name.casefold()).strip("-_")
    if not safe_name or safe_name != name.casefold() or "/" in name or "\\" in name:
        raise ValueError("project name must contain only lowercase letters, digits, hyphens, or underscores")
    project = paths.projects / safe_name
    if project.exists():
        raise ValueError("project drop directory already exists; use workspace discover")
    if not apply:
        return {
            "valid": True, "applied": False, "approval_required": True,
            "project_id": _slug(safe_name, prefix="prj"), "project": project.as_posix(),
            "effects": ["create empty project drop directory", "commission new project", "register isolated tracking and memory"],
        }
    with FileLock(paths.tracking / ".workspace-control.lock"):
    ...
    admitted = [item for item in discovered["admitted"] if item["project_id"] == project_id]
    if len(admitted) != 1:
        raise RuntimeError("new project directory was not uniquely admitted")
    return {"valid": True, "applied": True, "project": admitted[0], "drop_location": project.as_posix()}


def _new_scope(workspace_id: str, project_id: str, *, agent_id: str, session_id: str) -> ScopeEnvelope:
    suffix = _stable({"workspace": workspace_id, "project": project_id, "agent": agent_id, "session": session_id, "time": _now()})[:16]
    return ScopeEnvelope(workspace_id, project_id, agent_id, session_id, f"work_{suffix}", f"lease_{suffix}", f"intent_{suffix}", f"corr_{suffix}")
...
def activate_project(
    ...
    *,
    ...
    if target.get("state") not in {"registered", "active"}:
        raise ValueError(f"project cannot be activated from state: {target.get('state')}")
    ...
    operation_id = _stable({"operation": "activate", "project_id": project_id, "session_id": session_id, "lease_id": new_scope.lease_id})[:24]
    operation_intent_written = False
    switch_receipt = None
    if previous and previous.get("status") == "active":
        if not context_reset_confirmed:
            return {"valid": False, "activated": False, "approval_required": True, "errors": ["project switch requires explicit context-reset confirmation"]}
        ...
        checkpoint = {
            "schema_version": "1.0", "project_id": old_scope.project_id,
            "session_id": old_scope.session_id, "scope": asdict(old_scope),
            "status": "released_for_switch", "next_safe_action": f"activate {project_id}", "created_utc": _now(),
        }
        checkpoint_path = paths.checkpoints / old_scope.project_id / f"switch-{_stable(checkpoint)[:20]}.json"
        _write_json_new(checkpoint_path, checkpoint)
        foreign = ContextObject("negative-old-project-memory", "project", old_scope.project_id, "memory", "internal", "switch-negative-test")
        negative = authorize_context(new_scope, (foreign,))
        evidence = SwitchEvidence(True, True, True, True, True, True, True, negative.decision == "deny")
        switch_receipt = switch_project(_event_ledger(paths), old_scope, new_scope, evidence)
        if switch_receipt.get("decision") != "active":
            raise RuntimeError(f"project switch rejected: {switch_receipt.get('reasons')}")
    ...
    active = {
        "schema_version": "1.0", "status": "active", "workspace_id": config["workspace_id"],
        "project_id": project_id, "project_root": project.relative_to(paths.root).as_posix(),
        "memory_namespace": target["memory_namespace"], "memory_root": target["memory_root"],
        "scope": asdict(new_scope), "writable_roots": [target["path"]],
        "created_utc": _now(), "expires_utc": expires.isoformat(),
...
@_locked_mutation
def release_project(root: Path, *, session_id: str = "session_operator", context_reset_confirmed: bool) -> dict[str, object]:
    paths = workspace_paths(root)
    _load_config(paths)
    registry = _load_registry(paths)
    session_path = _session_path(paths, session_id)
    ...
    if active.get("status") != "active":
        return {"valid": True, "released": False, "reason": "no_active_project"}
    if not context_reset_confirmed:
        return {"valid": False, "released": False, "approval_required": True, "errors": ["release requires explicit context-reset confirmation"]}
    released = {**active, "status": "released", "released_utc": _now()}
    scope = ScopeEnvelope(**active["scope"])
    operation_id = _stable({"operation": "release", "project_id": scope.project_id, "session_id": scope.session_id, "lease_id": scope.lease_id})[:24]
    ...
    active_project_ids = {str(item["project_id"]) for item in _active_sessions(paths)}
    for item in registry["projects"]:
        if item["project_id"] == scope.project_id and item["state"] == "active" and scope.project_id not in active_project_ids:
            item["state"] = "registered"
    registry["revision"] = int(registry["revision"]) + 1
    registry["updated_utc"] = _now()
    _replace_registry(paths, registry)
...
def current_project(root: Path, *, session_id: str = "session_operator") -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
    session_path = _session_path(paths, session_id)
    active = _verified_session(paths, session_id)
...
def list_projects(root: Path) -> dict[str, object]:
    ...
    config = _load_config(paths)
    registry = _load_registry(paths)
    return {"valid": True, "workspace_id": config["workspace_id"], "revision": registry["revision"], "projects": registry["projects"]}
...
def show_project(root: Path, project_id: str) -> dict[str, object]:
    ...
    active = [item for item in _active_sessions(paths) if item.get("project_id") == project_id]
    return {"valid": True, "workspace_id": config["workspace_id"], "project": record, "active_sessions": [item["scope"]["session_id"] for item in active]}


@_locked_mutation
def renew_project(root: Path, *, session_id: str = "session_operator", minutes: int = 60) -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
...
@_locked_when_apply
def transition_project(root: Path, project_id: str, action: str, evidence: Iterable[str], *, apply: bool = False) -> dict[str, object]:
    transitions = {"pause": ("registered", "paused"), "resume": ("paused", "registered"), "archive": ("registered", "archived")}
    ...
    active = [item for item in _active_sessions(paths) if item.get("project_id") == project_id]
    if active:
        raise ValueError("release the active project before changing its lifecycle state")
    ...
    if not apply:
        return {"valid": True, "applied": False, "approval_required": True, "project_id": project_id, "action": action, "from": expected, "to": target, "evidence": evidence_ids}
    record["state"] = target
    registry["revision"] = int(registry["revision"]) + 1
    registry["updated_utc"] = _now()
    _replace_registry(paths, registry)
    event = append_event(_event_ledger(paths), f"project-{action}", {"project_id": project_id, "from": expected, "to": target, "evidence": evidence_ids, "created_utc": _now()})
    _refresh_dashboard(paths, registry)
    return {"valid": True, "applied": True, "project_id": project_id, "state": target, "event": event.as_posix()}
...
def _vault(paths: WorkspacePaths, workspace_id: str, project_id: str) -> MemoryVault:
    return MemoryVault(_memory_root(paths, project_id), workspace_id=workspace_id, project_id=project_id)


@_locked_when_apply
def ingest_memory(root: Path, project_id: str, sources: Iterable[Path], *, source_root: Path, session_id: str = "session_operator", actor_id: str = "agent_operator", apply: bool = False) -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
    ...
    if not resolved or any(not path.is_file() or not _inside(path, project) for path in resolved):
        raise ValueError("memory sources must be existing files inside the active project")
    if not apply:
        return {"valid": True, "applied": False, "project_id": project_id, "sources": [path.relative_to(project).as_posix() for path in resolved], "approval_required": True}
    artifacts = tuple(replace(SourceArtifact.from_path(path), locator=path.relative_to(project).as_posix()) for path in resolved)
    scope = ScopeEnvelope(**active["scope"])
    ProjectStreamContext, execute_project_stream = _project_stream_api()
    context = ProjectStreamContext(
        "memory_ingest_distill", str(config["workspace_id"]), project_id, scope.agent_id,
        scope.session_id, scope.intent_id, scope.correlation_id, PREFLIGHT,
        ("read_local", "write_workspace"), True,
        {"vault": _vault(paths, str(config["workspace_id"]), project_id), "sources": artifacts, "lease_id": scope.lease_id, "next_safe_action": "review and validate candidate memory"},
        str(active["expires_utc"]), 120,
    )
    result = execute_project_stream(source_root, context, checkpoint_root=paths.checkpoints)
...
@_locked_when_apply
def transition_memory(root: Path, project_id: str, memory_id: str, target: str, evidence: Iterable[str], *, session_id: str = "session_operator", actor_id: str = "agent_operator", apply: bool = False) -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
    ...
    evidence_ids = tuple(sorted(set(map(str, evidence))))
    if not apply:
        return {"valid": True, "applied": False, "project_id": project_id, "memory_id": memory_id, "target": target, "evidence": evidence_ids, "approval_required": True}
    decision = _vault(paths, str(config["workspace_id"]), project_id).transition(memory_id, target, evidence=evidence_ids)
...
def correct_memory(
    ...
    *,
    ...
    if not source_path.is_file() or not _inside(source_path, project):
        raise ValueError("memory correction source must be an existing file inside the active project")
    source_bytes = source_path.read_bytes()
    if not source_bytes or len(source_bytes) > 10 * 1024 * 1024 or b"\x00" in source_bytes:
        raise ValueError("memory correction source must be nonempty bounded textual evidence")
    try:
        source_text = source_bytes.decode("utf-8")
    except UnicodeDecodeError as error:
        raise ValueError("memory correction source must be UTF-8 textual evidence") from error
    ...
    preview = {
        ...
        "source_validation": {"bytes": len(source_bytes), "utf8": True, "non_whitespace_characters": len(source_text.strip())},
        "retrieval_activation": "disabled_until_correction_certification",
        "derived_rebuild_required": ["embedding", "graph", "retrieval_cache", "summary", "transfer_exports"],
    }
    if not apply:
        return preview
    scope = ScopeEnvelope(**active["scope"])
    ...
    return {**preview, "applied": True, "approval_required": False, "write": asdict(written), "event": event.as_posix()}


def search_memory(root: Path, project_id: str, query: str, *, actor_id: str, session_id: str = "session_operator", limit: int = 5) -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
    _, _, active = _require_active(paths, project_id, session_id)
    if actor_id != active["scope"]["agent_id"]:
        raise ValueError("memory actor does not own the active project lease")
    records = _vault(paths, str(config["workspace_id"]), project_id).search(query, actor_id=actor_id, limit=limit)
    results = [
        {
            "memory_id": item.memory_id, "title": item.title, "summary": item.summary,
            "source_artifact": item.source_artifact, "source_sha256": item.source_sha256,
            "evidence_locator": item.evidence_locator, "revision": item.revision,
        }
        for item in records
    ]
    return {
        "valid": True, "project_id": project_id, "query": query,
        "results": results,
        "selection_receipt_sha256": _stable({
            "workspace_id": config["workspace_id"], "project_id": project_id,
            "lease_id": active["scope"]["lease_id"], "actor_id": actor_id,
            "query_sha256": hashlib.sha256(query.encode("utf-8")).hexdigest(),
            "selected": [(item["memory_id"], item["revision"], item["source_sha256"]) for item in results],
        }),
    }


@_locked_when_apply
def maintain_memory(root: Path, project_id: str, *, source_root: Path, session_id: str = "session_operator", actor_id: str = "agent_operator", apply: bool = False) -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
    _, _, active = _require_active(paths, project_id, session_id)
    if actor_id != active["scope"]["agent_id"]:
        raise ValueError("memory actor does not own the active project lease")
    if not apply:
        return {"valid": True, "applied": False, "project_id": project_id, "approval_required": True, "effects": ["append index generation", "preserve previous generations"]}
    scope = ScopeEnvelope(**active["scope"])
    ProjectStreamContext, execute_project_stream = _project_stream_api()
    context = ProjectStreamContext(
        "memory_maintenance", str(config["workspace_id"]), project_id, scope.agent_id,
        scope.session_id, scope.intent_id, scope.correlation_id, PREFLIGHT,
        ("read_local", "write_workspace"), True,
        {"vault": _vault(paths, str(config["workspace_id"]), project_id), "next_safe_action": "review memory health evidence"},
        str(active["expires_utc"]), 120,
    )
    result = execute_project_stream(source_root, context, checkpoint_root=paths.checkpoints)
    output = asdict(result)
...
def memory_status(root: Path, project_id: str, *, actor_id: str, session_id: str = "session_operator") -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
    _, _, active = _require_active(paths, project_id, session_id)
    if actor_id != active["scope"]["agent_id"]:
    ...
    errors = sorted({error for record in records for error in record.validation_errors()})
    return {
        "valid": not errors, "project_id": project_id, "memory_root": vault.root.as_posix(),
        "record_count": len(records), "retrievable_count": len(retrieval), "validation_errors": errors,
        "index": vault.reconcile_indexes(),
    }


@_locked_when_apply
def reconcile_memory(root: Path, project_id: str, *, session_id: str = "session_operator", actor_id: str = "agent_operator", apply: bool = False) -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
    _, _, active = _require_active(paths, project_id, session_id)
    if actor_id != active["scope"]["agent_id"]:
    ...
    if not orphans:
        return {"valid": True, "applied": False, "project_id": project_id, "orphan_generations": [], "action": "none", "hard_delete": False}
    if not apply:
        return {"valid": True, "applied": False, "approval_required": True, "project_id": project_id, "orphan_generations": list(orphans), "action": "move_to_recoverable_quarantine", "hard_delete": False}
    quarantine = paths.quarantine / "memory" / project_id / f"orphans-{_stable({'orphans': orphans, 'time': _now()})[:16]}"
    inventory = []
    with FileLock(vault.root / ".memory-control" / "vault.lock"):
        for generation in orphans:
            source = vault.root / ".memory-control" / "index" / "generations" / generation
            if not source.is_dir():
                raise ValueError(f"orphan generation disappeared before reconciliation: {generation}")
            files = tuple(item for item in sorted(source.rglob("*")) if item.is_file())
            for item in files:
                inventory.append({"generation": generation, "path": item.relative_to(source).as_posix(), "sha256": _sha(item), "bytes": item.stat().st_size})
        ...
        manifest = {
            "schema_version": "1.0", "operation": "recoverable_move_to_quarantine",
            "workspace_id": config["workspace_id"], "project_id": project_id,
            "orphan_generations": list(orphans), "inventory": inventory,
            "inventory_reconciled": True, "hard_delete": False, "created_utc": _now(),
        }
    ...
    event = append_event(_event_ledger(paths), "memory-orphans-quarantined", {"project_id": project_id, "quarantine": quarantine.relative_to(paths.root).as_posix(), "generations": orphans, "inventory_reconciled": True, "hard_delete": False})
    return {"valid": True, "applied": True, "project_id": project_id, "quarantine": quarantine.as_posix(), "orphan_generations": list(orphans), "inventory_count": len(inventory), "event": event.as_posix(), "hard_delete": False}


def list_workflows(source_root: Path) -> dict[str, object]:
    definitions = json.loads((source_root / "registry" / "project_stream_orchestrations.json").read_text(encoding="utf-8"))
    bindings = json.loads((source_root / "registry" / "project_stream_handlers.json").read_text(encoding="utf-8"))
    ...
    for item in definitions["orchestrations"]:
        ...
        workflows.append({
            "workflow_id": item["orchestration_id"], "title": item["title"],
            "status": binding.get("status", "missing"), "effects": binding.get("effects", []),
            "skills": item.get("skills", []),
            "deferred_capabilities": item.get("deferred_capabilities", []),
            "outcomes": item.get("outcomes", []),
        })
    return {"valid": len(workflows) == 17 and all(item["status"] == "executable" for item in workflows), "workflow_count": len(workflows), "workflows": workflows}


def _request_path(paths: WorkspacePaths, project: Path, value: object, *, scope: str = "project") -> Path:
    if not isinstance(value, str) or not value.strip():
        raise ValueError("workflow path value must be a non-empty string")
    requested = Path(value)
    if requested.is_absolute() or ".." in requested.parts:
        raise ValueError("workflow path must be relative and must not contain parent traversal")
    ...
    *,
    ...
    if workflow_id in {"memory_ingest_distill", "continuous_improvement"}:
        ...
        if not isinstance(sources, list) or not sources:
            raise ValueError("workflow sources must be a non-empty list of project-relative paths")
        materialized = tuple(_request_path(paths, project, value) for value in sources)
        payload["sources"] = tuple(replace(SourceArtifact.from_path(path), locator=path.relative_to(project).as_posix()) for path in materialized)
    ...
    if workflow_id == "cross_project_transfer":
        ...
        registry = _load_registry(paths)
        source_record = _project_by_id(registry, str(package_raw.get("source_project_id", "")))
        if str(source_record["project_id"]) == project_id:
            raise ValueError("cross-project transfer source must differ from the active destination project")
    ...
    if "candidates" in raw:
        ...
        payload["candidates"] = tuple(_request_path(paths, project, value) for value in raw["candidates"])
    if "quarantine_root" in raw:
        payload["quarantine_root"] = _request_path(paths, project, raw["quarantine_root"], scope="workspace")
        if not _inside(payload["quarantine_root"], paths.quarantine):
            raise ValueError("workflow quarantine path must remain below repo_quarantine")
    if "package" in raw:
    ...
    payload["next_safe_action"] = str(raw.get("next_safe_action", "review workflow receipt"))
    return payload


def _validate_workflow_payload_shape(workflow_id: str, payload: Mapping[str, object]) -> None:
    ...
    if errors:
        raise ValueError("workflow payload contract failed: " + ", ".join(sorted(errors)))


@_locked_when_apply
def run_workflow_request(root: Path, request_path: Path, *, source_root: Path, apply: bool = False) -> dict[str, object]:
    paths = workspace_paths(root)
    config = _load_config(paths)
    ...
    blocked_direct = {"workspace_bootstrap", "project_onboard", "project_switch", "project_pause_resume", "project_close"}
    if workflow_id in blocked_direct:
        raise ValueError(f"use the dedicated workspace/project command for {workflow_id}")
    listing = list_workflows(source_root)
    workflow = next((item for item in listing["workflows"] if item["workflow_id"] == workflow_id), None)
    ...
    if missing_effects:
        if apply:
            append_event(_event_ledger(paths), "workflow-policy-denied", {
                "workflow_id": workflow_id, "project_id": project_id, "session_id": session_id,
                "requested_effects": list(approved_effects), "missing_effects": missing_effects,
            })
        raise ValueError("workflow effects are not fully approved: " + ", ".join(missing_effects))
    ...
    if receipt_path.is_file():
        ...
        return {"valid": receipt.get("result", {}).get("status") == "completed", "replayed": True, "receipt": receipt}
    if not apply:
        return {
            "valid": True, "applied": False, "approval_required": True,
            "workflow_id": workflow_id, "project_id": project_id,
            "effects": list(approved_effects), "request_sha256": request_hash,
    ...
    payload = _materialize_workflow_payload(
        workflow_id, raw_payload, paths=paths, project=project,
        workspace_id=str(config["workspace_id"]), project_id=project_id,
    )
    scope = ScopeEnvelope(**active["scope"])
    ...
    if not isinstance(timeout_seconds, int) or isinstance(timeout_seconds, bool) or not 1 <= timeout_seconds <= 3600:
        raise ValueError("workflow timeout_seconds must be an integer between 1 and 3600")
    ...
    ProjectStreamContext, execute_project_stream = _project_stream_api()
    context = ProjectStreamContext(
        workflow_id, str(config["workspace_id"]), project_id, scope.agent_id,
        scope.session_id, scope.intent_id, scope.correlation_id, PREFLIGHT,
        approved_effects, True, payload, str(active["expires_utc"]), timeout_seconds,
    )
    result = execute_project_stream(source_root, context, checkpoint_root=paths.checkpoints)
    result_value = asdict(result)
    receipt = {
        "schema_version": "1.0", "idempotency_key": idempotency_key,
        "request_sha256": request_hash, "workflow_id": workflow_id,
        "workspace_id": config["workspace_id"], "project_id": project_id,
        "lease_id": scope.lease_id, "approved_effects": list(approved_effects),
        "handler_status": workflow["status"], "result": result_value,
        "checkpoint_sha256": {
            checkpoint: _sha(Path(checkpoint)) for checkpoint in result.checkpoints if Path(checkpoint).is_file()
        },
        "created_utc": _now(),
    }
    _write_json_new(receipt_path, receipt)
    append_event(_event_ledger(paths), "workflow-finished", {"receipt": receipt_path.relative_to(paths.root).as_posix(), "request_sha256": request_hash, "status": result.status})
    return {"valid": result.status == "completed", "applied": True, "replayed": False, "receipt": receipt}


def workspace_status(root: Path, *, source_root: Path | None = None) -> dict[str, object]:
    paths = workspace_paths(root)
    errors: list[str] = []
    try:
        config = _load_config(paths)
    ...
    for item in registry["projects"]:
        ...
        try:
            ...
            if not local_record.is_file() or _sha(local_record) != item.get("project_record_sha256"):
                raise ValueError("project record hash drift")
            binding_path = project / ".engineering-bootstrap" / "workspace-binding.json"
            if not binding_path.is_file() or _sha(binding_path) != item.get("workspace_binding_sha256"):
                raise ValueError("workspace binding hash drift")
            binding = json.loads(binding_path.read_text(encoding="utf-8"))
            if binding.get("workspace_id") != config["workspace_id"] or binding.get("project_id") != project_id or binding.get("cross_project_access") != "deny":
            ...
            checked = project_check(project, source_root=source_root) if source_root else {"valid": True, "errors": []}
            if not checked.get("valid"):
                raise ValueError("project check failed: " + ", ".join(checked.get("errors", [])))
    ...
    for active in active_sessions:
        if active.get("project_id") not in seen_ids:
            ...
            if active.get("writable_roots") != [bound["path"]] or active.get("memory_root") != bound["memory_root"]:
    ...
    try:
        ...
        if pending_operations:
            errors.append("incomplete_workspace_operation:" + ",".join(pending_operations))
    except (OSError, ValueError, json.JSONDecodeError) as error:
        pending_operations = ()
        errors.append(f"workspace_operation_journal_invalid:{error}")
    return {
        "valid": not errors, "workspace_id": config["workspace_id"], "root": paths.root.as_posix(),
        "drop_location": paths.projects.as_posix(), "tracking": paths.tracking.as_posix(),
        "registered_count": len(registry["projects"]),
        "active_session_count": len(active_sessions), "active_project_ids": sorted(active_project_ids),
        "pending_operation_ids": list(pending_operations),
        "active_project_id": next(iter(active_project_ids)) if len(active_project_ids) == 1 else None,
        "projects": project_results, "errors": errors,
    }
...
def workspace_monitor(root: Path, *, source_root: Path) -> dict[str, object]:
    ...
    if not status.get("valid"):
        return {"valid": False, "workspace": status, "memory": [], "integrations": {"valid": False, "errors": ["workspace invalid"]}}
    config = _load_config(paths)
    registry = _load_registry(paths)
    memory = []
    for record in registry["projects"]:
        ...
        memory.append({
            ...
            "bytes": sum(path.stat().st_size for path in vault.root.rglob("*") if path.is_file()),
            "index": vault.reconcile_indexes(),
        })
    integrations = validate_integrations(source_root, smoke=True)
    return {
        "valid": status["valid"] and integrations["valid"] and all(not item["index"]["orphan_generations"] for item in memory),
        "workspace": status,
...
def rebuild_workspace_projections(
    ...
    for path, event in zip(event_paths, ledger_validation["events"], strict=True):
        ...
        payload = event.get("payload")
        if not isinstance(payload, Mapping) or event.get("payload_sha256") != _stable(payload):
        ...
        if kind == "workspace-operation-intent" and operation_id:
            ...
            if operation_id:
                completed_ids.add(operation_id)
            session = payload.get("active_session")
            if not isinstance(session, Mapping) or str(session.get("project_id", "")) not in projects:
                raise ValueError("project activation event references an unknown project")
            session_id = str(session.get("scope", {}).get("session_id", ""))
            if not session_id:
            ...
            project_id = str(payload.get("project_id", ""))
            if project_id not in projects:
                raise ValueError("project lifecycle event references an unknown project")
    ...
    preview = {
        "valid": True, "applied": False, "approval_required": True,
        "event_count": len(event_paths), "registered_count": len(projects),
        "active_project_ids": sorted({str(item["project_id"]) for item in sessions.values() if item.get("status") == "active"}),
        "active_session_count": sum(item.get("status") == "active" for item in sessions.values()),
