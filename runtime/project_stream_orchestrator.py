...
def _append_checkpoint(
    ...
    record = {
        ...
        "next_safe_action": str(
            context.payload.get(
                "next_safe_action", "reconcile checkpoint before resume"
            )
        ),
...
def validate_checkpoint_resume(
    ...
    return {
        "valid": not reasons,
        "status": "resumable" if not reasons else "stale",
        "reasons": tuple(reasons),
        "next_safe_action": record.get("next_safe_action"),
...
def _sources(payload: Mapping[str, object]) -> tuple[SourceArtifact, ...]:
    ...
    if not isinstance(values, (tuple, list)) or not all(
        isinstance(item, SourceArtifact) for item in values
    ):
...
def memory_ingest_distill(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    for item in bundle.knowledge:
        ...
        record = MemoryRecord(
            ...
            context.agent_id,
            context.session_id,
            str(context.payload.get("lease_id", context.intent_id)),
            item.statement[:120],
            "procedure" if item.kind == "procedure" else "fact",
            item.statement,
            str(source["locator"]),
            str(source["sha256"]),
            evidence_id,
    ...
    return {
        "memory_notes_updated": len(created),
        "memory_ids": tuple(created),
...
def memory_maintenance(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    if before["orphan_generations"]:
        return {
            "memory_health_report": "review_required",
            "index": before,
            "action": "quarantine_orphans_after_human_review",
            "hard_delete": False,
        }
...
def continuous_improvement(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    recommendations = evolution_recommendations(
        bundle.skills, usage=usage, failure_rates=failure_rates
    )
    return {
...
def chaos_resilience_cycle(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    if not isinstance(experiments, (list, tuple)) or not all(
        isinstance(item, Mapping) for item in experiments
    ):
...
def guarded_change(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    result = apply_guarded_change(
        _path(context, "active_root"),
        _path(context, "staged_file"),
        _path(context, "destination"),
        _path(context, "quarantine_root"),
        _path(context, "ledger"),
        evidence,
...
def incident_diagnose_recover(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    result = recover_incident(
        _path(context, "active_root"),
        _path(context, "recovery_candidate"),
        _path(context, "destination"),
        _path(context, "quarantine_root"),
        _path(context, "ledger"),
        evidence,
        ...
        collision_policy=str(
            context.payload.get("collision_policy", "replace_preserving_previous")
        ),
    )
...
def project_switch(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    if (
        not isinstance(old, ScopeEnvelope)
        or not isinstance(new, ScopeEnvelope)
        or not isinstance(evidence, SwitchEvidence)
    ):
        raise ValueError(
            "switch workflow requires old/new ScopeEnvelope and SwitchEvidence"
        )
    return {
        "new_project_session_active": switch_project(
            _path(context, "ledger"), old, new, evidence
        )
    }
...
def safe_cleanup(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    if not isinstance(candidates, (list, tuple)) or not all(
        isinstance(item, Path) for item in candidates
    ):
...
def shared_capability_promote(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    result = promote_capability(
        _path(context, "candidate"),
        _path(context, "shared_root"),
        _path(context, "ledger"),
        evidence,
        staging_root=_path(context, "staging_root"),
        expected_source_sha256=str(context.payload.get("expected_source_sha256", "")),
...
def workspace_bootstrap(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    if project.parent.resolve() != drop_root.resolve():
        raise ValueError(
            "project must be a direct child of the workspace projects drop root"
        )
    source_root = context.payload.get("source_root")
    if not isinstance(source_root, Path):
        raise ValueError(
            "workspace bootstrap requires the installed framework source root"
        )
    initialized = initialize_workspace(
        workspace, workspace_id=context.workspace_id, apply=True
    )
...
def workstream_plan_dispatch(context: ProjectStreamContext) -> Mapping[str, object]:
    ...
    if (
        ...
        raise ValueError(
            "dispatch workflow requires workstream list and resource snapshot"
        )
...
def execute_project_stream(
    ...
    *,
    ...
    workflows = {
        str(item["orchestration_id"]): item for item in registry["orchestrations"]
    }
    workflow = workflows.get(context.workflow_id)
    if workflow is None:
        return ProjectStreamResult(
            context.workflow_id, "blocked", ("unknown_workflow",), {}, (), None
        )
    checkpoints = []
    required_values = (
        context.workspace_id,
    ...
    if not all(value.strip() for value in required_values):
        return ProjectStreamResult(
            context.workflow_id, "blocked", ("required_context_missing",), {}, (), None
        )
    if context.workflow_id != "workspace_bootstrap" and not context.project_id.strip():
        return ProjectStreamResult(
            context.workflow_id, "blocked", ("project_binding_missing",), {}, (), None
        )
    if context.timeout_seconds < 1 or context.timeout_seconds > 3600:
        return ProjectStreamResult(
            context.workflow_id,
            "blocked",
            ("workflow_timeout_outside_budget",),
            {},
            (),
            None,
        )
    if _authorization_expired(context):
        return ProjectStreamResult(
            context.workflow_id, "blocked", ("project_lease_expired",), {}, (), None
        )
    missing_preflight = tuple(
        name
        for name in workflow["preflight"]
        if context.preflight.get(name) is not True
    )
    if missing_preflight:
        return ProjectStreamResult(
            context.workflow_id,
            "blocked",
            tuple(f"preflight_failed:{name}" for name in missing_preflight),
            {},
            (),
            None,
        )
    checkpoints.append(
        _append_checkpoint(
            checkpoint_root, context, "preflight", "passed", context.preflight
        )
    )
    registry_handlers = _load(root / "registry" / "project_stream_handlers.json")
    binding = next(
        item
        for item in registry_handlers["workflows"]
        if item["orchestration_id"] == context.workflow_id
    )
    if binding["status"] != "executable":
        checkpoints.append(
            _append_checkpoint(
                checkpoint_root, context, "activation", "blocked", binding
            )
        )
        return ProjectStreamResult(
            context.workflow_id,
            "plan_only",
            ("workflow_implementation_not_admitted",),
            {
                "skills": tuple(workflow["skills"]),
    ...
    if _authorization_expired(context) or elapsed > context.timeout_seconds:
        reason = (
            "project_lease_expired"
            if _authorization_expired(context)
            else "workflow_timeout_exceeded"
        )
        checkpoints.append(
            _append_checkpoint(
                checkpoint_root,
    ...
    missing_outcomes = tuple(
        name for name in workflow["outcomes"] if name not in outputs
    )
    status = "completed" if not missing_outcomes else "incomplete"
    checkpoints.append(
        _append_checkpoint(checkpoint_root, context, "verification", status, outputs)
    )
    return ProjectStreamResult(
        context.workflow_id,
        status,
        tuple(f"outcome_missing:{name}" for name in missing_outcomes),
        outputs,
        tuple(checkpoints),
        str(binding["handler"]),
    )
