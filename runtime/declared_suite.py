...
def list_outcomes(root: Path, *, kind: str | None = None, owner: str | None = None) -> dict:
...
def describe_outcome(root: Path, kind: str, outcome_id: str) -> dict:
    ...
    record = next((item for item in owners if item["kind"] == kind and item["source_id"] == outcome_id), None)
    if record is None:
        return {"valid": False, "errors": [f"unknown declared outcome: {kind}/{outcome_id}"]}
    if kind == "orchestration":
        workflows = _load(root, "orchestration/workflows/declared-suite.yaml")["workflows"]
        contract = next(item for item in workflows if item["id"] == outcome_id)
    else:
        name = "capability-contracts.json" if kind == "skill" else "script-contracts.json"
        contracts = _load(root, f".agents/skills/{record['owner']}/references/{name}")["contracts"]
        contract = next(item for item in contracts if item["id"] == outcome_id)
    return {"valid": True, "metadata_only": False, "owner": record["owner"], "contract": contract}


def plan_outcome(root: Path, kind: str, outcome_id: str, payload: Mapping[str, Any]) -> dict:
    ...
    if target in (None, "", []) or not isinstance(constraints, Mapping):
        return {"valid": False, "errors": ["target and object-valued constraints are required"], "outcome": outcome_id}
    ...
    return {
        ...
        "recovery": contract.get("recovery") or contract.get("rollback_or_compensation"),
        "evidence_required": contract.get("evidence") or contract.get("evidence_outputs"),
...
def _walk_inventory(target: Path, maximum_files: int) -> list[dict]:
    ...
    for path in paths:
        ...
        records.append({"path": path.relative_to(base).as_posix(), "bytes": len(data), "sha256": hashlib.sha256(data).hexdigest()})
    return records
...
def _compare(payload: Mapping[str, Any]) -> dict:
    ...
    if isinstance(baseline, (int, float)) and isinstance(candidate, (int, float)):
        ...
        return {"baseline": baseline, "candidate": candidate, "delta": delta, "relative_delta": relative}
    left = json.dumps(baseline, sort_keys=True)
    right = json.dumps(candidate, sort_keys=True)
    return {"equal": left == right, "baseline_sha256": _stable_hash(baseline), "candidate_sha256": _stable_hash(candidate)}
...
def _rank(payload: Mapping[str, Any]) -> dict:
    ...
    for index, item in enumerate(candidates):
        ...
        ranked.append({"id": item.get("id", f"candidate-{index}"), "score": score, "input_index": index})
...
def _scan(payload: Mapping[str, Any]) -> dict:
    ...
    for pattern in patterns:
        ...
        for match in re.finditer(re.escape(pattern), text, flags=re.IGNORECASE):
            matches.append({"pattern": pattern, "start": match.start(), "end": match.end()})
...
def _validate(payload: Mapping[str, Any]) -> dict:
    ...
    unknown = sorted(str(name) for name in record if isinstance(allowed, list) and name not in allowed)
    return {"accepted": not missing and not unknown, "missing": missing, "unknown": unknown}
...
def run_script_outcome(root: Path, outcome_id: str, payload: Mapping[str, Any]) -> dict:
    ...
    try:
        if words & {"map", "mapper", "index", "inventory", "bom", "manifest", "archive", "provenance"}:
            target = Path(str(payload["target"])).resolve()
            result = {"files": _walk_inventory(target, int(payload.get("maximum_files", 10000)))}
        elif words & {"compare", "differential", "regression"}:
            result = _compare(payload)
        elif words & {"route", "router", "rank", "planner", "calibrator", "scorer", "budget"}:
            ...
            result = {"normalized_record": dict(sorted(payload.items())), "record_sha256": _stable_hash(payload)}
    except (OSError, TypeError, ValueError) as error:
        return {"valid": False, "outcome": outcome_id, "errors": [str(error)], "request_sha256": plan.get("request_sha256")}
    return {"valid": True, "outcome": outcome_id, "owner": described["owner"], "read_only": True, "result": result, "result_sha256": _stable_hash(result), "plan": plan}
...
def validate_declared_suite(root: Path) -> dict:
    ...
    if owners["record_count"] != 257 or len(seen) != 257:
        errors.append(f"owner denominator mismatch: declared={owners['record_count']} unique={len(seen)}")
    if workflows["workflow_count"] != 62:
        errors.append(f"workflow denominator mismatch: {workflows['workflow_count']}")
    return {"valid": not errors, "outcomes": len(seen), "workflows": workflows["workflow_count"], "errors": errors}
