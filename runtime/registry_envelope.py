...
def derive_count(payload: Mapping[str, Any], record: Mapping[str, Any]) -> int:
    ...
    if rule == "filtered":
        return sum(isinstance(item, dict) and item.get(record["field"]) == record.get("equals") for item in collection)
    if rule == "unique":
        return len({item.get(record["field"]) for item in collection if isinstance(item, dict)})
    if rule == "duplicates":
        values = [item.get(record["field"]) for item in collection if isinstance(item, dict)]
        return len(values) - len(set(values))
    if rule == "source_cards":
        return sum(isinstance(item, dict) and item.get("id") in SOURCE_CARD_IDS for item in collection)
    if rule == "child_cards":
        return sum(isinstance(item, dict) and item.get("id") not in SOURCE_CARD_IDS for item in collection)
    if rule == "nested_length":
        return sum(len(item.get(record["nested"], ())) for item in collection if isinstance(item, dict))
    if rule == "nested_filtered":
        return sum(
            isinstance(nested, dict) and nested.get(record["field"]) == record.get("equals")
...
def validate_envelope_document(payload: Mapping[str, Any], record: Mapping[str, Any]) -> list[str]:
    ...
    try:
        ...
        return [str(error)]
    return [] if value == expected else [f"{key}={value} does not match derived count {expected}"]
...
def validate_registry_envelopes(root: Path) -> dict[str, Any]:
    ...
    for item in records:
        ...
        payload = by_path.setdefault(relative, json.loads((root / relative).read_text(encoding="utf-8")))
        errors.extend(f"{relative}: {error}" for error in validate_envelope_document(payload, item))
        for required in ("schema", "builder", "consumer"):
            if not item.get(required):
                errors.append(f"{relative}/{item.get('count_key')}: missing {required}")
    return {"schema_version": "1.0", "valid": not errors, "record_count": len(records), "registry_count": len(by_path), "errors": errors}
