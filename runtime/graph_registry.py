...
def build_graph_artifacts(root: Path) -> dict[str, bytes]:
    ...
    for name, edges in {
        ...
        artifacts[name] = _render({
            "schema_version": "1.0", "generator": GENERATOR,
            "nodes": graphs.capability_nodes, "edges": [asdict(edge) for edge in edges],
        })
    ...
    orchestrations = _load(root / "registry" / "project_stream_orchestrations.json")["orchestrations"]
    edges: list[dict[str, Any]] = []
    nodes: set[str] = set()
    for orchestration in sorted(orchestrations, key=lambda item: item["orchestration_id"]):
        source = orchestration["orchestration_id"]
        nodes.add(source)
        for order, target in enumerate(orchestration.get("skills", ())):
            nodes.add(target)
            edges.append({"source": source, "target": target, "relation": "executes", "order": order})
        for order, target in enumerate(orchestration.get("deferred_capabilities", ())):
            nodes.add(target)
            edges.append({"source": source, "target": target, "relation": "deferred_enhancement", "order": order})
    edges.sort(key=lambda item: (item["source"], item["relation"], item["order"], item["target"]))
    artifacts["project_stream_dependency_graph.json"] = _render({
        "schema_version": "2.0", "generator": GENERATOR,
        "orchestration_count": len(orchestrations), "node_count": len(nodes),
        "capability_reference_count": len(edges), "nodes": sorted(nodes), "edges": edges,
    })
    source_paths = {
        "registry/capability_map.json", "registry/skill_catalog.toml", "registry/builders.json",
        "registry/tools.json", "registry/models.json", "registry/knowledge_sources.json",
        "registry/integrations.json", "registry/project_stream_orchestrations.json",
        *(str(item["contract"]) for item in active),
    }
    sources = [{"path": path, "sha256": _sha(root / path)} for path in sorted(source_paths)]
    outputs = [{"path": f"registry/graphs/{name}", "sha256": hashlib.sha256(payload).hexdigest()} for name, payload in sorted(artifacts.items())]
...
def write_graph_artifacts(root: Path, output: Path | None = None) -> dict[str, Any]:
    ...
    return {"valid": True, "output": output.as_posix(), "artifact_count": len(artifacts)}
...
def validate_graph_artifacts(root: Path) -> dict[str, Any]:
    ...
    if actual_names != set(expected):
        errors.append(f"graph file set mismatch: expected {sorted(expected)}, found {sorted(actual_names)}")
    for name, payload in expected.items():
        path = graph_root / name
        if not path.is_file() or path.read_bytes() != payload:
            errors.append(f"stale graph artifact: {name}")
    return {"valid": not errors, "artifact_count": len(expected), "generator": GENERATOR, "errors": errors}
