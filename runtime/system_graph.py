...
def build_system_graph(root: Path) -> SystemGraph:
    ...
    for item in catalog.get("skills", ()):
        ...
        add(
            ...
            if item["admission_record"] != item["id"]
            else (),
        )
    for name, key, asset_type in (
        ("builders.json", "builders", "builder"),
        ("tools.json", "tools", "tool"),
        ("knowledge_sources.json", "knowledge_sources", "knowledge"),
        ("integrations.json", "integrations", "integration"),
    ):
    ...
    return SystemGraph(
        tuple(sorted(nodes.values(), key=lambda item: (item.asset_type, item.node_id))),
        tuple(
