...
def _resolve_pointer(
    document: dict[str, Any], fragment: str, ref: str, schema_path: Path
) -> dict[str, Any]:
    if not fragment:
        return document
    if not fragment.startswith("/"):
        raise ValueError(
            f"unsupported JSON Schema reference fragment {ref!r} in {schema_path}"
        )
    target: Any = document
...
def _resolve_external_path(
    ref_path: str, schema_path: Path, contract_root: Path
) -> Path:
    ...
    try:
        ...
        raise ValueError(
            f"unresolved external schema reference {ref_path!r} in {schema_path}"
        ) from exc
    ...
    while True:
        if cursor.is_symlink() or (
            ...
            raise ValueError(
                f"symlinked schema references are not allowed: {ref_path!r}"
            )
        try:
...
def _resolve_ref(
    ...
    if not isinstance(ref, str) or not ref:
        raise ValueError(
            f"schema reference must be a non-empty string in {schema_path}"
        )
    parsed = urlsplit(ref)
    ...
    target = _resolve_pointer(
        target_document, fragment if marker else "", ref, schema_path
    )
...
def _errors(
    ...
    if "$ref" in rule:
        ...
        siblings = {
            key_name: value for key_name, value in rule.items() if key_name != "$ref"
        }
        if siblings:
            errors.extend(
                _errors(
    ...
    for child in rule.get("allOf", ()):
        errors.extend(
            _errors(
                instance, child, root_schema, schema_path, at, contract_root, ref_stack
            )
        )
    ...
    if "not" in rule and not _errors(
        instance, rule["not"], root_schema, schema_path, at, contract_root, ref_stack
    ):
        errors.append(f"{at}: prohibited schema matched")
    if (
        "if" in rule
        and not _errors(
            instance, rule["if"], root_schema, schema_path, at, contract_root, ref_stack
        )
        and "then" in rule
    ):
        errors.extend(
...
def _pattern_example(pattern: str) -> str:
    ...
    if pattern == "^handoff-[a-f0-9]{24}$":
        return "handoff-" + "a" * 24
    if pattern == "^memory-plan-[0-9a-f]{24}$":
        return "memory-plan-" + "a" * 24
    if pattern == "^model-attachment-[0-9a-f]{24}$":
        return "model-attachment-" + "a" * 24
    if pattern == "^task-plan-[0-9a-f]{24}$":
        return "task-plan-" + "a" * 24
...
def _minimal(
    ...
    if "$ref" in rule:
        ...
        if siblings and _errors(
            candidate, siblings, root_schema, schema_path, "$", contract_root, ref_stack
        ):
            sibling_candidate = _minimal(
                siblings, root_schema, schema_path, contract_root, ref_stack
            )
            if not _errors(
                sibling_candidate,
                resolved,
    ...
    if "anyOf" in rule:
        return _minimal(
            rule["anyOf"][0], root_schema, schema_path, contract_root, ref_stack
        )
    declared = rule.get("type")
    choices = [declared] if isinstance(declared, str) else list(declared or ())
    selected = next(
        (item for item in choices if item != "null"),
        choices[0] if choices else "object",
    )
    if selected == "object":
        value: dict[str, Any] = {}
        properties = rule.get("properties", {})
        ...
        for child in rule.get("allOf", ()):
            if "if" in child:
                ...
                continue
            addition = _minimal(
                child, root_schema, schema_path, contract_root, ref_stack
            )
        ...
        if (
            ...
            addition = _minimal(
                rule["then"], root_schema, schema_path, contract_root, ref_stack
            )
            if isinstance(addition, dict):
                value.update(addition)
        return value
    ...
    if selected == "string":
        ...
        result = (
            _pattern_example(str(rule["pattern"])) if "pattern" in rule else "value"
        )
        minimum = int(rule.get("minLength", 0))
        return (
            result if len(result) >= minimum else result + "x" * (minimum - len(result))
        )
    if selected == "integer":
...
def build_minimal_instance(
    schema_path: Path, *, contract_root: Path | None = None
) -> Any:
...
def _schema_structure_errors(
    value: Any, at: str = "$", *, schema_object: bool = True
) -> list[str]:
    errors: list[str] = []
    ...
    if schema_object:
        ...
        errors.extend(
            f"{at}: unsupported schema keyword {key}" for key in sorted(unknown)
        )
        if "$ref" in value and (
            not isinstance(value["$ref"], str) or not value["$ref"]
        ):
        ...
        if "type" in value:
            ...
            if (
                ...
                or not choices
                or not all(
                    isinstance(item, str) and item in known_types for item in choices
                )
            ):
                errors.append(
                    f"{at}: type must declare one or more supported JSON types"
                )
        ...
        if "additionalProperties" in value and not isinstance(
            value["additionalProperties"], (bool, dict)
        ):
            errors.append(
                f"{at}: additionalProperties must be a boolean or schema object"
            )
        ...
        if (
            "multipleOf" in value
            and _finite_number(value["multipleOf"])
            and value["multipleOf"] <= 0
        ):
        ...
        if "format" in value and (
            ...
            or value["format"] not in SUPPORTED_FORMATS
        ):
        ...
        for map_key in ("properties", "$defs"):
            ...
            if child_map and not isinstance(child_map, dict):
                ...
                for key, child in child_map.items():
                    errors.extend(
                        _schema_structure_errors(child, f"{at}/{map_key}/{key}")
                    )
        ...
        for key in ("allOf", "anyOf"):
            ...
            if key in value and not isinstance(children, list):
                ...
                for index, child in enumerate(children):
                    errors.extend(
                        _schema_structure_errors(child, f"{at}/{key}/{index}")
                    )
    return errors
...
def _schema_children(rule: dict[str, Any]) -> Iterable[dict[str, Any]]:
    for map_key in ("properties", "$defs"):
        ...
        if isinstance(child_map, dict):
            yield from (
                child for child in child_map.values() if isinstance(child, dict)
            )
...
def _admit_schema(
    schema: dict[str, Any], schema_path: Path, contract_root: Path
) -> set[Path]:
...
def validate_contract_corpus(root: Path) -> dict[str, Any]:
    ...
    paths = sorted(
        path.relative_to(root).as_posix() for path in contract_root.rglob("*.json")
    )
    errors: list[str] = []
    owned: dict[str, str] = {}
    ...
    for relative in paths:
        ...
        try:
            schema = _load(path)
            errors.extend(
                f"{relative}: {item}" for item in _schema_structure_errors(schema)
            )
            ...
            contract_digests[relative] = contract_digest(
                path, contract_root=contract_root
            )
            example = build_minimal_instance(path, contract_root=contract_root)
            example_errors = _errors(
                example, schema, schema, path, "$", contract_root.resolve(strict=True)
            )
            errors.extend(
                f"{relative}: generated valid fixture failed: {item}"
            ...
            if schema.get("required") and not _errors(
                ...
                errors.append(
                    f"{relative}: empty-object negative fixture unexpectedly passed"
                )
        ...
        if ownership_record is None:
            ...
            if ownership_record.get("contract_version") != "1.0.0":
                errors.append(
                    f"{relative}: ownership contract version is missing or invalid"
                )
    ...
    errors.extend(
        f"ownership record references missing contract: {relative}"
    ...
    for record in ownership_records:
        state = str(record.get("enforcement", "missing"))
        enforcement_counts[state] = enforcement_counts.get(state, 0) + 1
    corpus_bytes = json.dumps(
        contract_digests, sort_keys=True, separators=(",", ":")
    ).encode("utf-8")
    return {
        "valid": not errors,
