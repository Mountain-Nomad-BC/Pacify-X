...
SERIAL_EFFECTS = frozenset({"write_workspace", "install_tool", "network", "run_service", "migration", "destructive"})
...
def _contracts(records: Iterable[Mapping[str, object]]) -> dict[str, Mapping[str, object]]:
    ...
    for capability_id, contract in contracts.items():
        ...
        for dependency in contract.get("dependencies", ()):
            if isinstance(dependency, str):
                capability_edges.append(GraphEdge(capability_id, dependency, "depends_on"))
                dependency_effect_edges.append(GraphEdge(capability_id, dependency, "hard_dependency"))
        for conflict in contract.get("conflicts", ()):
            if isinstance(conflict, str):
                capability_edges.append(GraphEdge(capability_id, conflict, "conflicts_with"))
        for effect in contract.get("effects", ()):
            if isinstance(effect, str):
                dependency_effect_edges.append(GraphEdge(capability_id, effect, "declares_effect"))
        approval = contract.get("approval", {})
        if isinstance(approval, Mapping) and approval:
            dependency_effect_edges.append(GraphEdge(capability_id, "approval", "requires"))
        evidence = contract.get("evidence", {})
        if isinstance(evidence, Mapping) and evidence:
            dependency_effect_edges.append(GraphEdge(capability_id, "evidence", "requires"))
        ...
        for consumed in contract.get("consumes", ()):
            ...
            for source_id in producers.get(consumed, ()):
                if source_id != target_id:
                    io_edges.append(GraphEdge(source_id, target_id, "produces_for", consumed))
    sorter = lambda edge: (edge.source, edge.target, edge.relation, edge.detail)
    return GraphBundle(tuple(sorted(contracts)), tuple(sorted(capability_edges, key=sorter)), tuple(sorted(io_edges, key=sorter)), tuple(sorted(dependency_effect_edges, key=sorter)))


def find_io_paths(records: Iterable[Mapping[str, object]], source_type: str, target_type: str, *, limit: int = 3) -> tuple[tuple[str, ...], ...]:
    ...
    while queue and len(paths) < limit:
        ...
        for capability_id, contract in sorted(contracts.items()):
            consumes = set(value for value in contract.get("consumes", ()) if isinstance(value, str))
            provides = sorted(value for value in contract.get("provides", ()) if isinstance(value, str))
...
def rank_io_paths(records: Iterable[Mapping[str, object]], source_type: str, target_type: str, *, limit: int = 3) -> tuple[RankedPath, ...]:
    """Rank compatible paths by bounded metadata, never by loading implementations."""
    records = tuple(records)
    contracts = _contracts(records)
    candidates = find_io_paths(records, source_type, target_type, limit=max(limit * 4, limit))
    ranked: list[RankedPath] = []
    for path in candidates:
        path_records = [contracts[item] for item in path]
        risks = [{"R0": 1.0, "R1": 0.8, "R2": 0.5, "R3": 0.2, "R4": 0.0}.get(str(item.get("risk", "R2")), 0.5) for item in path_records]
        mutating = sum(bool(set(item.get("effects", ())) & SERIAL_EFFECTS) for item in path_records)
        safety = max(0.0, (sum(risks) / len(risks)) - 0.15 * mutating)
        cost = sum(float(item.get("cost", {}).get("max_tool_calls", 0)) for item in path_records if isinstance(item.get("cost"), Mapping))
        latency = sum(float(item.get("latency", {}).get("max_seconds", 0)) for item in path_records if isinstance(item.get("latency"), Mapping))
        freshness = sum(1.0 if isinstance(item.get("evidence"), Mapping) and item.get("evidence", {}).get("status") == "current" else 0.25 for item in path_records) / len(path_records)
        score = round(20 * safety + 5 * freshness - cost - latency - len(path), 3)
        ranked.append(RankedPath(path, score, round(safety, 3), cost, latency, (f"steps={len(path)}", f"freshness={freshness:.3f}", f"mutating_steps={mutating}")))
    return tuple(sorted(ranked, key=lambda item: (-item.score, item.capabilities))[:limit])


def validate_orchestration(spec: Mapping[str, object], contracts: Iterable[Mapping[str, object]]) -> tuple[str, ...]:
    """Validate a declarative DAG before it can be admitted to a workflow registry."""
    known = _contracts(contracts)
    errors: list[str] = []
    for field in ("id", "version", "status", "inputs", "outputs", "steps", "parallelism", "stop_conditions"):
    ...
    for step in steps:
        ...
        if step_id in ids:
            ...
            continue
        ids.add(step_id); by_id[step_id] = step
    ...
    def visit(step_id: str) -> None:
        if step_id in visiting:
            errors.append(f"orchestration cycle includes {step_id}"); return
        if step_id in visited:
            return
        visiting.add(step_id)
        dependencies = list(by_id[step_id].get("depends_on", ())) + [value for value in by_id[step_id].get("optional_depends_on", ()) if value in by_id]
    ...
    for step_id in sorted(by_id):
        visit(step_id)
    selected = {str(step.get("capability")) for step in by_id.values() if isinstance(step.get("capability"), str)}
    for capability_id in sorted(selected):
        for conflict in known.get(capability_id, {}).get("conflicts", ()):
            if conflict in selected:
                errors.append(f"effect conflict: {capability_id} conflicts with {conflict}")
    parallelism = spec.get("parallelism", {})
    if not isinstance(parallelism, Mapping) or not isinstance(parallelism.get("max_agents"), int) or parallelism["max_agents"] < 1:
        errors.append("parallelism.max_agents must be a positive integer")
    resource = spec.get("resource_budget", {})
    if resource and (not isinstance(resource, Mapping) or not isinstance(resource.get("max_tool_calls"), int) or resource["max_tool_calls"] < 0):
    ...
    if isinstance(parallelism, Mapping) and parallelism.get("max_agents", 1) > 1:
        serial_steps = [step_id for step_id, step in by_id.items() if set(step.get("effects", ())) & SERIAL_EFFECTS]
        if len(serial_steps) > 1:
            errors.append("effect conflict: serial effects require an explicit serial dependency chain")
