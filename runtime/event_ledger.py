...
def validate_event_ledger(ledger: Path, *, require_head: bool = True) -> dict[str, object]:
    ...
    for expected, path in enumerate(event_paths, start=1):
        try:
            ...
            continue
        required = {"schema_version", "sequence", "kind", "created_utc", "payload", "payload_sha256", "previous_event_sha256", "event_sha256"}
        if set(record) != required:
            errors.append(f"{path.name}: event fields are not exact")
            continue
        if record.get("sequence") != expected or not path.name.startswith(f"{expected:08d}-"):
            errors.append(f"{path.name}: event sequence is not contiguous")
        payload = record.get("payload")
        if not isinstance(payload, Mapping) or record.get("payload_sha256") != _sha(payload):
    ...
    if require_head and event_paths:
        try:
            ...
            if head != {"schema_version": "1.0", "sequence": len(event_paths), "event_sha256": previous}:
                errors.append("protected ledger head does not match event chain")
            anchor = anchors / f"{len(event_paths):08d}-{previous}.json"
            if not anchor.is_file() or json.loads(anchor.read_text(encoding="utf-8")) != head:
    ...
    return {
        "schema_version": "1.0", "valid": not errors, "event_count": len(event_paths),
        "head_sha256": previous, "errors": errors, "events": events,
    }
...
def _publish_head(ledger: Path, sequence: int, event_sha256: str) -> None:
    ...
    if head_path.is_file():
        ...
        with history.open("x", encoding="utf-8", newline="\n") as stream:
            stream.write(head_path.read_text(encoding="utf-8")); stream.flush(); os.fsync(stream.fileno())
...
def append_chained_event(ledger: Path, kind: str, payload: Mapping[str, object]) -> Path:
    ledger = ledger.resolve()
    ...
    with FileLock(ledger / ".event-ledger.lock"):
        ...
        if not current["valid"]:
            raise ValueError("event ledger integrity failure: " + "; ".join(current["errors"]))
        sequence = int(current["event_count"]) + 1
        record: dict[str, object] = {
            "schema_version": "1.0", "sequence": sequence, "kind": kind,
            "created_utc": datetime.now(timezone.utc).isoformat(), "payload": dict(payload),
            "payload_sha256": _sha(payload), "previous_event_sha256": current["head_sha256"],
        }
