...
VSCODE_SETTINGS = {
    ...
    "files.watcherExclude": {
        "**/raw/**": True, "**/quarantine/**": True, "**/evidence/**": True,
        "**/planning/inventory/**": True, "**/.engineering-bootstrap/cache/**": True,
    },
    "search.exclude": {"**/raw/**": True, "**/quarantine/**": True, "**/evidence/**": True},
}

VSCODE_TASKS = {
    "version": "2.0.0",
    "tasks": [
        {"label": "Bootstrap: validate installation", "type": "shell", "command": "engineering-bootstrap validate", "problemMatcher": []},
        {"label": "Bootstrap: check project", "type": "shell", "command": "engineering-bootstrap project-check --project .", "problemMatcher": []},
        {"label": "Bootstrap: bounded startup", "type": "shell", "command": "engineering-bootstrap startup --project .", "problemMatcher": []},
        {"label": "Bootstrap: doctor", "type": "shell", "command": "engineering-bootstrap doctor", "problemMatcher": []},
    ],
}
...
def _bootstrap_manifest(mode: str) -> bytes:
    return (json.dumps({
        "schema_version": "1.0", "mode": mode, "profile": "default", "stack": "project-owned",
        "questionnaire": {"stack": "discover_or_select", "deployment": "explicit", "sensitive_data": "policy_gated"},
        "baselines": {"security": True, "tests": True, "evidence": True, "accessibility": True},
        "model_routing": "capability_contract", "tool_installation": "approval_required",
        "startup": {"metadata_only": True, "maximum_selected_skills": 3, "skill_bodies_copied_to_project": False},
        "project_management": {"entrypoint": "PROJECT_MANAGEMENT.md", "state": ".engineering-bootstrap/project-management/state.json"},
        "prompts": {"new": ".engineering-bootstrap/prompts/new-project.md", "existing": ".engineering-bootstrap/prompts/existing-project.md"},
        "deployment_guardrails": ["tests_pass", "security_review", "evidence_current", "rollback_viable", "explicit_approval"],
    }, indent=2) + "\n").encode()


def _skill_registry(source: Path, mode: str) -> bytes:
    catalog = tomllib.loads((source / "registry/skill_catalog.toml").read_text(encoding="utf-8"))
    records = []
    for item in catalog.get("skills", ()):
        ...
        records.append({
            "id": str(item["id"]), "body": body.as_posix(), "status": status,
            "sha256": _sha256(content), "bytes": len(content), "source": "installed_framework",
        })
    return (json.dumps({
        "schema_version": "1.0", "mode": mode, "canonical_runtime_registry": "installed_framework",
        "loading_rule": "metadata_only_at_startup_body_after_selection", "max_selected_skills": 3,
        "skill_bodies_copied_to_project": False, "skills": sorted(records, key=lambda item: item["id"]),
    }, indent=2) + "\n").encode()


def _project_record(project: Path, mode: str, inventory: Mapping[str, object] | None) -> bytes:
    slug = re.sub(r"[^A-Za-z0-9_-]+", "-", project.resolve().name).strip("-") or "project"
    owners = list(map(str, (inventory or {}).get("canonical_owner_candidates", ())))
    record = {
        "project_id": f"prj_{slug}", "name": project.resolve().name, "state": "registered",
        "classification": "internal",
        "repositories": [{"repository_id": f"repo_{slug}", "logical_root": ".", "role": "application"}],
        "owners": owners, "policy_overlay": None, "memory_namespace": f"project/prj_{slug}",
...
def scaffold_files(
    ...
    files: dict[Path, bytes] = {
        ...
        Path("AI_ASSISTANT.md"): b"# Assistant entry point\n\nFollow `.engineering-bootstrap/AI_ASSISTANT.md`, `PROJECT_MANAGEMENT.md`, and existing repository instructions.\n",
        Path("CLAUDE.md"): b"# Assistant instructions\n\nFollow `.engineering-bootstrap/AI_ASSISTANT.md` and existing repository instructions.\n",
        Path("GEMINI.md"): b"# Assistant instructions\n\nFollow `.engineering-bootstrap/AI_ASSISTANT.md` and existing repository instructions.\n",
        Path(".ai/assistant.toml"): AI_CONFIG.encode(),
        Path(".github/copilot-instructions.md"): b"# Repository assistant contract\n\nFollow `.engineering-bootstrap/AI_ASSISTANT.md` and existing repository instructions.\n",
        Path(".codex/config.toml"): CODEX_CONFIG.encode(),
        Path(".vscode/settings.json"): (json.dumps(VSCODE_SETTINGS, indent=2) + "\n").encode(),
        Path(".vscode/tasks.json"): (json.dumps(VSCODE_TASKS, indent=2) + "\n").encode(),
        ...
        Path(".engineering-bootstrap/bootstrap-manifest.json"): _bootstrap_manifest(mode),
        Path(".engineering-bootstrap/project-registry.json"): _skill_registry(source, mode),
        Path(".engineering-bootstrap/project-record.json"): _project_record(target, mode, inventory),
        Path(".engineering-bootstrap/prompts/new-project.md"): (prompts / "NEW_PROJECT_PROMPT.md").read_bytes(),
        Path(".engineering-bootstrap/prompts/existing-project.md"): (prompts / "EXISTING_PROJECT_PROMPT.md").read_bytes(),
        Path(".engineering-bootstrap/vscode.settings.json"): (json.dumps(VSCODE_SETTINGS, indent=2) + "\n").encode(),
        Path(".engineering-bootstrap/vscode.tasks.json"): (json.dumps(VSCODE_TASKS, indent=2) + "\n").encode(),
        Path(".engineering-bootstrap/SECURITY_BASELINE.md"): b"# Security baseline\n\nLeast privilege, secret redaction, dependency review, and explicit mutation approval are required.\n",
        Path(".engineering-bootstrap/TEST_BASELINE.md"): b"# Test baseline\n\nNew capabilities require positive, negative, integration, and effect-boundary tests plus the repository's own validation commands.\n",
        Path(".engineering-bootstrap/EVIDENCE_BASELINE.md"): b"# Evidence baseline\n\nCompletion requires deterministic, sanitized, current, task-scoped evidence and independent outcome verification.\n",
        Path(".engineering-bootstrap/DEPLOYMENT_GUARDRAILS.md"): b"# Deployment guardrails\n\nDeployment is blocked without tests, security review, current evidence, viable rollback, and explicit approval.\n",
    }
    ...
    for profile in sorted(profile_root.glob("*.toml")):
        files[Path(".engineering-bootstrap/profiles") / profile.name] = profile.read_bytes()
    if mode == "existing":
...
def _diff(existing: bytes, proposed: bytes, path: str) -> list[str]:
    ...
    return list(difflib.unified_diff(before, after, fromfile=f"existing/{path}", tofile=f"bootstrap/{path}", lineterm=""))[:200]


def inspect_scaffold(project: Path, expected: Mapping[Path, bytes]) -> tuple[list[str], list[str], list[str], list[dict[str, object]]]:
    create: list[str] = []
    unchanged: list[str] = []
    conflicts: list[str] = []
    ...
    for relative, content in sorted(expected.items(), key=lambda item: item[0].as_posix()):
        ...
        record: dict[str, object] = {
            "path": path,
            "scope": "bootstrap_owned" if path.startswith(".engineering-bootstrap/") else "integration",
...
def commission(project: Path, mode: str, *, apply: bool = False, source_root: Path | None = None, questionnaire: Path | None = None) -> dict:
    if mode not in {"new", "existing"}:
        raise ValueError("mode must be new or existing")
    ...
    if questionnaire is not None:
        ...
        validate_instance(questionnaire_record, source / "contracts" / "commissioning-questionnaire.schema.json")
    ...
    bootstrap_conflicts = [path for path in conflicts if path.startswith(".engineering-bootstrap/")]
    ...
    flow = (
        ...
        if mode == "new"
        else ["inventory", "architecture-truth-map", "canonical-owners", "risk", "adoption-plan", "approval"]
    )
    result = {
        "valid": not blocking_conflicts,
        "mode": mode, "project": str(resolved), "applied": False,
        "effects": ["read_local"] if not apply else ["read_local", "write_workspace"],
        "flow": flow, "next": "resolve-bootstrap-conflicts" if blocking_conflicts else "approval",
        "create": create, "unchanged": unchanged, "conflicts": conflicts,
        "blocking_conflicts": blocking_conflicts, "preserved_existing": preserved, "file_plan": file_plan,
    }
    ...
    adoption = {
        "schema_version": "1.0", "mode": mode, "effects": ["read_local", "write_workspace"],
        "plan": file_plan, "blocking_conflicts": blocking_conflicts, "preserved_existing": preserved,
    }
...
def apply_project_brief(project: Path, questionnaire: Path, *, source_root: Path | None = None, apply: bool = False) -> dict[str, object]:
    """Validate and version a questionnaire into an already commissioned project."""
    resolved = project.resolve(); source = (source_root or framework_root()).resolve()
    ...
    validate_instance(answers, source / "contracts/commissioning-questionnaire.schema.json")
    ...
    updated["work"]["scope"]["included"] = [answers["answers"]["scope"]] if answers["answers"].get("scope") else []
    updated["knowledge"] = {
        "facts": answers["facts"], "assumptions": answers["assumptions"],
        "contradictions": answers["contradictions"], "unknowns": answers["unknowns"],
    }
    ...
    next_action = "create bounded milestones and acceptance-linked punch cards" if accepted else "resolve unaccepted commissioning decisions"
    updated["lifecycle"] = {"phase": "planning" if accepted else "commissioning", "status": "brief_accepted_awaiting_plan" if accepted else "brief_incomplete", "next_action": next_action}
    updated["checkpoint"]["revision"] = int(updated["checkpoint"]["revision"]) + 1
    updated["checkpoint"]["next_safe_action"] = next_action
    updated["evidence"]["commissioning_questionnaire"] = ".engineering-bootstrap/project-management/commissioning-questionnaire.json"
    ...
    result = {
        "valid": True, "applied": False, "accepted": accepted, "project": resolved.as_posix(),
        "effects": ["write_project_management"], "target": target.relative_to(resolved).as_posix(),
        "next_action": next_action,
    }
    if not apply:
        return {**result, "approval_required": True}
    history = state_path.parent / "history"; history.mkdir(parents=True, exist_ok=True)
    for current in (state_path, target, resolved / "PROJECT_MANAGEMENT.md"):
        if current.is_file():
            preserved = history / f"{current.stem}-{_sha256(current.read_bytes())[:16]}{current.suffix}"
    ...
    text = re.sub(r"## Current state\s+.*?(?=\n## )", summary.rstrip(), text, count=1, flags=re.DOTALL)
    ...
    return {**result, "applied": True, "state_revision": updated["checkpoint"]["revision"]}
...
def project_check(project: Path, source_root: Path | None = None) -> dict:
    ...
    errors: list[str] = []
    required = [
        Path(".engineering-bootstrap/AGENTS.md"), Path(".engineering-bootstrap/AI_ASSISTANT.md"),
        Path(".engineering-bootstrap/project.toml"), Path(".engineering-bootstrap/bootstrap-manifest.json"),
        Path(".engineering-bootstrap/project-registry.json"), Path(".engineering-bootstrap/commissioning-receipt.json"),
        Path(".engineering-bootstrap/project-record.json"),
        Path(".engineering-bootstrap/adoption-plan.json"), Path(".engineering-bootstrap/prompts/new-project.md"),
    ...
    try:
        project_toml = tomllib.loads((resolved / ".engineering-bootstrap/project.toml").read_text(encoding="utf-8"))
    ...
    if mode == "existing" and not (resolved / ".engineering-bootstrap/existing-project-inventory.json").is_file():
        errors.append("missing existing-project inventory")
    try:
        manifest = json.loads((resolved / ".engineering-bootstrap/bootstrap-manifest.json").read_text(encoding="utf-8"))
        if manifest.get("startup", {}).get("skill_bodies_copied_to_project") is not False:
            errors.append("bootstrap manifest permits eager skill copying")
    except (OSError, json.JSONDecodeError) as error:
        errors.append(f"invalid bootstrap manifest: {error}")
    skills: list[str] = []
    try:
        registry = json.loads((resolved / ".engineering-bootstrap/project-registry.json").read_text(encoding="utf-8"))
        if registry.get("max_selected_skills") != 3 or registry.get("skill_bodies_copied_to_project") is not False:
        ...
        for item in registry.get("skills", []):
            ...
            if _sha256(content) != item.get("sha256") or len(content) != item.get("bytes"):
            ...
            if not text.startswith("---\n") or not name_match or name_match.group(1) != item.get("id"):
    ...
    try:
        record = json.loads((resolved / ".engineering-bootstrap/project-record.json").read_text(encoding="utf-8"))
        if not re.fullmatch(r"prj_[A-Za-z0-9_-]+", str(record.get("project_id", ""))):
            errors.append("invalid project record identity")
        if record.get("state") not in {"discovered", "registered", "active", "paused", "dormant", "archived", "retired"}:
        ...
        if not repositories or any(not str(item.get("repository_id", "")).startswith("repo_") for item in repositories):
    ...
    for relative in (Path(".engineering-bootstrap/vscode.settings.json"), Path(".engineering-bootstrap/vscode.tasks.json")):
        try:
            json.loads((resolved / relative).read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as error:
            errors.append(f"invalid {relative.as_posix()}: {error}")
    return {"valid": not errors, "project": str(resolved), "mode": mode, "skill_count": len(skills), "skills": skills, "errors": errors}
