...
def bounded_startup(
    ...
    *,
    ...
    policy_payload = load_json(policy_path) if policy_path.is_file() else {"policies": []}
    raw_policies = policy_payload.get("policies")
    policy_summaries = (
        tuple(raw_policies)
        if isinstance(raw_policies, list)
        else tuple({"id": policy_id} for policy_id in policy_payload.get("startup_policy_ids", ()))
    )
    ...
    models = tuple(load_json(model_path).get("models", ())) if model_path.is_file() else ()
    profile_path = project_root / ".engineering-bootstrap" / "project.toml"
    profile = tomllib.loads(profile_path.read_text(encoding="utf-8")) if profile_path.is_file() else {"status": "unconfigured"}
    names = startup_candidates(root) if tool_names is None else tuple(sorted(set(tool_names)))
    with ThreadPoolExecutor(max_workers=min(max_probe_workers, max(1, len(names)))) as pool:
        tools = tuple(sorted(pool.map(lambda name: _probe(name, tool_resolver), names)))
    catalog_path = root / "registry" / "skill_catalog.toml"
    skill_metadata = tuple(load_skill_catalog(root).get("skills", ())) if catalog_path.is_file() else ()
    if len(skill_metadata) > config.budget.max_initial_registry_records:
        raise ValueError("skill metadata exceeds startup budget")
    return StartupSnapshot(config, capabilities, policy_summaries, tools, models, profile, skill_metadata)
