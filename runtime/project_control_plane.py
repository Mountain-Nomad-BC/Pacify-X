...
def _allowed_source(
    ...
    if not staging_root.is_dir() or not source.is_file() or not _inside(source, staging_root):
...
def _write_journal_record(transaction: Path, sequence: int, state: str, payload: Mapping[str, object]) -> Path:
...
def quarantine_candidates(
    ...
    *,
    ...
    replay_requested = bool(paths) and quarantine.is_dir() and all(
        _inside(path, root) and path != root and not path.exists()
        and (quarantine / path.relative_to(root)).exists()
        for path in paths
    )
    for path in paths:
        if path == root or not _inside(path, root) or (not path.exists() and not replay_requested):
    ...
    if not reasons:
        for path in paths:
            if replay_requested:
                ...
                files = (replay_target,) if replay_target.is_file() else tuple(item for item in sorted(replay_target.rglob("*")) if item.is_file())
                inventory.extend({"path": item.relative_to(quarantine).as_posix(), "sha256": _sha(item), "size": item.stat().st_size} for item in files)
            else:
    ...
    operation_id = _stable({"root": root.as_posix(), "quarantine": quarantine.as_posix(), "inventory": inventory})
    transactions = (transaction_root or (ledger.parent / "transactions" / "quarantine")).resolve()
    if not _inside(transactions, root.parent) or transactions == root or _inside(transactions, root):
    ...
    if quarantine.exists():
        if committed.is_file() and all(
            ...
            return {"decision": "quarantined", "count": len(inventory), "inventory": inventory, "event": None, "hard_delete": False, "idempotent_replay": True}
    ...
    _write_journal_record(transaction, 1, "intent", {
        "active_root": root.as_posix(), "quarantine_root": quarantine.as_posix(), "inventory": inventory,
    })
    ...
    try:
        for index, path in enumerate(paths, start=1):
            ...
            for item in inventory:
                ...
                if _inside(item_path, path) and (not item_path.is_file() or _sha(item_path) != item["sha256"]):
                    raise RuntimeError(f"source changed before quarantine commit: {item['path']}")
        ...
        _write_journal_record(transaction, 2, "moved", {"moved": [target.as_posix() for _, target in moved]})
        ...
        _write_journal_record(transaction, 2, "rolled-back", {"error": f"{type(error).__name__}: {error}", "rollback_errors": rollback_errors})
        if rollback_errors:
            raise RuntimeError(f"quarantine failed and rollback was incomplete: {rollback_errors}") from error
        raise
    event = append_event(ledger, "artifacts-quarantined", {
        "active_root": root.as_posix(), "quarantine_root": quarantine.as_posix(),
        "inventory": inventory, "hard_delete": False, "transaction_id": operation_id,
    })
    _write_journal_record(transaction, 3, "committed", {"event": event.as_posix(), "inventory_sha256": _stable(inventory)})
    return {"decision": "quarantined", "count": len(inventory), "inventory": inventory, "event": event.as_posix(), "hard_delete": False, "transaction_id": operation_id}
...
def promote_capability(
    ...
    *,
    ...
    if owned_staging is None or expected_source_sha256 is None:
        ...
        reasons.extend(_allowed_source(candidate, owned_staging, expected_source_sha256, allowed_suffixes))
    ...
    if _sha(candidate) != expected_source_sha256:
        return {"decision": "rejected", "reasons": ("source_digest_changed_before_commit",), "automatic_activation": False}
...
def guarded_change(
    ...
    *,
    ...
    if owned_staging is None or expected_source_sha256 is None:
        ...
        reasons.extend(_allowed_source(staged, owned_staging, expected_source_sha256, allowed_suffixes))
    ...
    if evidence.get("tests_passed") is not True or evidence.get("outcome_passed") is not True:
        reasons.append("validation_not_passed")
    if reasons:
        event = append_event(ledger, "change-rejected", {
            "source_sha256": _sha(staged) if staged.is_file() else None,
            "source_mutated": False, "reasons": reasons, "hard_delete": False,
        })
        return {"decision": "rejected", "reasons": tuple(sorted(set(reasons))), "event": event.as_posix(), "source_mutated": False, "hard_delete": False}
    if _sha(staged) != expected_source_sha256:
        return {"decision": "rejected", "reasons": ("source_digest_changed_before_commit",), "source_mutated": False, "hard_delete": False}
    destination.parent.mkdir(parents=True, exist_ok=True)
...
def recover_incident(
    ...
    *,
    ...
    if owned_staging is None or expected_source_sha256 is None:
        ...
        reasons.extend(_allowed_source(candidate, owned_staging, expected_source_sha256, allowed_suffixes))
    transactions = (transaction_root or (ledger.parent / "transactions" / "recovery")).resolve()
    if not _inside(transactions, root.parent) or transactions == root or _inside(transactions, root):
        reasons.append("transaction_root_must_be_sibling_bounded_tree")
    ...
    if reasons:
        return {"decision": "escalated", "reasons": tuple(sorted(set(reasons))), "source_mutated": False, "hard_delete": False}
    ...
    if committed.is_file() and destination.is_file() and _sha(destination) == expected_source_sha256:
        return {"decision": "recovered", "destination": destination.as_posix(), "preserved_previous": None, "event": None, "hard_delete": False, "idempotent_replay": True, "transaction_id": operation_id}
    if transaction.exists():
        ...
        if not destination.exists() and rollback_candidates:
            ...
            _write_journal_record(transaction, len(tuple(transaction.glob("*.json"))) + 1, "rolled-back-after-interruption", {"destination": destination.as_posix()})
        return {"decision": "escalated", "reasons": ("incomplete_transaction_recovered_original",), "source_mutated": False, "hard_delete": False, "transaction_id": operation_id}
    if destination.exists() and (not _inside(quarantine, root.parent) or quarantine.exists()):
        return {"decision": "escalated", "reasons": ("recovery_quarantine_invalid",), "hard_delete": False}
    transactions.mkdir(parents=True, exist_ok=True)
    _write_journal_record(transaction, 1, "intent", {
        "destination": destination.as_posix(), "expected_source_sha256": expected_source_sha256,
        "quarantine_root": quarantine.as_posix(), "evidence_sha256": _stable(evidence),
    })
    destination.parent.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        if _sha(candidate) != expected_source_sha256 or _sha(prepared) != expected_source_sha256:
            raise RuntimeError("recovery artifact hash mismatch before commit")
        _fsync_file(prepared)
        _write_journal_record(transaction, 2, "prepared", {"prepared": prepared.as_posix(), "sha256": expected_source_sha256})
        if fault_injector:
        ...
        _write_journal_record(transaction, 3, "replaced", {"destination": destination.as_posix(), "sha256": expected_source_sha256})
    except BaseException as error:
        abandoned = transaction / f"abandoned-{destination.name}"
        if destination.exists() and _sha(destination) == expected_source_sha256 and preserved.exists():
        ...
        if preserved.exists():
            ...
            _fsync_directory(destination.parent)
        _write_journal_record(transaction, len(tuple(transaction.glob("*.json"))) + 1, "rolled-back", {"error": f"{type(error).__name__}: {error}", "destination_restored": destination.exists()})
        raise
    if preserved.exists():
        quarantine.mkdir(parents=True, exist_ok=False)
    ...
    event = append_event(ledger, "incident-recovered", {
        "destination": destination.relative_to(root).as_posix(), "sha256": _sha(destination),
        "preserved_previous": preserved_result.as_posix() if preserved_result else None, "evidence": dict(evidence), "hard_delete": False,
        "transaction_id": operation_id,
    })
    _write_journal_record(transaction, 4, "committed", {"event": event.as_posix(), "destination_sha256": expected_source_sha256})
    return {"decision": "recovered", "destination": destination.as_posix(), "preserved_previous": preserved_result.as_posix() if preserved_result else None, "event": event.as_posix(), "hard_delete": False, "transaction_id": operation_id}
