...
from .release_identity import authoritative_version, capture_git_identity, validate_version_surfaces, verify_recorded_git_identity
...
def _verify_coverage_binding(root: Path, release: str, certificate: dict[str, Any]) -> dict[str, Any]:
    ...
    if not relative.startswith(expected_prefix) or not path.is_file():
        errors.append("certificate coverage evidence is missing or outside its release root")
...
def _write_supply_chain_evidence(
    ...
    *,
    ...
    subjects = [
        {"name": str(item["filename"]), "digest": {"sha256": str(item["sha256"])}, "size_bytes": int(item["size_bytes"])}
    ...
    sbom = {
        ...
        "metadata": {"component": {"type": "application", "name": "PACIFY-X", "version": release}},
        "components": [
            {
                "type": "file", "name": item["name"],
                "hashes": [{"alg": "SHA-256", "content": item["digest"]["sha256"]}],
                "properties": [{"name": "pacify-x:size-bytes", "value": str(item["size_bytes"])}],
            }
            for item in subjects
        ],
    }
...
def _certificate_ledger_errors(root: Path) -> list[str]:
    ...
    full_repair = validate_full_repair_ledger(
        root, require_all_passed=True, allowed_pending=FINALIZER_FULL_REPAIR_PENDING,
    )
...
def run_release_gates(
    ...
    *,
    ...
    wheel_record = next((item for item in artifact_records if item.get("type") == "wheel"), None)
    if wheel_record is None:
        return {"schema_version": "2.0", "valid": False, "errors": ["exact release wheel is missing"], "gates": {}}
    ...
    installed_root = Path(tempfile.mkdtemp(prefix="installed-wheel-", dir=environment_quarantine))
    install_started = time.monotonic()
    installed = install_exact_wheel(wheel_path, str(wheel_record["sha256"]), installed_root)
    toolchain_gate = {
    ...
    test_command = [
        str(release_python), "-m", "coverage", "run", "--rcfile=.coveragerc",
        "-m", "pytest", *profile["command"][3:], f"--junitxml={junit_path}",
    ]
    environment.update({
        ...
        "COVERAGE_FILE": str(environment_quarantine / f"coverage-{wheel_record['sha256']}.data"),
    })
    test_process = run_test_command(
        test_command, cwd=root, environment=environment,
    ...
    coverage_command = [
        str(release_python), "-m", "coverage", "json", "--rcfile=.coveragerc",
        "--show-contexts", "-o", str(coverage_path),
    ]
    coverage_process = run_test_command(
        coverage_command, cwd=root, environment=environment,
    ...
    if coverage_path.is_file() and coverage_process["valid"]:
        ...
        coverage_gate = {
            "schema_version": "1.0", "valid": False, "coverage_sha256": None,
            "errors": ["executed coverage evidence was not generated", str(coverage_process.get("stderr", ""))[-2000:]],
        }
    artifact_binding = bind_artifact_set(
        artifact_dir, artifact_records,
    ...
    gates: dict[str, Any] = {
        ...
        "full_tests": {"valid": test_exit == 0 and not test_timed_out and totals["tests"] > 0 and totals["failures"] == totals["errors"] == totals["skipped"] == 0, "exit_code": test_exit, "timed_out": test_timed_out, "duration_seconds": round(time.monotonic() - tests_started, 6), "log": f"{evidence_locator_prefix}/full-tests.log", **totals},
        "exact_artifact_install": {"valid": installed["valid"] and installed.get("installed_wheel_sha256") == wheel_record["sha256"], "errors": installed["errors"]},
        "artifact_binding": {"valid": artifact_binding["valid"], "errors": artifact_binding["errors"]},
    ...
    for control_name, control in sanitation_controls["gates"].items():
        gates[f"sanitation_{control_name}"] = {
            ...
            "status": control.get("status"), "tool": control.get("tool"),
            "corpus": control.get("corpus"), "corpus_sha256": control.get("corpus_sha256"),
            ...
            "errors": [] if control.get("status") == "passed" else [f"{control_name} did not pass"],
        }
...
def finalize_release(
    ...
    *,
    ...
    if errors:
        return {"valid": False, "certified": False, "published": False, "errors": errors}
    ...
    try:
        ...
        return {"valid": False, "certified": False, "published": False, "errors": [str(error)]}
    try:
        relative_key = signing_key.relative_to(root)
        if not relative_key.parts or relative_key.parts[0] != ".git":
            return {"valid": False, "certified": False, "published": False, "errors": ["release private key must be outside the tracked repository"]}
    except ValueError:
        pass
    source_control = capture_git_identity(root, version=release)
    if not source_control["valid"]:
        return {"valid": False, "certified": False, "published": False, "errors": source_control["errors"]}
    initial = classify_tree(root)
    if not initial["valid"]:
        return {"valid": False, "certified": False, "published": False, "errors": initial["errors"]}
    wheelhouse_manifest = build_wheelhouse_manifest(wheelhouse, root / "requirements-release.lock")
    if not wheelhouse_manifest["valid"]:
        return {"valid": False, "certified": False, "published": False, "errors": wheelhouse_manifest["errors"]}
    ...
    with FileLock(lock_path, timeout_seconds=5.0):
        ...
        if not frozen["valid"] or frozen["product_digest"] != initial["product_digest"]:
            return {"valid": False, "certified": False, "published": False, "errors": [*frozen["errors"], "staged source does not match captured product"]}
        toolchain_root = directory / "release-toolchain"
        venv.EnvBuilder(with_pip=True).create(toolchain_root)
        release_python = toolchain_root / ("Scripts/python.exe" if os.name == "nt" else "bin/python")
        environment = scrub_release_environment()
        install = subprocess.run(
            offline_install_command(str(release_python), staged / "requirements-release.lock", wheelhouse),
            cwd=staged, env=environment, text=True, capture_output=True, timeout=600,
        )
        if install.returncode:
            return {"valid": False, "certified": False, "published": False, "run_id": run_id, "errors": [f"offline release toolchain install failed: {install.stderr[-2000:]}"]}
        toolchain = toolchain_identity(str(release_python))
        build = build_release_artifacts_once(
            staged, selected_artifact_dir, python_executable=str(release_python), environment=environment,
            intermediate_quarantine=directory / "build-intermediates",
        )
        if not build["valid"] or build["build_invocations"] != 1:
            return {"valid": False, "certified": False, "published": False, "run_id": run_id, "errors": build["errors"]}
        ...
        if build.get("intermediate_custody") is not None:
            _dump(evidence_dir / "build-intermediate-custody.json", build["intermediate_custody"])
        product_manifest = {key: frozen[key] for key in ("schema_version", "policy_version", "policy_sha256", "product_digest", "harness_digest", "product_records")}
        _dump(evidence_dir / "product-manifest.json", product_manifest)
        supply_chain = _write_supply_chain_evidence(
            evidence_dir, release=release, source_control=source_control,
            product_digest=frozen["product_digest"], artifacts=build["artifacts"],
            toolchain=toolchain,
        )
        publication_plan = {
            "schema_version": "1.0", "status": "staged_exact_bytes", "release": release,
            "run_id": run_id, "source_control": source_control, "artifacts": build["artifacts"],
        ...
        try:
            if gate_runner is run_release_gates:
                gates = gate_runner(
                    staged, evidence_dir, release_python=str(release_python), artifact_dir=selected_artifact_dir,
                    artifact_records=build["artifacts"], toolchain=toolchain,
            ...
            gates = {"schema_version": "2.0", "valid": False, "errors": [f"gate runner failed: {type(error).__name__}: {error}"], "gates": {}}
        ...
        if not gates.get("valid") or not unchanged["valid"] or not staged_unchanged["valid"]:
            ...
            if not gate_errors and isinstance(gates.get("gates"), dict):
                for gate_name, gate in gates["gates"].items():
                    ...
                    if isinstance(details, list) and details:
                        gate_errors.extend(f"{gate_name}: {detail}" for detail in details)
                    else:
            ...
            return {
                ...
                "errors": [*gate_errors, *unchanged["errors"], *staged_unchanged["errors"]], "gates": gates,
            }
        roles: dict[str, dict[str, object]] = {}
        for path in sorted(evidence_dir.rglob("*"), key=lambda item: item.as_posix().casefold()):
            if path.is_file() and path.name != "evidence-manifest.json":
                relative = path.relative_to(evidence_dir).as_posix()
                roles[relative] = {
                    "type": path.suffix.casefold().lstrip(".") or "evidence",
                    "required": True, "generation_gate": path.stem, "producer": f"PACIFY-X {release}",
                }
        ...
        certificate = {
            "schema_version": "3.0", "release": release, "status": "self_certified",
            "run_id": run_id, "created_utc": datetime.now(timezone.utc).isoformat(),
            "source_control": source_control, "product_digest": frozen["product_digest"],
            "harness_digest": frozen["harness_digest"], "manifest_file_count": len(frozen["product_records"]),
            "artifact_policy_sha256": frozen["policy_sha256"], "artifacts": build["artifacts"],
            ...
            "toolchain": toolchain, "wheelhouse_manifest_sha256": wheelhouse_manifest["manifest_sha256"],
            ...
            "revocation_triggers": [
                "artifact hash mismatch", "evidence manifest mismatch", "Git identity mismatch",
                "product digest mismatch", "publisher signature failure", "signer revocation",
            ],
        }
        signature_path = release_root / "certificate.json.sig"
        certificate = sign_certificate(certificate, private_key=signing_key, signature_path=signature_path)
        _dump(release_root / "certificate.json", certificate)
        journal_path = root / ".engineering-bootstrap/release-transaction.json"
        journal = {
            "schema_version": "2.0", "run_id": run_id, "status": "publishing_evidence",
            "release": release, "product_digest": frozen["product_digest"],
            "certificate": certificate_relative, "signature": signature_relative, "hard_delete": False,
        }
    ...
    return {
        "valid": verification["valid"], "certified": verification["valid"], "published": False,
        "run_id": run_id, "certificate": certificate_relative, "signature": signature_relative,
        "artifact_dir": str(selected_artifact_dir), "product_digest": frozen["product_digest"],
        "errors": verification["errors"],
    }


def verify_release_certificate(root: Path, *, release: str | None = None, artifact_dir: Path | None = None) -> dict[str, Any]:
    root = root.resolve()
    ...
    if revocation_path.is_file():
        ...
        if revocation.get("status") == "revoked":
            return {"valid": False, "release": release, "errors": [f"release {release} is explicitly revoked"]}
    certificate_path = root / "evidence" / "releases" / release / "certificate.json"
    if not certificate_path.is_file():
        return {"valid": False, "release": release, "errors": ["signed release certificate is missing"]}
    ...
    signature_name = str(certificate.get("signature", {}).get("path", "")) if isinstance(certificate.get("signature"), dict) else ""
    signature = verify_certificate_signature(
        certificate, signature_path=certificate_path.parent / signature_name,
    ...
    source = verify_recorded_git_identity(root, dict(certificate.get("source_control", {})))
    ...
    if not evidence_relative.startswith(f"evidence/releases/{release}/") or not evidence_path.is_file():
        errors.append("certificate evidence manifest is missing or outside its release root")
    else:
    ...
    if (
        ...
        errors.append("certificate artifact manifest is missing or outside its release root")
    else:
        artifact_manifest = _json(artifact_manifest_path)
        if artifact_manifest.get("manifest_sha256") != certificate.get("artifact_manifest_sha256"):
            errors.append("recorded artifact manifest digest does not match certificate")
    ...
    if artifact_dir is None:
        ...
        if artifact_binding.get("artifact_manifest_sha256") != certificate.get("artifact_manifest_sha256"):
            errors.append("artifact manifest digest does not match certificate")
    errors.extend(_certificate_ledger_errors(root))
    return {
        "valid": not errors, "release": release, "run_id": certificate.get("run_id"),
        "product_digest": current["product_digest"], "signing_identity": signature.get("identity"),
        "errors": errors,
    }
