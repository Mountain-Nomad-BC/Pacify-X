...
def resolve_test_profile(root: Path, name: str) -> dict[str, Any]:
    ...
    if unknown_exclusions:
        raise ValueError("test profile contains unknown exclusions: " + ", ".join(unknown_exclusions))
    ...
    return {
        ...
        "command": [sys.executable, "-m", "pytest", "-q", "--durations=50", "-p", "no:cacheprovider", *members],
    }
