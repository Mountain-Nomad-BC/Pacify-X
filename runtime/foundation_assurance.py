...
KNOWN_SURFACE_CHECKS = {
    "config", "static", "unit", "integration", "contract", "build",
    "health", "logs", "route", "interaction", "accessibility", "rollback",
}
...
def compare_contract_surfaces(
    ...
    if len(provider_ids) != len(set(provider_ids)) or len(consumer_ids) != len(set(consumer_ids)):
    ...
    for consumer in consumer_values:
        ...
        if provider is None:
            findings.append({"contract_id": consumer.contract_id, "kind": "provider_missing", "severity": "high"})
            continue
        if not provider.owner.strip():
            findings.append({"contract_id": consumer.contract_id, "kind": "canonical_owner_missing", "severity": "high"})
        for field, left, right in (
            ...
            if left != right:
                findings.append({
                    "contract_id": consumer.contract_id, "kind": f"{field}_mismatch",
                    "severity": "high", "provider": left, "consumer": right,
                })
        missing = sorted(set(consumer.required_fields) - set(provider.fields))
        for field in missing:
            findings.append({"contract_id": consumer.contract_id, "kind": "required_field_missing", "field": field, "severity": "high"})
        for field in sorted(set(consumer.required_fields) & set(provider.fields)):
            ...
            if expected and actual != expected:
                findings.append({
                    "contract_id": consumer.contract_id, "kind": "field_type_mismatch",
                    "field": field, "provider": actual, "consumer": expected, "severity": "high",
                })
        ...
        for scope in sorted(consumer_scopes - provider_scopes):
            findings.append({"contract_id": consumer.contract_id, "kind": "provider_scope_missing", "scope": scope, "severity": "high"})
        for scope in sorted(provider_scopes - consumer_scopes):
            findings.append({"contract_id": consumer.contract_id, "kind": "consumer_scope_unmodeled", "scope": scope, "severity": "medium"})
...
def evaluate_retrieval_readiness(
    ...
    *,
    ...
    if k < 1 or not all(0 <= value <= 1 for value in (minimum_recall, minimum_mrr, minimum_coverage)):
    ...
    for case in values:
        ...
        rows.append({
            "case_id": case.case_id, "returned": len(result),
            "recall_at_k": round(recall, 6), "reciprocal_rank": round(reciprocal_rank, 6),
            "forbidden_exposure": exposed,
        })
    metrics = {
        "cases": len(values), "k": k,
        ...
        "forbidden_exposure_count": sum(len(item["ids"]) for item in forbidden_exposures),
    }
    ...
    return {
        ...
        "reasons": tuple(reasons), "metrics": metrics, "cases": tuple(rows),
...
def gate_model_dataset(
    ...
    *,
    ...
    reasons = sorted(set(reasons))
    split_counts = {split: sum(item.split == split for item in values) for split in sorted(KNOWN_SPLITS)}
    return {
        "decision": "admitted_metadata" if not reasons else "blocked",
        "training_or_model_load_performed": False,
        "reasons": tuple(reasons), "record_count": len(values),
        "split_counts": split_counts,
        "label_split_coverage": {label: tuple(sorted(splits)) for label, splits in sorted(label_splits.items())},
        "dataset_sha256": _stable([asdict(item) for item in values]),
    }


def evaluate_numeric_shift(
    baseline: Sequence[float], observed: Sequence[float], *, threshold: float = 0.25,
) -> dict[str, object]:
    ...
    if not errors:
        ...
        if variance == 0:
            score = 0.0 if right_mean == left_mean and all(value == left_mean for value in right) else 1.0
        else:
    ...
    return {
        ...
        "score": round(score, 6), "threshold": threshold, "errors": tuple(errors),
    }


def validate_dimension_steps(steps: Iterable[Mapping[str, object]]) -> dict[str, object]:
    """Check declared dimensional algebra without evaluating an expression."""
    findings = []
    normalized = []
    for index, raw in enumerate(steps):
        operation = str(raw.get("operation", ""))
        left = {str(key): int(value) for key, value in dict(raw.get("left", {})).items() if int(value)}
        right = {str(key): int(value) for key, value in dict(raw.get("right", {})).items() if int(value)}
        result = {str(key): int(value) for key, value in dict(raw.get("result", {})).items() if int(value)}
        if operation in {"add", "subtract"}:
            ...
            valid = False
        normalized.append({"operation": operation, "left": left, "right": right, "result": result})
        if not valid:
            findings.append({"step": index, "kind": "dimension_mismatch_or_unknown_operation"})
    return {
        "decision": "valid" if normalized and not findings else "invalid",
        "findings": tuple(findings), "steps_sha256": _stable(normalized),
    }


def plan_runtime_surface_validation(surfaces: Iterable[Mapping[str, object]]) -> dict[str, object]:
    """Order static-to-live validation and expose every approval boundary."""
    phases = ("config", "static", "unit", "integration", "contract", "build", "health", "logs", "route", "interaction", "accessibility", "rollback")
    ...
    for surface in values:
        ...
        if not surface_id or not owner:
            errors.append(f"surface_identity_or_owner_missing:{surface_id or 'unknown'}")
        if unknown:
            errors.append(f"unknown_checks:{surface_id}:{','.join(unknown)}")
        ordered = tuple(phase for phase in phases if phase in checks)
        planned.append({
            "id": surface_id, "owner": owner, "checks": ordered,
            "live_runtime_required": bool(set(ordered) & {"health", "logs", "route", "interaction", "accessibility"}),
            "approval_required": bool(surface.get("mutating")) or "build" in ordered,
        })
    return {
        "decision": "planned" if values and not errors else "blocked",
        "errors": tuple(sorted(errors)), "surfaces": tuple(planned),
        "execution_performed": False, "plan_sha256": _stable(planned),
    }
