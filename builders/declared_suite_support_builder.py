...
def main() -> int:
    ...
    support = [
        card for card in ledger["cards"] if card["class"] == "supporting_artifact"
    ]
    ...
    for name in SCHEMA_FIELDS:
        ...
        by_path[relative] = {
            ...
            "tests": [
                ...
                "tests/test_declared_suite_support.py",
            ],
        }
    ...
    formula_names = {
        ...
        "confidence-combination-independent",
        "context-utility-density",
        "expected-plan-utility",
        "impact-risk-score",
        "jaccard-change-overlap",
        "kv-cache-bytes",
        "little-law-serving",
        "mutation-score",
    ...
    for name in sorted(formula_names):
        formulas.append(
            {
                "id": name,
                "callable": f"engineering_bootstrap.declared_suite_formulas:{name.replace('-', '_')}",
                "tests": "tests/test_declared_suite_support.py",
            }
        )
    dump(
        root / "registry" / "declared_suite_formulas.json",
        {"schema_version": "1.0", "formula_count": len(formulas), "formulas": formulas},
    )
    dump(
    ...
    owners = json.loads(
        (root / "registry/declared_outcome_owners.json").read_text(encoding="utf-8")
    )["records"]
    packs = {}
    for number in range(1, 8):
        ...
        pack_records = [
            ...
            if next(
                ...
                and card["kind"] == record["kind"]
                and card["source_id"] == record["source_id"]
            )["pack"].startswith(prefix)
        ]
        packs[prefix] = {
            ...
            "orchestrations": sorted(
                record["source_id"]
                for record in pack_records
                if record["kind"] == "orchestration"
            ),
            "formulas": sorted(
                card["source_id"]
                for card in support
                if card["kind"] == "reference-or-knowledge"
                and any(
                    path.startswith(f"packs/{prefix}-") for path in card["source_paths"]
                )
                and card["source_id"] in formula_names
            ),
        }
    dump(
        root / "registry" / "declared_suite_pack_index.json",
        {"schema_version": "1.0", "pack_count": 7, "packs": packs},
    )
    dump(
        root / "registry" / "declared_suite_dependency_graph.json",
        {
            "schema_version": "1.0",
            "nodes": sorted({record["owner"] for record in owners}),
            "edges": [
                {
                    "from": "govern-operating-kernel",
                    "to": owner,
                    "relation": "governs-execution",
                }
                for owner in sorted({record["owner"] for record in owners})
                if owner != "govern-operating-kernel"
            ]
            + [
                {
                    "from": owner,
                    "to": "manage-revocable-certification",
                    "relation": "supplies-evidence",
                }
                for owner in sorted({record["owner"] for record in owners})
                if owner != "manage-revocable-certification"
            ],
        },
    )
    dump(
        root / "registry" / "declared_suite_pack_metadata.json",
        {
            "schema_version": "1.0",
            "pack_count": 7,
            "packs": {
                number: {
                    "owner": data["owner"],
                    "readme": f"Operational domain owned by {data['owner']}.",
                    "changelog": [
                        "Clean-room reconstruction established under REL-007."
                    ],
                    "historical_non_claim": "No unavailable historical body or validation is claimed.",
                }
    ...
    for record in owners:
        behavior_cases.append(
            {
                "id": f"{record['kind']}:{record['source_id']}",
                "owner": record["owner"],
                "positive": {
    ...
    dump(
        root / "registry" / "declared_suite_behavior_cases.json",
        {
            "schema_version": "1.0",
            "case_count": len(behavior_cases),
    ...
    for number, data in packs.items():
        dump(
            evidence_dir / f"pack-{number}-build-qa.json",
            {
                "pack": number,
                "status": "operational_tests_pending_full_suite",
                "owner": data["owner"],
            },
        )
        dump(
            evidence_dir / f"pack-{number}-certification-report.json",
            {
                "pack": number,
                "status": "provisional",
                "revocable": True,
                "blockers": [
                    "final full-suite and installed-package gates not yet recorded"
                ],
            },
        )
        pack_paths = [
            root / ".agents" / "skills" / data["owner"] / "SKILL.md",
            root / "registry" / "skill_packages" / f"{data['owner']}.json",
        ]
        dump(
            evidence_dir / f"pack-{number}-sha256sums.json",
            {
                "pack": number,
                "files": [
    ...
    target_by_kind = {
        "registry": [
            "registry/declared_suite_pack_index.json",
            "registry/declared_suite_dependency_graph.json",
        ],
        ...
        "test-or-evaluation": [
            "registry/declared_suite_behavior_cases.json",
            "tests/test_declared_suite_domains.py",
        ],
    }
    for card in support:
        if card["kind"] == "schema":
            ...
            card["implementation_targets"] = [
                f"evidence/declared-suite/{card['source_id']}.json"
            ]
            card["current_state"] = "provisional_pending_final_gates"
        elif card["kind"] == "reference-or-knowledge":
            card["implementation_targets"] = (
                [
                    "registry/declared_suite_formulas.json",
                    "runtime/declared_suite_formulas.py",
                ]
                if card["source_id"] in formula_names
                else ["registry/declared_suite_knowledge.json"]
            )
            card["current_state"] = "implemented_pending_validation"
        else:
    ...
    print(
        json.dumps(
            {
                "schemas": len(schema_targets),
                "formulas": len(formulas),
                "behavior_cases": len(behavior_cases),
                "support_cards": len(support),
            },
            sort_keys=True,
        )
    )
