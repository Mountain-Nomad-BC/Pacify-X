...
DOMAINS = {
    ...
    "03": {
        ...
        "steps": [
            "define-observable-claim",
            "select-independent-oracle",
            "generate-positive-and-adversarial-cases",
            "run-and-minimize",
            "score-and-promote-regressions",
        ],
    ...
    "04": {
        ...
        "steps": [
            "bind-project-and-subject-scope",
            "validate-ingestion-and-provenance",
            "retrieve-and-budget-context",
            "record-trace-and-quality",
            "retain-promote-or-purge-by-policy",
        ],
    ...
    "05": {
        ...
        "steps": [
            "classify-assets-and-trust-boundaries",
            "enumerate-threats-and-authority",
            "scan-and-contain",
            "verify-provenance-and-reproducibility",
            "release-or-quarantine-with-evidence",
        ],
    ...
    "06": {
        ...
        "steps": [
            ...
            "validate-contract-and-compatibility",
            "model-capacity-cost-and-quality",
            "stage-canary-with-fallback",
            "observe-decide-and-handoff",
        ],
    ...
    "07": {
        ...
        "steps": [
            "freeze-scope-and-denominators",
            "reconcile-owners-and-artifacts",
            "run-independent-completion-gates",
            "issue-hash-bound-certificate",
            "monitor-revoke-and-recertify",
        ],
...
def contract(card: dict, domain: dict) -> dict:
    ...
    return {
        ...
        "evidence": [
            "normalized inputs",
            "owner and dependency resolution",
            "structured result",
...
def workflow(card: dict, domain: dict) -> dict:
    return {
        ...
        "preconditions": [
            ...
            "declared authority and budgets",
            "available verification oracle",
        ],
        ...
        "steps": [
            {
                "id": step,
                "skill": domain["owner"],
                "depends_on": [] if index == 0 else [domain["steps"][index - 1]],
            }
...
def package(root: Path, domain: dict, tests: str) -> dict:
    ...
    return {
        ...
        "provenance": {
            "type": "clean_room_reconstruction_from_declared_outcomes",
            "basis": ["registry/declared_suite_reconstruction.json"],
        },
        "clean_room": True,
        "tests": tests,
        "evidence": tests,
...
def append_catalog(root: Path, domains: list[dict]) -> None:
    ...
    for domain in domains:
        tags = ", ".join(
            json.dumps(tag) for tag in domain["owner"].split("-") + ["declared-suite"]
        )
        block = f'''[[skills]]
id = "{domain["owner"]}"
version = "0.1.0"
...
def update_recovery_routes(root: Path, ledger: dict) -> None:
    ...
    owners_by_alias = {
        record["declared_outcome"]: record["canonical_owner"]
        for record in recovery["records"]
    }
    ...
    for alias, owner in owners_by_alias.items():
        records_by_alias[alias] = {
            "alias": alias,
            "owner": owner,
            "source_state": "manifest-only",
            "coverage": "clean-room-reconstruction",
        }
    aliases["records"] = sorted(
        records_by_alias.values(), key=lambda record: record["alias"]
    )
    aliases["rule"] = (
        "Aliases route advertised outcomes to independently tested clean-room domain owners; exact-recovered records retain their hash-backed owner."
    )
...
def update_specialty_projection(root: Path, domains: list[dict]) -> None:
    ...
    value["framework_only_active"] = sorted(
        set(value["framework_only_active"]) | {domain["owner"] for domain in domains}
    )
    dump(path, value)
...
def main() -> int:
    ...
    for number, domain in DOMAINS.items():
        domain_cards = [
            ...
            if c["class"] == "operational_outcome" and pack_number(c["pack"]) == number
        ]
        skill_dir = root / ".agents" / "skills" / domain["owner"]
        if not skill_dir.is_dir():
            raise SystemExit(f"initialize skill before building: {domain['owner']}")
        (skill_dir / "SKILL.md").write_text(
            skill_markdown(domain), encoding="utf-8", newline="\n"
        )
        skill_contracts = [
            contract(c, domain) for c in domain_cards if c["kind"] == "skill"
        ]
        script_contracts = [
            contract(c, domain) for c in domain_cards if c["kind"] == "script"
        ]
        dump(
        ...
        dump(
            skill_dir / "references" / "script-contracts.json",
            {
                "schema_version": "1.0",
                "owner": domain["owner"],
                "contracts": script_contracts,
            },
        )
        ...
        for card in domain_cards:
            ...
            card["wiring_targets"] = [
                ...
                f"registry/skill_packages/{domain['owner']}.json",
            ]
            ownership.append(
                {
                    "kind": card["kind"],
        ...
        dump(
            root / "registry" / "skill_packages" / f"{domain['owner']}.json",
            package(root, domain, "tests/test_declared_suite_domains.py"),
        )
        domains_built.append(
            {
                "pack": number,
                "owner": domain["owner"],
                "cards": len(domain_cards),
                "skills": len(skill_contracts),
                "scripts": len(script_contracts),
                "orchestrations": sum(
                    c["kind"] == "orchestration" for c in domain_cards
                ),
            }
        )

    dump(
        root / "registry" / "declared_outcome_owners.json",
        {
            "schema_version": "1.0",
            "status": "contract_wired_not_verified",
            "record_count": len(ownership),
            "records": sorted(ownership, key=lambda x: (x["kind"], x["source_id"])),
        },
    )
    dump(
        root / "orchestration" / "workflows" / "declared-suite.yaml",
        {
            "schema_version": "1.0",
            "status": "contract_wired_not_verified",
            "workflow_count": len(workflows),
            "workflows": sorted(workflows, key=lambda x: x["id"]),
        },
    )
    ledger["summary"]["contract_and_owner_wired_cards"] = sum(
        c["current_state"] == "contract_and_owner_wired_not_verified"
        for c in ledger["cards"]
    )
    ...
    dump(
        root / "evidence" / "declared-suite" / "domain-build-receipt.json",
        {
            "schema_version": "1.0",
            "status": "contract_wired_not_verified",
    ...
    print(
        json.dumps(
            {
                "domains": len(domains_built),
                "owners": len(ownership),
                "workflows": len(workflows),
                "states": dict(Counter(c["current_state"] for c in ledger["cards"])),
            },
            sort_keys=True,
        )
    )
