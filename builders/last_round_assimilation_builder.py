...
def disposition_for(relative: Path, suite: str) -> tuple[str, str]:
    ...
    if suite == "complete":
        ...
        if "/orchestrations/" in f"/{text}" or name == "orchestrations.json":
            return (
                "merge_authoritative_orchestration",
                "canonical declared-suite workflow registry",
            )
        if "/scripts/" in f"/{text}" and name in UNSAFE_SCRIPT_RULES:
            return (
                "reject_unsafe_body_use_safe_replacement",
                "existing fail-closed runtime owner",
            )
        if "/scripts/" in f"/{text}" and name.endswith(".py"):
            return "admit_namespaced_tool", "owning domain skill scripts"
        if "/schemas/" in f"/{text}":
            return (
                "merge_authoritative_schema",
                "declared-suite schema contract registry",
            )
        ...
        if name in {"formulas.json", "knowledge.json"} or "/knowledge/" in f"/{text}":
            return (
                "merge_operational_knowledge",
                "declared-suite formula/knowledge registries",
            )
        if (
            "/tests/" in f"/{text}"
            or "/evidence/" in f"/{text}"
            or "/certification/" in f"/{text}"
        ):
            return "retain_validation_evidence", "REL-008 evidence ledger"
        return (
            "retain_provenance_or_package_metadata",
            "REL-008 source disposition ledger",
        )
    if suite == "scheduler":
        ...
        if "/orchestrations/" in f"/{text}":
            return (
                "compose_with_existing_orchestration",
                "orchestration/workflows/capability-scheduling.yaml",
            )
        if "/src/scheduler/" in f"/{text}" and name == "engine.py":
            return (
                "admit_and_harden_observe_only_runtime",
                "runtime/capability_scheduler.py",
            )
        if "/scripts/" in f"/{text}" and name.endswith(".py"):
            return (
                "retain_validation_or_cli_reference",
                "runtime/capability_scheduler.py and REL-008 evidence",
            )
        if "/schemas/" in f"/{text}":
    ...
    if (
        ...
        return (
            "consolidate_lazy_capability_contract",
            "metacognitive capability registry and canonical owners",
        )
    if "/orchestration/" in f"/{text}" or name == "orchestrations.json":
        return (
            "compose_with_existing_orchestration",
            "metacognitive workflow projection",
        )
    if "/runtime/" in f"/{text}" and name.endswith(".py"):
        return (
            "admit_namespaced_runtime",
            "engineering_bootstrap.metacognitive_evolution",
        )
...
def enrich_declared_suite(root: Path, complete: Path) -> dict:
    ...
    for pack in sorted((complete / "packs").iterdir()):
        ...
        for name, target in (
            ...
            if path.exists():
                value = load(path)
                records = value.get(
                    name.removesuffix(".json"), value.get("records", [])
                )
                for record in records:
                    target.append(
                        {"pack": number, "source_sha256": digest(path), **record}
                    )
        ...
        for path in (
            sorted((pack / "templates").glob("*"))
            if (pack / "templates").exists()
            else []
        ):
            if path.is_file():
                target = (
                    root / "templates" / "declared_suite" / f"pack-{number}-{path.name}"
                )
                ...
                templates.append(
                    {
                        "pack": number,
                        "source": path.relative_to(complete).as_posix(),
                ...
                validation_scripts.append(
                    {
                        ...
                        "source_sha256": digest(path),
                        "disposition": "validation_only_existing_framework_gate",
                    }
                )
                continue
            if path.name in UNSAFE_SCRIPT_RULES:
                rejected_scripts.append(
                    {
                        "pack": number,
                        "id": path.stem.replace("_", "-"),
                        "source_sha256": digest(path),
                        "reason": UNSAFE_SCRIPT_RULES[path.name],
                    }
                )
                continue
            target = (
                ...
                / PACK_OWNERS[number]
                / "scripts"
                / f"authoritative_{path.name}"
            )
            shutil.copy2(path, target)
            admitted_scripts.append(
                {
                    "pack": number,
                    "id": path.stem.replace("_", "-"),
                    "source_sha256": digest(path),
    ...
    for number, owner in PACK_OWNERS.items():
        for filename, kind in (
            ("capability-contracts.json", "skill"),
            ("script-contracts.json", "script"),
        ):
            path = root / ".agents" / "skills" / owner / "references" / filename
            value = load(path)
            for contract in value["contracts"]:
                match = (
                    skills.get((number, contract["id"])) if kind == "skill" else None
                )
                if match:
                    source_path, authoritative = match
                    contract["authoritative_contract"] = authoritative
                    contract["authoritative_source"] = source_path.relative_to(
                        complete
                    ).as_posix()
                    ...
                    script = next(
                        (
                            x
                            for x in admitted_scripts
                            + rejected_scripts
                            + validation_scripts
                            if x["pack"] == number and x["id"] == contract["id"]
                        ),
                        None,
                    )
                    if script:
                        contract["authoritative_source_sha256"] = script[
                            "source_sha256"
                        ]
                        contract["implementation_disposition"] = (
                            "namespaced_exact_body"
                            if "target" in script
                            else script.get("disposition", "safe_replacement")
                        )
                        if "target" in script:
            ...
            dump(
                path.parent / f"{split_name}-index.json",
                {"schema_version": "1.0", "count": len(index), "records": index},
            )
    ...
    for workflow in workflows["workflows"]:
        match = next(
            (
                (path, item)
                for (number, item_id), (path, item) in orchestrations.items()
                if item_id == workflow["id"]
            ),
            None,
        )
        if match:
            ...
            workflow["authoritative_source"] = source_path.relative_to(
                complete
            ).as_posix()
    ...
    dump(
        ...
        {
            "schema_version": "2.0",
            "source": "REL-008 authoritative suite",
            "formula_count": len(formulas),
            "formulas": formulas,
        },
    )
    dump(
        root / "registry" / "declared_suite_knowledge.json",
        {
            "schema_version": "2.0",
            "source": "REL-008 authoritative suite",
            "count": len(knowledge),
    ...
    dump(
        root / "registry" / "declared_suite_schema_contracts.json",
        {"schema_version": "1.0", "count": len(schemas), "records": schemas},
    )
    dump(
        root / "registry" / "declared_suite_authoritative_tools.json",
        {
            "schema_version": "1.0",
            "admitted": admitted_scripts,
            "rejected": rejected_scripts,
            "validation_only": validation_scripts,
        },
    )
    return {
        "contracts_enriched": enriched,
        "skills": len(skills),
        ...
        "admitted_scripts": len(admitted_scripts),
        "rejected_scripts": len(rejected_scripts),
...
def integrate_metacognitive(root: Path, meta: Path) -> dict:
    ...
    owners = []
    for skill in skills:
        composed_owner = DOMAIN_OWNERS.get(
            skill["domain"], "govern-metacognitive-evolution"
        )
        owners.append(
            {
                "id": skill["id"],
                "domain": skill["domain"],
                "owner": "govern-metacognitive-evolution",
                "composed_with": composed_owner,
                "disposition": "extend_or_compose",
                "version": skill["version"],
            }
        )
    dump(
        root / "registry" / "metacognitive_capabilities.json",
        {"schema_version": "1.0", "count": len(skills), "capabilities": skills},
    )
    dump(
        root / "registry" / "metacognitive_capability_owners.json",
        {"schema_version": "1.0", "count": len(owners), "records": owners},
    )
    dump(root / "registry" / "metacognitive_formulas.json", formulas)
    dump(root / "registry" / "metacognitive_policies.json", policies)
    dump(
        root / "orchestration" / "workflows" / "metacognitive-evolution.yaml",
        {
            "schema_version": "1.0",
    ...
    dump(
        skill_dir / "references" / "capability-contracts.json",
        {"schema_version": "1.0", "count": len(skills), "contracts": skills},
    )
    ...
    dump(
        skill_dir / "references" / "capability-index.json",
        {"schema_version": "1.0", "count": len(meta_index), "records": meta_index},
    )
    dump(
        skill_dir / "references" / "orchestration-index.json",
        {
            "schema_version": "1.0",
            "count": len(workflows),
            "workflows": [
                {"id": x["id"], "trigger": x["trigger"], "steps": x["steps"]}
                for x in workflows
            ],
        },
    )
...
def integrate_scheduler(root: Path, scheduler: Path) -> dict:
    ...
    for source_dir in sorted((scheduler / ".agents" / "skills").iterdir()):
        ...
        capability = {
            ...
            "source_contract_sha256": digest(reference_path)
            if reference_path.is_file()
            else None,
    ...
    dump(
        skill_dir / "references" / "capability-index.json",
        {
            ...
            "records": capability_index,
        },
    )
    dump(
        root / "registry" / "scheduling_capabilities.json",
        {
            "schema_version": "1.0",
    ...
    owners = [
        {
            ...
            "composed_with": item["composed_with"],
            "disposition": "bounded_extension",
        }
        for item in capabilities
    ]
    dump(
        root / "registry" / "scheduling_capability_owners.json",
        {"schema_version": "1.0", "count": len(owners), "records": owners},
    )
    ...
    for source in sorted((scheduler / "orchestrations").glob("*.yaml")):
        ...
        workflow_index.append(
            {
                "id": workflow["name"],
                "path": "orchestration/workflows/capability-scheduling.yaml",
                "source_sha256": digest(source),
            }
        )
    dump(
        root / "orchestration" / "workflows" / "capability-scheduling.yaml",
        {
            "schema_version": "1.0",
            "workflow_count": len(workflows),
            "workflows": workflows,
    ...
    dump(
        skill_dir / "references" / "workflow-index.json",
        {
            "schema_version": "1.0",
            "count": len(workflow_index),
            "records": workflow_index,
        },
    )
    ...
    dump(
        root / "registry" / "scheduling_policies.json",
        {"schema_version": "1.0", "count": len(policies), "policies": policies},
    )
...
def wire_product(root: Path) -> dict:
    for owner in PACK_OWNERS.values():
        ...
        package["provenance"] = {
            ...
            "basis": [
                "registry/declared_suite_authoritative_tools.json",
                "registry/declared_suite_schema_contracts.json",
                "evidence/rel008-assimilation.json",
            ],
            "unsafe_body_policy": "shell execution and hard deletion bodies rejected in favor of tested bounded replacements",
        }
        package["capability_tags"] = sorted(
            set(package.get("capability_tags", [])) - {"reconstruction"}
            | {"authoritative"}
        )
    ...
    skill_body = (
        root / ".agents" / "skills" / "govern-metacognitive-evolution" / "SKILL.md"
    )
    package = {
        ...
        "provenance": {
            "type": "sanitized_authoritative_assimilation",
            "basis": ["evidence/rel008-assimilation.json"],
        },
    ...
    scheduling_body = (
        root / ".agents" / "skills" / "orchestrate-capability-scheduling" / "SKILL.md"
    )
    dump(
        root / "registry" / "skill_packages" / "orchestrate-capability-scheduling.json",
        {
            ...
            "tests": "tests/test_capability_scheduler.py",
            "evidence": "evidence/rel008-assimilation.json",
            "validation_freshness": "pending",
            "context_budget_bytes": 32768,
        },
...
"""
    ...
    catalog = (
        pattern.sub(block.rstrip(), catalog)
...
"""
    ...
    catalog = (
        scheduling_pattern.sub(scheduling_block.rstrip(), catalog)
        if scheduling_pattern.search(catalog)
        else catalog.rstrip() + "\n\n" + scheduling_block
    )
    ...
    by_id["orchestrate-capability-scheduling"] = {
        "id": "orchestrate-capability-scheduling",
        "source_disposition": "bounded_extension",
        "implementation": "sanitized_authoritative_observe_only_runtime",
        "status": "active",
        "validation": {"passed": 0, "failed": 0, "state": "pending_REL008_gates"},
        "effects": ["read_local", "write_workspace"],
    ...
    for record in load(root / "registry" / "metacognitive_capability_owners.json")[
        "records"
    ]:
        ...
        by_alias[alias] = {
            ...
            "coverage": "typed_contract_and_runtime_layer",
        }
    for record in load(root / "registry" / "scheduling_capability_owners.json")[
        "records"
    ]:
        alias = record["id"].replace("-", " ")
        by_alias[alias] = {
            ...
            "owner": "orchestrate-capability-scheduling",
            "source_state": "authoritative",
            "coverage": "typed_contract_and_observe_only_runtime",
        }
    aliases["records"] = sorted(by_alias.values(), key=lambda item: item["alias"])
    aliases["rule"] = (
        "Aliases route exact authoritative outcomes to one admitted lazy owner; composed_with records identify existing canonical stores and policy owners."
    )
    ...
    for record in recovery["records"]:
        owner_body = (
            root / ".agents" / "skills" / record["canonical_owner"] / "SKILL.md"
        )
        record["owner_body_sha256"] = digest(owner_body)
        if (
            record.get("pack", "")[0:2] in PACK_OWNERS
            or record.get("canonical_owner") in PACK_OWNERS.values()
        ):
    ...
    specialty["framework_only_active"] = sorted(
        set(specialty["framework_only_active"])
        | {"govern-metacognitive-evolution", "orchestrate-capability-scheduling"}
    )
    dump(specialty_path, specialty)

    capabilities = load(root / "registry" / "metacognitive_capabilities.json")[
        "capabilities"
    ]
    workflows = load(
        root / "orchestration" / "workflows" / "metacognitive-evolution.yaml"
            ...
            edges.append(
                {
                    ...
                    "target": dependency,
                    "type": "depends_on",
                    "target_scope": "metacognitive"
                    if dependency in known
                    else "existing_framework",
                }
            )
    for workflow in workflows:
        for step in workflow.get("steps", []):
            edges.append(
                {
                    "source": workflow["id"],
                    "target": step["skill_id"],
                    "type": "orchestrates",
                    "order": step["order"],
                }
            )
    dump(
        root / "registry" / "metacognitive_dependency_graph.json",
        {
            "schema_version": "1.0",
            "nodes": nodes
            + [{"id": x["id"], "kind": "orchestration"} for x in workflows],
            "edges": edges,
        },
    )
    ...
    by_integration["metacognitive-evolution-runtime"] = {
        ...
        "provides": [
            "bounded_metacognitive_analysis",
            "semantic_effect_lint",
            "revocable_improvement_proposals",
        ],
        ...
        "rollback": {
            "strategy": "disable integration record; runtime is read-only and retains no canonical state"
        },
    }
    by_integration["capability-scheduling-runtime"] = {
        "id": "capability-scheduling-runtime",
        "version": "1.0.0",
        "owner": "runtime/capability_scheduler.py",
        "status": "active",
        ...
        "provides": [
            "deterministic_observe_only_schedule",
            "hard_gate_reason_codes",
            "replayable_decision_hash",
        ],
        ...
        "approval": {"required": False},
        "rollback": {
            "strategy": "disable integration record; scheduler is observe-only and retains no canonical state"
        },
    }
    integrations["integrations"] = sorted(
        by_integration.values(), key=lambda item: item["id"]
    )
    ...
    for schema_path in sorted((root / "contracts" / "metacognitive").glob("*.json")):
        ...
        by_contract_path[relative] = {
            ...
            "consumers": [
                ...
                "runtime/metacognitive_evolution/facade.py",
            ],
            "tests": [
                "tests/test_contract_runtime.py",
                "tests/test_metacognitive_evolution.py",
            ],
    ...
    for schema_path in sorted((root / "contracts" / "scheduling").glob("*.json")):
        ...
        by_contract_path[relative] = {
            ...
            "tests": [
                "tests/test_contract_runtime.py",
                "tests/test_capability_scheduler.py",
            ],
            "enforcement": "scheduling_runtime_boundary",
            "packaged": True,
        }
    ownership["records"] = sorted(
        by_contract_path.values(), key=lambda item: item["path"]
    )
    ...
    return {
        ...
        "aliases": len(capabilities) + 30,
        "graph_nodes": len(nodes) + len(workflows),
        "graph_edges": len(edges),
        "owned_contracts": len(
            list((root / "contracts" / "metacognitive").glob("*.json"))
        )
        + len(list((root / "contracts" / "scheduling").glob("*.json"))),
    }


def file_ledger(source_root: Path, root: Path) -> dict:
    records = []
    for suite_dir, suite in (
        (source_root / "complete-expert-capability-system_v1.0.0", "complete"),
        (
            source_root / "metacognitive_capability_evolution_pack_v1.0.0",
            "metacognitive",
        ),
        (
            source_root / "capability_orchestration_scheduling_system_v1.0.0",
            "scheduler",
        ),
    ):
            ...
            records.append(
                {
                    ...
                    "target_owner_or_surface": target,
...
def main() -> int:
    ...
    dump(
        root / "registry" / "semantic_capability_index.json", build_semantic_index(root)
    )
    ...
    receipt = {
        ...
        "source_files": ledger["source_file_count"],
        "open_file_dispositions": ledger["open_count"],
